...
UsbClientDevice::~UsbClientDevice()
{
    ...
    CloseTransfer();
	while(m_pClassInterfaceList){ // Delete all the Interfaces that were created
...
BOOL UsbClientDevice::Init()
{
    ...
    if (m_szUniqueDriverId &&m_hSetupComleteEvent!=NULL && Attach() &&
		...
		{
			if(DriverSettings.dwProductId == 0x1414)
			{
				m_hDriver = m_hDriver_1414[0] = ActivateDevice(ClientRegistryPath,(DWORD) this);
				m_hDriver_1414[1] = ActivateDevice(ClientRegistryPath,(DWORD) this);
				m_hDriver_1414[2] = ActivateDevice(ClientRegistryPath,(DWORD) this);
				m_hDriver_1414[3] = ActivateDevice(ClientRegistryPath,(DWORD) this);
			}
			else if(DriverSettings.dwProductId == 0x1412)
			{
				m_hDriver = m_hDriver_1414[0] = ActivateDevice(ClientRegistryPath,(DWORD) this);
				m_hDriver_1414[1] = ActivateDevice(ClientRegistryPath,(DWORD) this);
			}else if(DriverSettings.dwProductId == 0x1410)
			{
				m_hDriver = m_hDriver_1414[0] = ActivateDevice(ClientRegistryPath,(DWORD) this);
			}
...
BOOL UsbClientDevice::Attach()
{
    ...
    if (m_lpInputInterface!=NULL) { // This driver is loaded by Interface So.
        ...
        if (pDevice) {
            ...
            while (dwNumInterfaces && pUsbInterface && pSupportedInterface ) {
...
BOOL UsbClientDevice::Detach()
{
    ...
    if (m_hDriver) {
		...
		//DeactivateDevice(m_hDriver_1414[0]);
		if(DriverSettings.dwProductId == 0x1414)
		{
			DeactivateDevice(m_hDriver_1414[1]);
			DeactivateDevice(m_hDriver_1414[2]);
			DeactivateDevice(m_hDriver_1414[3]);
			m_hDriver_1414[0] = m_hDriver_1414[1] = m_hDriver_1414[2] = m_hDriver_1414[3] = NULL;
		}
		else if(DriverSettings.dwProductId == 0x1412)
		{
			DeactivateDevice(m_hDriver_1414[1]);
