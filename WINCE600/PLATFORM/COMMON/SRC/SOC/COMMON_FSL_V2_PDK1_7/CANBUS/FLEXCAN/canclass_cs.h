...
class CANClass
{
	...
	HANDLE	m_hRevEvent;						// received event
	HANDLE	m_hErrEvent;						// error event

	class RingBuffer* pTxRing[8];     
	class RingBuffer* pRxRing;
...
public:
	...
	BOOL ReadPacket( PCAN_PACKET pCANPk );
	BOOL WritePacket( PCAN_PACKET pCANPkt );

	BOOL Start( );
	BOOL Stop( );
	BOOL Reset( ) ;// Only reset CAN, does not affect the class fields
