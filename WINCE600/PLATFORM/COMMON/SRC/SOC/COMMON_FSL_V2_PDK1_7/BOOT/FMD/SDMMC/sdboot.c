...
#define NUMSECTORS_REQUIRED_FOR_BOOTIMAGES (SDImageCfg.dwSdSize / ESDHC_SECTOR_SIZE)
...
BOOL SDHCWriteXldr(DWORD dwStartAddr, DWORD dwLength)
{
    ...
    pSectorBuf = pImage;
    sectorAddr = SDImageCfg.dwXldrOffset / flashInfo.dwBytesPerBlock;
...
BOOL SDHCWriteBoot(DWORD dwStartAddr, DWORD dwLength)
{
    ...
    // Fill unused space with 0xFF
    memset(pImage + dwLength, 0xFF, (SDImageCfg.dwBootSize - dwLength));

    OALMSG(1, (_T("INFO: Programming EBOOT/SBOOT image from flash cache address 0x%x, size = %d\r\n"), pImage, dwLength));
    
    // Write EBOOT to SDHC flash
    pSectorBuf = pImage;
    startSectorAddr = SDImageCfg.dwBootOffset / flashInfo.dwBytesPerBlock;;
    dwNumSectors = (SDImageCfg.dwBootSize / flashInfo.dwBytesPerBlock);
    ...
    if(memcmp(pImage, pImage + SDImageCfg.dwBootSize, SDImageCfg.dwBootSize) != 0)
    {
        OALMSG(1, (_T("ERROR: Failed to verify EBOOT/SBOOT\r\n")));
        return FALSE;
    }
...
BOOL SDHCWriteSB(DWORD dwStartAddr, DWORD dwLength)
{
    ...
    if (dwLength % flashInfo.dwBytesPerBlock)
    {
        ...
        // Fill unused space with 0xFF
        memset(pImage + dwLength, 0xFF, (dwBootSize - dwLength));
    }
    else
    {
        dwBootSize = dwLength;
    ...
    pSectorBuf = pImage;
    startSectorAddr = SDImageCfg.dwBootOffset / flashInfo.dwBytesPerBlock;;
    endSectorAddr = startSectorAddr + (dwBootSize / flashInfo.dwBytesPerBlock);
...
BOOL SDHCWriteNK(DWORD dwStartAddr, DWORD dwLength)
{
    ...
    pSectorBuf = pImage;
    startSectorAddr = SDImageCfg.dwNkOffset / flashInfo.dwBytesPerBlock;;
    ...
    if (SDImageCfg.dwNkSize - dwLength >= NK_UNUSED_BLOCKS *flashInfo.dwBytesPerBlock )
    {
        sectorAddr = startSectorAddr + SDImageCfg.dwNkSize/flashInfo.dwBytesPerBlock - 1;
...
BOOL SDHCLoadNK(VOID)
{
    ...
    startSectorAddr = SDImageCfg.dwNkOffset / flashInfo.dwBytesPerBlock;
    endSectorAddr = startSectorAddr + (SDImageCfg.dwNkSize / flashInfo.dwBytesPerBlock);    
...
BOOL SDHCCreateFileSystemPartition(void)
{
    ...
    DWORD startSect1 = SDImageCfg.dwXldrOffset / ESDHC_SECTOR_SIZE;
    ...
    // PC tool is available for i.MX233 to perform this same task, and that would avoid overwriting the existing boot code
    if(SDImageCfg.dwSocId == 233)
    {
...
BOOL SDHCLoadBootCFG(BYTE *pBootCfg, DWORD cbBootCfgSize)
{
    ...
    {
        cbBootCfgSize = SDImageCfg.dwCfgSize;
    }

    // Calculate the physical block range for the boot configuration
    startSectorAddr = SDImageCfg.dwCfgOffset / flashInfo.dwBytesPerBlock;
    dwNumSectors = (cbBootCfgSize + flashInfo.dwBytesPerBlock - 1) / flashInfo.dwBytesPerBlock;

    // For MX233, the offset is already provided as the sector address
    if (SDImageCfg.dwSocId == 233)
    {
        startSectorAddr = SDImageCfg.dwCfgOffset;
    }
