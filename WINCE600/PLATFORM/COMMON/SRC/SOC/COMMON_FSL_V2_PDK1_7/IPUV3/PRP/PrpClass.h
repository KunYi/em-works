...
class PrpClass 
{
    public:
        ...
        BOOL  PrpAddInputBuffer(UINT32 *pBufs, BOOL bWait, UINT8 IntType);
        BOOL  PrpAddInputCombBuffer(UINT32 *pBufs);
    ...
    private:
        ...
        BOOL PrpConfigureResize4OversizeWidth(UINT16 inSize, UINT16 outSize, prpOutputChannel outputChannel);
        ...
        BOOL    m_bENCOversizedFrame;   //The output frame is too large so that we need to split it. 
        BOOL    m_bVFOversizedFrame;   //The output frame is too large so that we need to split it. 
