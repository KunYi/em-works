...
void  ENETClass::ENETSetMII(  IN PENET_ENET_PRIVATE pENET  )
{
    ...
    {
        BW_ENET_MAC_RCR_MAX_FL(indextmp,PKT_MAXBUF_SIZE);
        BW_ENET_MAC_RCR_MII_MODE(indextmp,1);                    
        HW_ENET_MAC_TCR_WR(indextmp,0x0);
        {
            if (pENET->fUseRMII == FALSE)
            {
                // Set MII speed to 2.5MHz
                pENET->MIIPhySpeed = 40;
                BW_ENET_MAC_RCR_RMII_MODE(indextmp, 0);
            ...
            else
            {
                ...
                BW_ENET_MAC_RCR_RMII_MODE(indextmp, 1);
            }

            BW_ENET_MAC_MSCR_MII_SPEED(indextmp,pENET->MIIPhySpeed);
        }
    }    
    DEBUGMSG(ZONE_FUNCTION, (TEXT("ENET: -ENETSetMII\r\n")));
}
    ...
    )
{
   ...
   DEBUGMSG(ZONE_FUNCTION, (TEXT("ENET: +ENETGetPHYId  pENET->MIIPhyAddr :%d \r\n"), pENET->MIIPhyAddr));
    ...
    pENET->MIIPhySpeed = 0;
    BW_ENET_MAC_MSCR_MII_SPEED(0,pENET->MIIPhySpeed);       
    DEBUGMSG(ZONE_INFO,
...
//
//------------------------------------------------------------------------------
void ENETClass::ENETPHYID( IN NDIS_HANDLE MiniportAdapterHandle)
{
    pENET_t pEthernet = ((pENET_t)(MiniportAdapterHandle));
    PENET_ENET_PRIVATE pENET = &(pEthernet->ENETPrivateInfo);
...
BOOL ENETClass::ENETEnetInit(IN NDIS_HANDLE MiniportAdapterHandle )
{
    ...
    for(indextmp=0;indextmp<2;indextmp++)
    {
        ...
        BSPENETIomuxConfig( indextmp,TRUE );
        BSPENETClockConfig( indextmp,TRUE );
    }
    ...
    for(indextmp=0;indextmp<2;indextmp++)
    {
        BW_ENET_MAC_ECR_RESET(indextmp,1);

        while(HW_ENET_MAC_ECR_RD(indextmp)&BM_ENET_MAC_ECR_RESET); 

        BW_ENET_MAC_ECR_ENA_1588(indextmp,1);
        //Enable magic wake up
        BW_ENET_MAC_ECR_MAGIC_ENA(indextmp,1);

        // set the receive and transmit BDs ring base to 
        // hardware registers(ERDSR & ETDSR)
      
        HW_ENET_MAC_ERDSR_WR(indextmp,(long)pENET->RingPhysicalBase.QuadPart);
        HW_ENET_MAC_ETDSR_WR(indextmp,(long)pENET->RingPhysicalBase.QuadPart + ENET_RX_RING_SIZE*sizeof(EBUFFER_DESC));
        
                   
        // set other hardware registers
        HW_ENET_MAC_EIR_WR(indextmp,0x0);
        

        HW_ENET_MAC_IAUR_WR(indextmp,0x0);
        HW_ENET_MAC_IALR_WR(indextmp,0x0);
        HW_ENET_MAC_GAUR_WR(indextmp,0x0);
        HW_ENET_MAC_GALR_WR(indextmp,0x0);

       
        HW_ENET_MAC_EMRBR_WR(indextmp,PKT_MAXBLR_SIZE);
        
        Sleep(10);

        HW_ENET_MAC_EIMR_WR(indextmp,BM_ENET_MAC_EIR_RXF|BM_ENET_MAC_EIR_TXF|BM_ENET_MAC_EIR_MII|BM_ENET_MAC_EIR_TS_AVAIL|BM_ENET_MAC_EIR_TS_TIMER);   

     
        BW_ENET_MAC_TCR_FEDN(indextmp,1); 
        BW_ENET_MAC_RCR_LOOP(indextmp,0);
        
        // Set the station address for the ENET Adapter
        HW_ENET_MAC_PALR_WR(indextmp,pEthernet->EnetMacAddress[3] |
                                   pEthernet->EnetMacAddress[2]<<8 |
                                   pEthernet->EnetMacAddress[1]<<16 |
                                   pEthernet->EnetMacAddress[0]<<24);
                                   

        HW_ENET_MAC_PAUR_WR(indextmp,pEthernet->EnetMacAddress[5]<<16 |
                                   (pEthernet->EnetMacAddress[4]+(UCHAR)(indextmp))<<24);
    }
    HW_ENET_SWI_ERDSR_WR((long)pENET->RingPhysicalBase.QuadPart);
    HW_ENET_SWI_ETDSR_WR((long)pENET->RingPhysicalBase.QuadPart + ENET_RX_RING_SIZE*sizeof(EBUFFER_DESC));
    HW_ENET_SWI_EMRBR_WR(PKT_MAXBLR_SIZE);
    ...
    for(indextmp=0;indextmp<2;indextmp++)
    {
        BW_ENET_MAC_RCR_FCE(indextmp,1);

        BW_ENET_MAC_RCR_PROM(indextmp,1);
        // BW_ENET_MAC_RCR_BC_REJ(indextmp,1);
        BW_ENET_MAC_RCR_CRC_FWD(indextmp,1);

        HW_ENET_MAC_TCR_WR(indextmp,0x1c);
        BW_ENET_MAC_TCR_TX_CRC_FWD(indextmp,0); 

        
        BW_ENET_MAC_ECR_ETHER_EN(indextmp,1);
    }

    BW_ENET_SWI_RDAR_RDAR(1);
    for(indextmp=0;indextmp<2;indextmp++)
    {
        BW_ENET_MAC_RCR_NO_LGTH_CHECK(indextmp, 1);
    }
...
void ENETClass::ENETEnetDeinit( IN NDIS_HANDLE MiniportAdapterHandle)
{
    ...
    for(indextmp=0;indextmp<2;indextmp++)
    {
        ...
        BSPENETIomuxConfig( indextmp,FALSE );
        BSPENETClockConfig( indextmp,FALSE );
    }
    ...
    )
{
    ...
    for(indextmp=0;indextmp<2;indextmp++)
    {
        ...
        else
        {
            ...
            BW_ENET_MAC_RCR_MAX_FL(indextmp,PKT_MAXBUF_SIZE);
            HW_ENET_MAC_EMRBR_WR(indextmp,PKT_MAXBLR_SIZE);
            BW_ENET_MAC_RCR_DRT(indextmp,1);
            HW_ENET_MAC_TCR_WR(indextmp,0);
        }
        ...
        BW_ENET_MAC_TCR_RFC_PAUSE(indextmp,1); 

        BW_ENET_MAC_TCR_TFC_PAUSE(indextmp,1); 
...
void ENETClass::AddMultiCast( UCHAR *pAddr )
{
    ...
    for(indextmp=0;indextmp<2;indextmp++)
    {
        ...
        if( HashValue > 31 )
            HW_ENET_MAC_GAUR_WR(indextmp,HW_ENET_MAC_GAUR_RD(indextmp)|(1 << (HashValue-32)));
        else
            HW_ENET_MAC_GALR_WR(indextmp,HW_ENET_MAC_GALR_RD(indextmp)|(1 << (HashValue)));
    }
