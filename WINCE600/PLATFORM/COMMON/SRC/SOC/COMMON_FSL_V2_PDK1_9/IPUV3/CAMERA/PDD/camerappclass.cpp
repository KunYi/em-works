...
void CameraPPClass::CameraPPWorkerLoop(UINT32 timeout)
{
    ...
    HANDLE hLoopEvent[NUM_PIN_BUFFER_MAX + 1];
    DWORD result = 0;

    hLoopEvent[0] = m_hExitCameraPPWorkerThread;

    for (i = 0; i<NUM_PIN_BUFFER_MAX; i++)
    {
        hLoopEvent[i + 1] = m_hCameraPPInputBufEvent[i];
    }

    // loop here
    while((result = WaitForMultipleObjects (NUM_PIN_BUFFER_MAX + 1, hLoopEvent, FALSE, timeout )) != WAIT_TIMEOUT)
    {
