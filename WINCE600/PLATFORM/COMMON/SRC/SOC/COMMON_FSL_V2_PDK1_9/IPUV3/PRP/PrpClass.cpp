...
BOOL PrpClass::PrpAddInputBuffer(UINT32 *pBuf, BOOL bWait, UINT8 IntType)
{
    ...
    if(m_ENCOversizedFrameFlag)
    {
            ...
            &&(m_PrpENCLastModule == prpSourceType_PRP))
        {
            ...
            //If DP buffer is not switched, wait for DP buffer switching
            if(IDMACChannelBUF0IsReady(IDMAC_CH_DP_PRIMARY_FLOW_AUX_PLANE))
            {
                while(IDMACChannelCurrentBufIsBuf1(IDMAC_CH_DP_PRIMARY_FLOW_AUX_PLANE))
                    Sleep(1);
            }
            else if(IDMACChannelBUF1IsReady(IDMAC_CH_DP_PRIMARY_FLOW_AUX_PLANE))
            {
                while(!IDMACChannelCurrentBufIsBuf1(IDMAC_CH_DP_PRIMARY_FLOW_AUX_PLANE))
                    Sleep(1);
            }
            ...
            //-----------------------------------------------
            //        processing half frame in the up-left                      
            //-----------------------------------------------
            //Start interrupt
            PrpIntrEnable(FRAME_INTERRUPT);            
            ...
            if ((m_ENCchannelParams.VOffset&&(m_ENCchannelParams.VOffset!=m_ENCchannelParams.UOffset))
                ||((m_ENCOversizedFrameFlag&PRP_HEIGHT_OVERSIZE)&&m_ENCchannelParams.UOffset))
            {
            ...
            PrpWaitForNotBusy(FRAME_INTERRUPT);

            if(m_ENCOversizedFrameFlag&PRP_HEIGHT_OVERSIZE)
            {
                //-----------------------------------------------
                //        processing half frame in the down-left                       
                //-----------------------------------------------
                //Start interrupt
                PrpIntrEnable(FRAME_INTERRUPT);
                //If v offset is not zero and u,v offset is different  it means YV12 format is used, 
                ...
                {
                    IDMACChannelDisable(IDMAC_CH_PRP_INPUT_VIDEO);
                    OffsetData.uOffset= m_ENCchannelParams.UOffset - 3*m_OversizedInputOffsetHeight/4;
                    OffsetData.vOffset = m_ENCchannelParams.VOffset - 3*m_OversizedInputOffsetHeight/4;
                    CPMEMWriteOffset(IDMAC_CH_PRP_INPUT_VIDEO,
                                     &OffsetData, OffsetData.bInterleaved);
                    IDMACChannelEnable(IDMAC_CH_PRP_INPUT_VIDEO);
                }
                else if(m_ENCchannelParams.UOffset)
                {
                    IDMACChannelDisable(IDMAC_CH_PRP_INPUT_VIDEO);
                    OffsetData.uOffset= m_ENCchannelParams.UOffset - m_OversizedInputOffsetHeight/2;
                    OffsetData.vOffset = m_ENCchannelParams.VOffset - m_OversizedInputOffsetHeight/2;
                    CPMEMWriteOffset(IDMAC_CH_PRP_INPUT_VIDEO,
                                     &OffsetData, OffsetData.bInterleaved);
                    IDMACChannelEnable(IDMAC_CH_PRP_INPUT_VIDEO);               
                }
                ...
                CPMEMWriteBuffer(IDMAC_CH_PRP_OUTPUT_ENC, EBAnum, (UINT32 *)(dwOutputBuf+m_OversizedOutputOffsetHeight));
                CPMEMWriteBuffer(IDMAC_CH_PRP_INPUT_VIDEO, EBAnum, (UINT32 *)((UINT32)pBuf+m_OversizedInputOffsetHeight));
                if(EBAnum)
                {
                    IDMACChannelBUF1SetReady(IDMAC_CH_PRP_OUTPUT_ENC);
                    IDMACChannelBUF1SetReady(IDMAC_CH_PRP_INPUT_VIDEO);
            ...
            if(m_ENCOversizedFrameFlag&PRP_WIDTH_OVERSIZE)
            {
                //-----------------------------------------------
                //        processing half frame in the up-right                     
                //-----------------------------------------------
                PrpIntrEnable(FRAME_INTERRUPT);
                ...
                else if((m_ENCOversizedFrameFlag&PRP_HEIGHT_OVERSIZE)&&m_ENCchannelParams.UOffset)
                {
                    OffsetData.uOffset= m_ENCchannelParams.UOffset;
                    OffsetData.vOffset = m_ENCchannelParams.VOffset;
                    CPMEMWriteOffset(IDMAC_CH_PRP_INPUT_VIDEO,
                ...
                {
                    //-----------------------------------------------
                    //        processing half frame in the down-right                       
                    //-----------------------------------------------
                    //Start interrupt
                    PrpIntrEnable(FRAME_INTERRUPT);
                    //If v offset is not zero and u,v offset is different  it means YV12 format is used, 
                    ...
                    {
                        IDMACChannelDisable(IDMAC_CH_PRP_INPUT_VIDEO);
                        OffsetData.uOffset= m_ENCchannelParams.UOffset - 3*m_OversizedInputOffsetHeight/4 - m_OversizedInputOffsetStride/2;
                        OffsetData.vOffset = m_ENCchannelParams.VOffset - 3*m_OversizedInputOffsetHeight/4 - m_OversizedInputOffsetStride/2;
                        CPMEMWriteOffset(IDMAC_CH_PRP_INPUT_VIDEO,
                                         &OffsetData, OffsetData.bInterleaved);
                        IDMACChannelEnable(IDMAC_CH_PRP_INPUT_VIDEO);
                    }
                    ...
                    //Start to fill half buffer in the right side
                    CPMEMWriteBuffer(IDMAC_CH_PRP_OUTPUT_ENC, EBAnum, (UINT32 *)(dwOutputBuf+m_OversizedOutputOffsetHeight+m_OversizedOutputOffsetStride));
                    CPMEMWriteBuffer(IDMAC_CH_PRP_INPUT_VIDEO, EBAnum, (UINT32 *)((UINT32)pBuf+m_OversizedInputOffsetHeight+m_OversizedOutputOffsetStride));
                    if(EBAnum)
                    {
                        IDMACChannelBUF1SetReady(IDMAC_CH_PRP_OUTPUT_ENC);
                        IDMACChannelBUF1SetReady(IDMAC_CH_PRP_INPUT_VIDEO);
    ...
    else if(m_VFOversizedFrameFlag)
    {
        //oversized frame processing is always blocked.
        if((m_PrpFirstModule == prpSourceType_PRP)
            &&(m_PrpVFLastModule == prpSourceType_PRP))
        {
            ...
            if ((m_VFchannelParams.VOffset&&(m_VFchannelParams.VOffset!=m_VFchannelParams.UOffset))
             ||((m_VFOversizedFrameFlag&PRP_HEIGHT_OVERSIZE)&&m_VFchannelParams.UOffset))
            {
            ...
            if(m_VFOversizedFrameFlag&PRP_HEIGHT_OVERSIZE)
            {
                //-----------------------------------------------
                //        processing half frame in the down-left                       
                //-----------------------------------------------
                ...
                {
                    IDMACChannelDisable(IDMAC_CH_PRP_INPUT_VIDEO);
                    OffsetData.uOffset= m_VFchannelParams.UOffset - 3*m_OversizedInputOffsetHeight/4;
                    OffsetData.vOffset = m_VFchannelParams.VOffset - 3*m_OversizedInputOffsetHeight/4;
                    CPMEMWriteOffset(IDMAC_CH_PRP_INPUT_VIDEO,
                                     &OffsetData, OffsetData.bInterleaved);
                    IDMACChannelEnable(IDMAC_CH_PRP_INPUT_VIDEO);
                }
                else if(m_VFchannelParams.UOffset)
                {
                    //NV12 frame
                    IDMACChannelDisable(IDMAC_CH_PRP_INPUT_VIDEO);
                    OffsetData.uOffset= m_VFchannelParams.UOffset - m_OversizedInputOffsetHeight/2;
                    OffsetData.vOffset = m_VFchannelParams.VOffset - m_OversizedInputOffsetHeight/2;
                    CPMEMWriteOffset(IDMAC_CH_PRP_INPUT_VIDEO,
                                     &OffsetData, OffsetData.bInterleaved);
                    IDMACChannelEnable(IDMAC_CH_PRP_INPUT_VIDEO);               
                }                      
                //Start to fill half buffer in the right side
                CPMEMWriteBuffer(IDMAC_CH_PRP_OUTPUT_VF, EBAnum, (UINT32 *)(dwOutputBuf+m_OversizedOutputOffsetHeight));
                CPMEMWriteBuffer(IDMAC_CH_PRP_INPUT_VIDEO, EBAnum, (UINT32 *)((UINT32)pBuf+m_OversizedInputOffsetHeight));
                if(EBAnum)
                {
                    IDMACChannelBUF1SetReady(IDMAC_CH_PRP_OUTPUT_VF);
            ...
            if(m_VFOversizedFrameFlag&PRP_WIDTH_OVERSIZE)
            {
                //-----------------------------------------------
                //        processing half frame in the up-right                      
                //-----------------------------------------------
                ...
                else if((m_VFOversizedFrameFlag&PRP_HEIGHT_OVERSIZE)&&m_VFchannelParams.UOffset)
                {
                    //NV12
                    OffsetData.uOffset= m_VFchannelParams.UOffset;
                    OffsetData.vOffset = m_VFchannelParams.VOffset;
                ...
                if(m_VFOversizedFrameFlag&PRP_HEIGHT_OVERSIZE)
                {
                    ...
                    {
                        IDMACChannelDisable(IDMAC_CH_PRP_INPUT_VIDEO);
                        OffsetData.uOffset= m_VFchannelParams.UOffset - 3*m_OversizedInputOffsetHeight/4- m_OversizedInputOffsetStride/2;
                        OffsetData.vOffset = m_VFchannelParams.VOffset - 3*m_OversizedInputOffsetHeight/4- m_OversizedInputOffsetStride/2;
                        CPMEMWriteOffset(IDMAC_CH_PRP_INPUT_VIDEO,
                                         &OffsetData, OffsetData.bInterleaved);
                        IDMACChannelEnable(IDMAC_CH_PRP_INPUT_VIDEO);
                    }
                    ...
                    //Start to fill half buffer in the right side
                    CPMEMWriteBuffer(IDMAC_CH_PRP_OUTPUT_VF, EBAnum, (UINT32 *)(dwOutputBuf+m_OversizedOutputOffsetHeight+m_OversizedOutputOffsetStride));
                    CPMEMWriteBuffer(IDMAC_CH_PRP_INPUT_VIDEO, EBAnum, (UINT32 *)((UINT32)pBuf+m_OversizedInputOffsetHeight+m_OversizedInputOffsetStride));
                    if(EBAnum)
                    {
                        IDMACChannelBUF1SetReady(IDMAC_CH_PRP_OUTPUT_VF);
                        IDMACChannelBUF1SetReady(IDMAC_CH_PRP_INPUT_VIDEO);
...
//-----------------------------------------------------------------------------
//
// Function:  PrpAddVFOutputBuffer
//
// This function fill output buffer into hardware.
//
// Parameters:
//      pBuf
...
//
//-----------------------------------------------------------------------------
BOOL PrpClass::PrpAddVFOutputBuffer(UINT32 *pBuf)
{
    ...
    if((IDMACChannelBUF0IsReady(IDMAC_CH_PRP_OUTPUT_VF)
        &&IDMACChannelCurrentBufIsBuf1(IDMAC_CH_PRP_OUTPUT_VF))
       ||(IDMACChannelBUF1IsReady(IDMAC_CH_PRP_OUTPUT_VF)
       &&( !IDMACChannelCurrentBufIsBuf1(IDMAC_CH_PRP_OUTPUT_VF))))
    {
        DEBUGMSG(ZONE_ERROR,
        (TEXT("%s: Hardware is busy..\r\n"), __WFUNCTION__));
    ...
    if (IDMACChannelCurrentBufIsBuf1(IDMAC_CH_PRP_OUTPUT_VF)) 
    {
        // Program input buffer into task parameter memory
        CPMEMWriteBuffer(IDMAC_CH_PRP_OUTPUT_VF,0, pBuf);
        IDMACChannelBUF0SetReady(IDMAC_CH_PRP_OUTPUT_VF);
    }
    else
    {
        // Program input buffer into task parameter memory
        CPMEMWriteBuffer(IDMAC_CH_PRP_OUTPUT_VF,1, pBuf);
        IDMACChannelBUF1SetReady(IDMAC_CH_PRP_OUTPUT_VF);
...
BOOL PrpClass::PrpConfigure(pPrpConfigData pConfigData)
{
    ...
    m_ENCOversizedFrameFlag = 0;
    m_VFOversizedFrameFlag = 0;
        ...
        ||(pConfigData->bVFEnable && pConfigData->bCombineEnable))
    {
        ...
        //if the prp output data is not going to dp, the csc should be handled in prp
        if(pConfigData->VFCSCEquation != CSCNoOp && pConfigData->bNoDirectDP) 
        ...
        if(pConfigData->bCombineEnable)
        {
            ...
            else
            {
                ...
                {
                    PrpConfigureCSC(CSCNoOp,icCSC1,NULL,prpOutputChannel_VF);
                    PrpConfigureCSC(CSCY2R_A1,icCSC2,NULL,prpOutputChannel_VF);
                }
        ...
        //-----------------------------------------------------------------
        PrpConfigureOutput(&pConfigData->outputVFIDMAChannel,  
                            prpOutputChannel_VF, 
                            !(iVFFunctionFlag & PRP_FUNC_CSC));
        m_PrpVFLastModule = prpSourceType_PRP;
        //----------------------------------------------------------------------
        ...
        if(!pConfigData->bNoDirectDP)
        {
            ...
            if(pConfigData->bENCEnable)
            {
                ...
                {
                    dmfcData.StartSeg = IPU_DMFC_CHAN_DMFC_ST_ADDR_SEGMENT_4;
                    IDMACWaterMark(IDMAC_CH_DP_PRIMARY_FLOW_AUX_PLANE, TRUE);
                    DMFCConfigure(IDMAC_CH_DP_PRIMARY_FLOW_AUX_PLANE, &dmfcData);
                    DpChannel = DP_CHANNEL_SYNC;
                    m_bSYNCDPEnabled = TRUE;
                    m_bSYNCisDI1= TRUE;
                ...
                {
                    dmfcData.StartSeg = IPU_DMFC_CHAN_DMFC_ST_ADDR_SEGMENT_6;
                    IDMACWaterMark(IDMAC_CH_DP_SECONDARY_FLOW_AUX_PLANE, TRUE);
                    DMFCConfigure(IDMAC_CH_DP_SECONDARY_FLOW_AUX_PLANE, &dmfcData);
                    DpChannel = DP_CHANNEL_ASYNC0;
                    m_bASYNCDPEnabled = TRUE;
                }
        ...
        else
        {
            ...
            if(m_VFchannelParams.iLineStride!= pConfigData->outputVFIDMAChannel.LineStride)
            {
                m_VFchannelParams.iLineStride = pConfigData->outputVFIDMAChannel.LineStride;
                PrpIDMACChannelConfig(IDMAC_CH_PRP_OUTPUT_VF,&m_VFchannelParams);
            }
        }        
        m_bConfigured = TRUE;
    }
    else if(pConfigData->bVFEnable)
    {
        ...
        {        
            ...
            if((m_VFchannelParams.iHeight > PRP_MAX_INPUT_HEIGHT)
                ||( m_PrpVFOutputHeight> PRP_MAX_OUTPUT_HEIGHT))
            {
                m_VFOversizedFrameFlag |= PRP_HEIGHT_OVERSIZE;
                m_VFchannelParams.iHeight = m_VFchannelParams.iHeight/2;
                m_PrpVFOutputHeight = m_PrpVFOutputHeight/2;
                m_OversizedInputOffsetHeight = m_VFchannelParams.iHeight*m_VFchannelParams.iLineStride;
                if(iVFinputbpp == 8)
                    m_OversizedOutputOffsetHeight = m_PrpVFOutputHeight* m_PrpVFOutputWidth* 2;  //convert 420 to 422
                else
                    m_OversizedOutputOffsetHeight = m_PrpVFOutputHeight* m_PrpVFOutputWidth * iVFinputbpp /8;                
            }            
            if((m_VFchannelParams.iWidth > PRP_MAX_INPUT_WIDTH)
                ||( m_PrpVFOutputWidth > PRP_MAX_OUTPUT_WIDTH))
            {
                m_VFOversizedFrameFlag |= PRP_WIDTH_OVERSIZE;
            ...
            // Setup CSC parameter
            //-----------------------------------------------------------------
            if(iVFFunctionFlag & PRP_FUNC_CSC)
            {
                PrpConfigureCSC(pConfigData->VFCSCEquation, icCSC1,
                                     NULL,prpOutputChannel_VF);
            }
            ...
            {
                m_PrpVFOutputHeight = m_PrpVFOutputHeight*2;
                m_VFchannelParams.iHeight= m_PrpVFOutputHeight;
            }
        ...
        {        
            ...
            UINT32 tempInputHeight=0,tempOutputHeight=0,tempINH=0;
            if((m_VFchannelParams.iHeight> PRP_MAX_INPUT_HEIGHT)
                ||( m_PrpVFOutputHeight> PRP_MAX_OUTPUT_HEIGHT))
            {
                m_VFOversizedFrameFlag |= PRP_HEIGHT_OVERSIZE;
                tempInputHeight= m_VFchannelParams.iHeight;
                tempOutputHeight= m_PrpVFOutputHeight;
                tempINH = ((tempInputHeight-1)/2)&0xfffffffe;
                m_VFchannelParams.iHeight = (UINT16)((tempInputHeight-tempINH+1)&0xfffffffe);
                m_PrpVFOutputHeight= (UINT16)(m_PrpVFOutputHeight-(((tempOutputHeight-1)*tempINH + (tempInputHeight-1)-1)/(tempInputHeight-1)));
                m_OversizedInputOffsetHeight = tempINH*m_VFchannelParams.iLineStride;
                if(iVFinputbpp == 8)
                    m_OversizedOutputOffsetHeight = (tempOutputHeight- m_PrpVFOutputHeight)*m_PrpVFOutputWidth*2;  //convert 420 to 422
                else
                    m_OversizedOutputOffsetHeight = (tempOutputHeight- m_PrpVFOutputHeight)*m_PrpVFOutputWidth*iVFinputbpp /8;
            }
            if((m_VFchannelParams.iWidth > PRP_MAX_INPUT_WIDTH)
                ||( m_PrpVFOutputWidth > PRP_MAX_OUTPUT_WIDTH))
            {
            ...
            if(m_VFOversizedFrameFlag&PRP_WIDTH_OVERSIZE)
            {
                PrpConfigureResize4OversizeParam((UINT16)tempINW, (UINT16)(tempOutputWidth-m_PrpVFOutputWidth), prpOutputChannel_VF, TRUE);
            }
            if(m_VFOversizedFrameFlag&PRP_HEIGHT_OVERSIZE)
            {
                PrpConfigureResize4OversizeParam((UINT16)tempINH, (UINT16)(tempOutputHeight-m_PrpVFOutputHeight), prpOutputChannel_VF, FALSE);
            }
            ...
            if(m_VFOversizedFrameFlag&PRP_HEIGHT_OVERSIZE)
            {
                m_PrpVFOutputHeight = (UINT16)tempOutputHeight;
                m_VFchannelParams.iHeight= m_PrpVFOutputHeight;
            }
    ...
    else if(pConfigData->bENCEnable)
    {
        ...
        {        
            ...
            if((m_ENCchannelParams.iHeight > PRP_MAX_INPUT_HEIGHT)
                ||( m_PrpENCOutputHeight> PRP_MAX_OUTPUT_HEIGHT))
            {
                m_ENCOversizedFrameFlag |= PRP_HEIGHT_OVERSIZE;
                m_ENCchannelParams.iHeight = m_ENCchannelParams.iHeight/2;
                m_PrpENCOutputHeight = m_PrpENCOutputHeight/2;
                m_OversizedInputOffsetHeight = m_ENCchannelParams.iHeight*m_ENCchannelParams.iLineStride;
                if(iENCinputbpp == 8)
                    m_OversizedOutputOffsetHeight = m_PrpENCOutputHeight* m_PrpENCOutputWidth* 2;  //convert 420 to 422
                else
                    m_OversizedOutputOffsetHeight = m_PrpENCOutputHeight* m_PrpENCOutputWidth * iENCinputbpp /8;                
            } 
            if(( m_ENCchannelParams.iWidth > PRP_MAX_INPUT_WIDTH)
               || ( m_PrpENCOutputWidth > PRP_MAX_OUTPUT_WIDTH))
            {
                m_ENCOversizedFrameFlag |= PRP_WIDTH_OVERSIZE;
            ...
            {
                m_PrpENCOutputHeight = m_PrpENCOutputHeight*2;
                m_ENCchannelParams.iHeight= m_PrpENCOutputHeight;
            }
            m_PrpENCLastModule = prpSourceType_PRP;
        }
        ...
        {        
            ...
            {
                m_ENCOversizedFrameFlag |= PRP_HEIGHT_OVERSIZE;
                tempInputHeight= m_ENCchannelParams.iHeight;
                tempOutputHeight= m_PrpENCOutputHeight;
                tempINH = ((tempInputHeight-1)/2)&0xfffffffe;
                m_ENCchannelParams.iHeight = (UINT16)((tempInputHeight-tempINH+1)&0xfffffffe);
                m_PrpENCOutputHeight= (UINT16)(m_PrpENCOutputHeight-(((tempOutputHeight-1)*tempINH + (tempInputHeight-1)-1)/(tempInputHeight-1)));
                m_OversizedInputOffsetHeight = tempINW*m_ENCchannelParams.iLineStride;
                if(iENCinputbpp == 8)
                    m_OversizedOutputOffsetHeight = (tempOutputHeight- m_PrpENCOutputHeight)*m_PrpENCOutputWidth*2;  //convert 420 to 422
                else
                    m_OversizedOutputOffsetHeight = (tempOutputHeight- m_PrpENCOutputHeight)*m_PrpENCOutputWidth*iENCinputbpp /8;
            }
            if((m_ENCchannelParams.iWidth > PRP_MAX_INPUT_WIDTH)
                ||( m_PrpENCOutputWidth > PRP_MAX_OUTPUT_WIDTH))
            {
                m_ENCOversizedFrameFlag = TRUE;
            ...
            if(m_ENCOversizedFrameFlag&PRP_WIDTH_OVERSIZE)
            {
                PrpConfigureResize4OversizeParam((UINT16)tempINW, (UINT16)(tempOutputWidth-m_PrpENCOutputWidth), prpOutputChannel_ENC, TRUE);
            }
            if(m_ENCOversizedFrameFlag&PRP_HEIGHT_OVERSIZE)
            {
                PrpConfigureResize4OversizeParam((UINT16)tempINH, (UINT16)(tempOutputHeight-m_PrpENCOutputHeight), prpOutputChannel_ENC, FALSE);
            }
            ...
            if(m_ENCOversizedFrameFlag&PRP_HEIGHT_OVERSIZE)
            {
                m_PrpENCOutputHeight = (UINT16)tempOutputHeight;
                m_ENCchannelParams.iHeight= m_PrpENCOutputHeight;
            }
            m_PrpENCLastModule = prpSourceType_PRP;
        }
...
{
    // PP and PRP shares IPU_IC_CONF_IC_GLB_LOC_A, 
    // so we must commit it's impossible both pp and prp reachs here.
...
//-----------------------------------------------------------------------------
//
// Function: PrpConfigureResize4OversizeParam
//
// This function computes the resizing coefficients from
// the input and output size for oversized width or height.
//
// Parameters:
//      inSize
...
//
//-----------------------------------------------------------------------------
BOOL PrpClass::PrpConfigureResize4OversizeParam(UINT16 inSize, UINT16 outSize, prpOutputChannel outputChannel, BOOL bIsWidth)
{
    UINT16 tempSize;
    UINT16 tempDownsize;
    UINT16 tempOutSize;
...
BOOL PrpClass::DPConfigureInput(BOOL bIsSyncFlow, BOOL bIsInterlaced, prpOutputChannel outputChannel)
{
    ...
    if(bIsSyncFlow)
    {
        ...
        if(m_bCurrentPlanar)
        {
            //For none interleaved format, the maximum burst size can be 64.
            pIDMAChannelParams->iPixelBurstCode = 
                (UINT8)((pIDMAChannelParams->iWidth >= (CPMEM_PIXEL_BURST_64+1))? CPMEM_PIXEL_BURST_64 : (pIDMAChannelParams->iWidth - 1));
        }
        else if(pIDMAChannelParams->iBitsPerPixelCode == CPMEM_BPP_32)
        {
            //For 32bpp the maximum burst size is 16.
            pIDMAChannelParams->iPixelBurstCode = 
                (UINT8)((pIDMAChannelParams->iWidth >= (CPMEM_PIXEL_BURST_16+1))? CPMEM_PIXEL_BURST_16 : (pIDMAChannelParams->iWidth - 1));
        }
        else if(pIDMAChannelParams->iBitsPerPixelCode == CPMEM_BPP_24)
        {
            //For 24bpp the maximum burst size is 20.
            pIDMAChannelParams->iPixelBurstCode = 
                (UINT8)((pIDMAChannelParams->iWidth >= (CPMEM_PIXEL_BURST_20+1))? CPMEM_PIXEL_BURST_20 : (pIDMAChannelParams->iWidth - 1));
        }
        else
        {
            pIDMAChannelParams->iPixelBurstCode = 
                (UINT8)((pIDMAChannelParams->iWidth >= (CPMEM_PIXEL_BURST_32+1))? CPMEM_PIXEL_BURST_32 : (pIDMAChannelParams->iWidth - 1));
        }
#else
        pIDMAChannelParams->iPixelBurstCode = 
            (UINT8)((pIDMAChannelParams->iWidth >= (CPMEM_PIXEL_BURST_16+1))? CPMEM_PIXEL_BURST_16 : (pIDMAChannelParams->iWidth - 1));
...
BOOL PrpClass::PrpConfigureOutput(pIdmaChannel pIDMAChannel, prpOutputChannel outputChannel, BOOL bNoCSC)
{
    ...
    if(bNoCSC&&
       ((pIDMAChannelParams->iFormatCode == IDMAC_NON_INTERLEAVED_FORMAT_CODE_YUV420)
    ||(pIDMAChannelParams->iFormatCode == IDMAC_PARTIAL_INTERLEAVED_FORMAT_CODE_YUV420)))
    {
        ...
        if(!((m_ENCOversizedFrameFlag&PRP_WIDTH_OVERSIZE)||(m_VFOversizedFrameFlag&PRP_WIDTH_OVERSIZE)))
        {
    ...
    if (m_bCurrentPlanar)
    {
        ...
        if(m_ENCOversizedFrameFlag||m_VFOversizedFrameFlag)
        {
...
BOOL PrpClass::PrpStartChannel()
{
    ...
    if(m_bENCPrpEnabled)
    {
        ...
        //Oversized frame processing, we use single buffer mode
        if((!m_bVFPrpEnabled)&&(!m_ENCOversizedFrameFlag))
            IDMACChannelDBMODE(IDMAC_CH_PRP_INPUT_VIDEO,TRUE);
        if(!m_ENCOversizedFrameFlag)
