...
BOOL USBOTG::Init()
{
    ...
    {
        if (!GetRegValue(USBOTG_DYNAMIC_CLIENT_LOAD, (LPBYTE) &m_dwDynamicClietLoad, sizeof(m_dwDynamicClietLoad)))  // "DynamicClientLoad"
        {
            m_dwDynamicClietLoad = 0;
        }

        DWORD dwValue = 0;
        if (GetRegValue(USBOTG_DISABLE_ROLE_SWITCH, (LPBYTE) &dwValue, sizeof(dwValue)) && dwValue != 0)   // "DisableRoleSwitch"
        {
            m_fDisalbeRoleSwitch = TRUE;
        }

        dwValue = 1;
        if (GetRegValue(USBOTG_HCD_PORT_NUMBER_ON_OTG, (LPBYTE) &dwValue, sizeof(dwValue)))   // "HCDPortNumberOnOTG"
        {
            m_bHCDPortOnOTG = (BYTE)dwValue;
        }

        dwValue = 0;
        if (GetRegValue(USBOTG_THREAD_PRIORITY, (LPBYTE)&dwValue, sizeof(dwValue)))         // "Priority256"
        {
            m_iThreadPriority = (INT)dwValue;
        }

        m_HCD_Registry = new CRegistryEdit(GetHKey(), USBOTG_HOST_DRIVER);              // "Hcd"
        ...
        m_USBFN_Registry = new CRegistryEdit(GetHKey(), USBOTG_FUNC_DRIVER);            // "UsbFn"
        ...
        if (m_HCD_Registry != NULL || m_USBFN_Registry != NULL)
        {
            ...
            if (m_HCD_Registry)
            {
                ...
                GetRegValue(REG_TA_AIDL_BDIS_VAL, (PBYTE)&m_UsbOtgTimers.a_aidl_bdis_tmr, sizeof(m_UsbOtgTimers.a_aidl_bdis_tmr));  // "TA_AIDL_BDIS"
                GetRegValue(REG_TA_WAIT_BCON_VAL, (PBYTE)&m_UsbOtgTimers.a_wait_bcon_tmr, sizeof(m_UsbOtgTimers.a_wait_bcon_tmr));  // "TA_WAIT_BCON"
                GetRegValue(REG_TA_WAIT_VRISE_VAL, (PBYTE)&m_UsbOtgTimers.a_wait_vrise_tmr, sizeof(m_UsbOtgTimers.a_wait_vrise_tmr)); // "TA_WAIT_VRISE"
                GetRegValue(REG_TB_SRP_FAIL_VAL, (PBYTE)&m_UsbOtgTimers.b_srp_fail_tmr, sizeof(m_UsbOtgTimers.b_srp_fail_tmr)); // "TB_SRP_FAIL"
            ...
            m_UsbOtgState = (m_HCD_Registry != NULL? USBOTG_a_idle: USBOTG_b_idle);     // "a" take priority of "b"
            ...
            if (m_HCD_Registry)
            {
                ...
                if (GetRegValue(USBOTG_INITIAL_ROLE_MASTER, (PBYTE)&dwInitMaster, sizeof(dwInitMaster)) && dwInitMaster != 0)  // "InitialRoleMaster"
                {
...
BOOL USBOTG::EnterState(USBOTG_STATES usbOtgState)
{
    ...
    RETAILMSG(OTG_PROTOCOL_LOG, (L"\t-+-+-+- USBOTG : State Change to \""));
    switch (usbOtgState)
    {
    case USBOTG_a_idle:
        RETAILMSG(OTG_PROTOCOL_LOG, (L"USBOTG_a_idle"));
    ...
    case USBOTG_a_wait_vrise:
        RETAILMSG(OTG_PROTOCOL_LOG, (L"USBOTG_a_wait_vrise"));
        m_UsbOtgInput.bit.a_srp_det = 0;
    ...
    case USBOTG_a_wait_bcon:
        RETAILMSG(OTG_PROTOCOL_LOG, (L"USBOTG_a_wait_bcon"));
        ...
        break;
    case USBOTG_a_host:
        RETAILMSG(OTG_PROTOCOL_LOG, (L"USBOTG_a_host"));
        m_UsbOtgInput.bit.a_bus_drop = 0;
    ...
    case USBOTG_a_suspend:
        RETAILMSG(OTG_PROTOCOL_LOG, (L"USBOTG_a_suspend"));
    ...
    case USBOTG_a_peripheral:
        RETAILMSG(OTG_PROTOCOL_LOG, (L"USBOTG_a_peripheral"));
        m_UsbOtgInput.bit.b_bus_suspend = 0;
        m_UsbOtgOutputValues.bit.drv_vbus = 1;
    ...
    case USBOTG_a_wait_vfall:
        RETAILMSG(OTG_PROTOCOL_LOG, (L"USBOTG_a_wait_vfall"));
        m_UsbOtgInput.bit.a_wait_bcon_tmout = 0;

        m_UsbOtgOutputValues.ul = 0;
        break;
    case USBOTG_a_vbus_err:
        RETAILMSG(OTG_PROTOCOL_LOG, (L"USBOTG_a_vbus_err"));
        m_UsbOtgOutputValues.ul = 0;
        break;
// Default B Device.
    case USBOTG_b_idle:
        RETAILMSG(OTG_PROTOCOL_LOG, (L"USBOTG_b_idle"));
        m_UsbOtgInternal.bit.b_hnp_enable = 0;
    ...
    case USBOTG_b_srp_init:
        RETAILMSG(OTG_PROTOCOL_LOG, (L"USBOTG_b_srp_init"));
    ...
    case USBOTG_b_peripheral:
        RETAILMSG(OTG_PROTOCOL_LOG, (L"USBOTG_b_peripheral"));
        m_UsbOtgOutputValues.bit.drv_vbus = 0;
        m_UsbOtgOutputValues.bit.loc_con = 1;
        m_UsbOtgOutputValues.bit.loc_sof = 0;
        break;
    case USBOTG_b_wait_acon:
        RETAILMSG(OTG_PROTOCOL_LOG, (L"USBOTG_b_wait_acon"));
        ...
        break;
    case USBOTG_b_host:
        RETAILMSG(OTG_PROTOCOL_LOG, (L"USBOTG_b_host"));
...
{
    RETAILMSG(OTG_PROTOCOL_LOG, (L"\t-+-+-+- USBOTG : State Change from \""));
    switch (usbOtgState)
    {
    case USBOTG_states_unknown:
    default:
        RETAILMSG(OTG_PROTOCOL_LOG, (L"USBOTG_states_unknown"));
        ASSERT(FALSE);
        break;
// A Port States
    case USBOTG_a_idle:
    ...
    case USBOTG_b_idle:
        ...
        else
        {
            ...
            else if (m_UsbOtgInput.bit.b_bus_req && m_UsbOtgInput.bit.b_sess_end && m_UsbOtgInput.bit.b_se0_srp)
            {
                ...
                {
                    RETAILMSG(OTG_PROTOCOL_LOG, (TEXT("SRP TB_SRT_FAIL has expired. Make sure the cable is connected!!!\r\n")));
                    m_WaitSRPCount = 0;
