...
void ENETClass::ENETHandleInterrupt(IN NDIS_HANDLE MiniportAdapterContext)
{
    ...
    while( (InterruptEvent = HW_ENET_MAC_EIR_RD(index)) != 0 )
    {
        // Clear the EIR
        HW_ENET_MAC_EIR_WR(index,InterruptEvent);
        DEBUGMSG(ZONE_FUNCTION, (TEXT("ENET%x: +ENETHandleInterruptInterruptEvent:%x Mark:%x ECR:%x\r\n"),index,InterruptEvent,HW_ENET_MAC_EIMR_RD(index),HW_ENET_MAC_ECR_RD(index)));
        
        // Handle receive packet event
        if(InterruptEvent & BM_ENET_MAC_EIR_RXF)
            ProcessReceiveInterrupts(pEthernet);
            
        // Handle transmit packet interrupt
        if(InterruptEvent &BM_ENET_MAC_EIR_TXF)
            ProcessTransmitInterrupts(pEthernet);
            
        // Handle MII interrupt
        if(InterruptEvent & BM_ENET_MAC_EIR_MII)
        {           
            WaitForSingleObject(hMiiMutex, INFINITE);
                    
            if(((HW_ENET_MAC_MMFR_RD(MAC0)&BM_ENET_MAC_MMFR_PA)>>BP_ENET_MAC_MMFR_PA)==MAC0)
        ...
        // Handle PTP TX timer
        if(InterruptEvent & BM_ENET_MAC_EIR_TS_AVAIL)
            GetPTPTxTimer(&TxTimer);

         // Handle PTP Current  timer update
        if(InterruptEvent & BM_ENET_MAC_EIR_TS_TIMER)
            UpdateCurrentTimer();
    }
...
void ENETClass::ENETEnableInterrupt(IN NDIS_HANDLE MiniportAdapterContext)
{
    ...
    HW_ENET_MAC_EIMR_WR(index,BM_ENET_MAC_EIR_RXF|BM_ENET_MAC_EIR_MII|BM_ENET_MAC_EIR_TXF|BM_ENET_MAC_EIR_TS_AVAIL|BM_ENET_MAC_EIR_TS_TIMER);   
 //   if(index==MAC1)
 //       HW_ENET_MAC_EIMR_WR(MAC0,BM_ENET_MAC_EIR_MII);
         ...
         IN NDIS_HANDLE MiniportAdapterHandle)
{
    ...
    DWORD sequenceId=0 , messageType=0;
