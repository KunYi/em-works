...
extern BOOL  BSPFECusesKitlMode();
extern BOOL  BSPFECMacAddress(unsigned char FecMacAddress[6]);		// CS&ZHL NOV-5-2011: use MAC address set in Eboot
...
BOOL GetMacAdd(unsigned char FecMacAddress[6])
{
	...
	//
	if(FALSE == BSPFECMacAddress(FecMacAddress))
	{
		...
		{
			FecMacAddress[0]=(unsigned char)uuid.Data1 & 0xfe;
