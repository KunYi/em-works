                           ...
                           PBYTE pBufIn,PBYTE pBufOut)
{
    ...
    case LRADC_IOCTL_SET_ANALOG_POWER_UP:
    {
        ...
        fSuccess = pLRADC->SetAnalogPowerUp(stLRADCConfig.bValue);
    }
    break;
    ...
    case LRADC_IOCTL_ENABLE_X_DRIVE:
    {
        ...
        fSuccess = pLRADC->EnableTouchDetectXDrive(LradcConfigure.bValue);
    }
    break;
    case LRADC_IOCTL_ENABLE_Y_DRIVE:
    {
        ...
        fSuccess = pLRADC->EnableTouchDetectYDrive(LradcConfigure.bValue);
    }
    break;
...
//
//-----------------------------------------------------------------------------
BOOL LRADCClass::SetAnalogPowerUp(BOOL bValue)
{
    if(bValue)
    {
        // Enable Analog Power
...
// None
//-----------------------------------------------------------------------------
BOOL LRADCClass::EnableTouchDetectXDrive(BOOL bValue)
{
    if(bValue)
    {   //Enable the X-Drive
...
// None
//-----------------------------------------------------------------------------
BOOL LRADCClass::EnableTouchDetectYDrive(BOOL bValue)
{
    if(bValue)
    {   //Enable the Y-Drive
...
{
    UNREFERENCED_PARAMETER(eBatteryMode);
...
UINT32 LRADCClass::MeasureDieTemperature()
{
    ...
    BF_WR(LRADC_CTRL4, LRADC1SELECT, 0x9);

    if((HW_LRADC_CTRL1_RD() & BM_LRADC_CTRL1_LRADC1_IRQ_EN) == 0)
        BF_WR(LRADC_CTRL1, LRADC1_IRQ_EN, 0x1);

    BF_WRn(LRADC_CHn, LRADC_CH1, VALUE,0);
    ...
    {
        // Schedule the measurement and wait for it to complete.
        ScheduleChannel(LRADC_CH1);
        ScheduleChannel(LRADC_CH6);
        while((GetInterruptFlag(LRADC_CH1) == 0)||(GetInterruptFlag(LRADC_CH6) == 0));
        ClearInterruptFlag( LRADC_CH1 );
        ClearInterruptFlag( LRADC_CH6 );

        // Read the measurement and then clear the accumulator to prepare
        // for the next reading.
        u32HighCurrentReading += GetAccumValue(LRADC_CH1);
        u32LowCurrentReading += GetAccumValue(LRADC_CH6);
        ClearAccum( LRADC_CH1 );
