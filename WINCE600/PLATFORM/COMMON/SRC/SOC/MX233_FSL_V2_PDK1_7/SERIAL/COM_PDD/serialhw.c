...
#define DISABLEUART        OUTREG32 ( &pHWHead->pUartappReg->CTRL2[CLRREG], BM_UARTAPP_CTRL2_UARTEN | \
                                      BM_UARTAPP_CTRL2_RXE | \
                                      BM_UARTAPP_CTRL2_TXE)

#define ENABLEUART         OUTREG32 ( &pHWHead->pUartappReg->CTRL2[SETREG], BM_UARTAPP_CTRL2_UARTEN | \
                                      BM_UARTAPP_CTRL2_RXE | \
                                      BM_UARTAPP_CTRL2_TXE)
                            ...
                            UINT32 size)
{
    ...
    dma->RegCTRL0.U = INREG32(&pHWHead->pUartappReg->CTRL0);
                            ...
                            UINT32 size)
{
    ...
    dma->RegCTRL1.U = INREG32(&pHWHead->pUartappReg->CTRL1);
...
VOID SL_ReadModemStatus(PVOID pContext)
{
    ...
    if ( INREG32(&pHWHead->pUartappReg->INTR) & BM_UARTAPP_INTR_CTSMIS )
    {
        DEBUGMSG(ZONE_FUNCTION,(TEXT("EV_CTS\r\n")));
        OUTREG32(&pHWHead->pUartappReg->INTR[CLRREG], BM_UARTAPP_INTR_CTSMIS);
        Events |= EV_CTS;
    }
...
UINT32 SL_ReadLineStatus(PVOID pContext)
{
    ...
    try {
        ...
        u32Status = INREG32(&pHWHead->pUartappReg->STAT) & ALL_RX_ERROR_MASK;
        // Clear all Rx error bits.
        CLRREG32(&pHWHead->pUartappReg->STAT,ALL_RX_ERROR_MASK);
    }
    ...
    if((u32Status & BM_UARTAPP_STAT_OERR))
    {
    ...
    if(u32Status & BM_UARTAPP_STAT_BERR)
    {
        DEBUGMSG(ZONE_ERROR, (TEXT("Error Break detect!!!\r\n")));
        LineEvents |= EV_BREAK;
    }
    // Check for a parity error.
    if(u32Status & BM_UARTAPP_STAT_PERR)
    {
    ...
    if(u32Status & BM_UARTAPP_STAT_FERR)
    {
...
VOID SL_ClearPendingInts(PVOID pContext)
{
    ...
    try {
        ...
        OUTREG32(&pHWHead->pUartappReg->INTR[CLRREG],ALL_INT_STATUS_MASK);
...
BOOL SL_SetStopBits(PVOID pContext, ULONG StopBits)
{
    ...
    try {
        ...
        switch (StopBits) {
        case ONESTOPBIT:
            OUTREG32(&pHWHead->pUartappReg->LINECTRL[CLRREG],BM_UARTAPP_LINECTRL_STP2);
        ...
        case ONE5STOPBITS:
            ...
            break;
        case TWOSTOPBITS:
            OUTREG32(&pHWHead->pUartappReg->LINECTRL[SETREG],BM_UARTAPP_LINECTRL_STP2);
...
BOOL SL_SetBaudRate(PVOID pContext, UINT32 u32BaudRate)
{
    ...
    try
    {
        ...
        OUTREG32(&pHWHead->pUartappReg->LINECTRL[CLRREG],BM_UARTAPP_LINECTRL_BAUD_DIVINT);

        // Set new value
        OUTREG32(&pHWHead->pUartappReg->LINECTRL[SETREG],
                 CSP_BITFVAL(UARTAPP_LINECTRL_BAUD_DIVINT,GET_UARTAPP_BAUD_DIVINT(u32BaudRate)));

        // First clear the bits
        OUTREG32(&pHWHead->pUartappReg->LINECTRL[CLRREG],BM_UARTAPP_LINECTRL_BAUD_DIVFRAC);

        // Set new value
        OUTREG32(&pHWHead->pUartappReg->LINECTRL[SETREG],
                 CSP_BITFVAL(UARTAPP_LINECTRL_BAUD_DIVFRAC,GET_UARTAPP_BAUD_DIVFRAC(u32BaudRate)));
...
BOOL SL_SetByteSize(PVOID pContext, ULONG ByteSize)
{
    ...
    try {
        ...
        switch (ByteSize) {
        case 5:
            OUTREG32(&pHWHead->pUartappReg->LINECTRL[CLRREG],CSP_BITFMASK(UARTAPP_LINECTRL_WLEN));
            break;
        case 6:
            OUTREG32(&pHWHead->pUartappReg->LINECTRL[SETREG],
                     CSP_BITFVAL(UARTAPP_LINECTRL_WLEN,UARTAPP_LINECTRL_WLEN_6) &
                     CSP_BITFMASK(UARTAPP_LINECTRL_WLEN));
            break;

        case 7:
            OUTREG32(&pHWHead->pUartappReg->LINECTRL[SETREG],
                     CSP_BITFVAL(UARTAPP_LINECTRL_WLEN,UARTAPP_LINECTRL_WLEN_7) &
                     CSP_BITFMASK(UARTAPP_LINECTRL_WLEN));
            break;
        case 8:
            OUTREG32(&pHWHead->pUartappReg->LINECTRL[SETREG],CSP_BITFMASK(UARTAPP_LINECTRL_WLEN));
            break;
        default:
            bRet = FALSE;
            break;
...
BOOL SL_SetParity(PVOID pContext, ULONG Parity)
{
    ...
    try {
        ...
        case ODDPARITY:
            OUTREG32(&pHWHead->pUartappReg->LINECTRL[SETREG],BM_UARTAPP_LINECTRL_PEN);
            break;
        case EVENPARITY:
            OUTREG32(&pHWHead->pUartappReg->LINECTRL[SETREG],(BM_UARTAPP_LINECTRL_EPS | BM_UARTAPP_LINECTRL_PEN));
            break;
        case MARKPARITY:
        case SPACEPARITY:
            bRet = FALSE;
            break;
        case NOPARITY:
            OUTREG32(&pHWHead->pUartappReg->LINECTRL[CLRREG],(BM_UARTAPP_LINECTRL_PEN |BM_UARTAPP_LINECTRL_EPS));
            break;
        default:
            bRet = FALSE;
            break;
...
VOID SL_Reset(PVOID pContext)
{
    ...
    try {
        ...
        // Reset the Block
        OUTREG32 ( &pHWHead->pUartappReg->CTRL0[SETREG], BM_UARTAPP_CTRL0_SFTRST);
        Sleep(10);
        // Release the Block from Reset and starts the clock
        OUTREG32 ( &pHWHead->pUartappReg->CTRL0[CLRREG], BM_UARTAPP_CTRL0_SFTRST | BM_UARTAPP_CTRL0_CLKGATE);
    }
    except (GetExceptionCode() == EXCEPTION_ACCESS_VIOLATION ?
            EXCEPTION_EXECUTE_HANDLER : EXCEPTION_CONTINUE_SEARCH) {
             ...
             PVOID pContext, EVENT_FUNC EventCallback,PVOID pMDDContext)
{
    ...
    pHWHead->UseIrDA =bIR;
    pHWHead->pUartappReg = (PCSP_UARTAPP_REG)pRegBase;
...
VOID SL_Deinit(PVOID pContext)
{
    ...
    OUTREG32 ( &pHWHead->pUartappReg->CTRL0[SETREG],BM_UARTAPP_CTRL0_CLKGATE);
...
VOID SL_Open(PVOID pContext)
{
    ...
    try {
        ...
        OUTREG32 ( &pHWHead->pUartappReg->LINECTRL[SETREG],BM_UARTAPP_LINECTRL_FEN );

        // Set the FIFO trigger levels
        OUTREG32(&pHWHead->pUartappReg->CTRL2,CSP_BITFVAL(UARTAPP_CTRL2_TXIFLSEL,UART_APP_FIFO_EMPTY/*UART_APP_FIFO_half*/) |
                 CSP_BITFVAL(UARTAPP_CTRL2_RXIFLSEL,UART_APP_FIFO_half));
        ...
        if(pSerHead->useDMA)
        {
            ...
            OUTREG32(&pHWHead->pUartappReg->CTRL2[SETREG],BM_UARTAPP_CTRL2_RXDMAE |BM_UARTAPP_CTRL2_TXDMAE);

            // Set the Rx DMA abort on error bit, if requested.
            // Note: Hardware defaults to OFF out of reset.
            // OUTREG32(&pHWHead->pUartappReg->CTRL2[SETREG],BM_UARTAPP_CTRL2_DMAONERR);

            // Set the RXDMA Timeout counter
            // OUTREG32(&pHWHead->pUartappReg->CTRL0[SETREG],BM_UARTAPP_CTRL0_RXTO_ENABLE);

            // First clear the bits
            // OUTREG32(&pHWHead->pUartappReg->CTRL0[CLRREG],BM_UARTAPP_CTRL0_RXTIMEOUT);

            // Set new value
            // OUTREG32(&pHWHead->pUartappReg->CTRL0[SETREG],
            //CSP_BITFVAL(UARTAPP_CTRL0_RXTIMEOUT, SERIAL_DMA_RX_TIMEOUT));
        ...
        if(!pSerHead->useDMA)
        {
            OUTREG32( &pHWHead->pUartappReg->INTR[SETREG], (BM_UARTAPP_INTR_RXIEN |
...
VOID SL_PowerOn(PVOID pContext)
{
    ...
    OUTREG32 ( &pHWHead->pUartappReg->CTRL0[CLRREG],BM_UARTAPP_CTRL0_CLKGATE);
    ...
    )
{
    ...
    u32Interrupt = INREG32(&pHWHead->pUartappReg->INTR) &( ALL_INT_STATUS_MASK|ALL_INT_ENABLE_MASK);

    if ( (u32Interrupt & BM_UARTAPP_INTR_CTSMIS) && (u32Interrupt & BM_UARTAPP_INTR_CTSMIEN))
    {
        interrupts |= INTR_MODEM;
    }

    if ( (u32Interrupt & BM_UARTAPP_INTR_OEIS) || (u32Interrupt & BM_UARTAPP_INTR_BEIS) ||
         (u32Interrupt & BM_UARTAPP_INTR_FEIS) || (u32Interrupt & BM_UARTAPP_INTR_PEIS))
    {
        OUTREG32(&pHWHead->pUartappReg->INTR[CLRREG],((BM_UARTAPP_INTR_OEIS |
                                                       BM_UARTAPP_INTR_BEIS |
                                                       BM_UARTAPP_INTR_FEIS |
                                                       BM_UARTAPP_INTR_PEIS))& ALL_INT_STATUS_MASK);
    ...
    else
    {
        ...
        try {
            if ( u32Interrupt & BM_UARTAPP_INTR_TXIS )
            {
                OUTREG32(&pHWHead->pUartappReg->INTR[CLRREG],BM_UARTAPP_INTR_TXIS & ALL_INT_STATUS_MASK);
                interrupts |= INTR_TX;
            }
            if ((u32Interrupt & BM_UARTAPP_INTR_RTIS) || (u32Interrupt & BM_UARTAPP_INTR_RXIS))
            {
                OUTREG32(&pHWHead->pUartappReg->INTR[CLRREG], ((BM_UARTAPP_INTR_RTIS|
                                                                BM_UARTAPP_INTR_RXIS))& ALL_INT_STATUS_MASK);
                interrupts |= INTR_RX;
            }
        }
        except (GetExceptionCode() == EXCEPTION_ACCESS_VIOLATION ?
    ...
    try
    {
        if(pSerHead->useDMA == FALSE )
        {
            while ( TargetRoom )
            {
                ...
                u32Status = INREG32(&pHWHead->pUartappReg->STAT);
                if (!(u32Status & BM_UARTAPP_STAT_RXFE))
                {
                    // Read the RX register
                    u32Data = INREG32(&pHWHead->pUartappReg->DATA);
                    cRXChar = (UCHAR)( u32Data & 0xFF );

                    if (INREG32(&pHWHead->pUartappReg->STAT) & INVALID_DATA)
                    {
            ...
            *pByteNumber = 0;
            ...
            while (TargetRoom > 0)
            {
                ...
                if (pSerHead->currRxDmaBufId == 0)
                {
                    ...
                    pSerHead->availRxByteCount = INREG32(&pHWHead->pUartappReg->STAT) & 0xFFFF;
...
VOID SL_TxIntrHandler(PVOID pContext,PUCHAR pSourceBuffer,ULONG *pByteNumber)
{
    ...
    if (!*pByteNumber)
    {
        ...
        else
        {
            ...
            OUTREG32 ( &pHWHead->pUartappReg->INTR[CLRREG],BM_UARTAPP_INTR_TXIEN);
        }
        return;
    }
    ...
    *pByteNumber = 0;  // In case we don't send anything below.
    ...
    if (pHWHead->dcb.fRtsControl != RTS_CONTROL_HANDSHAKE)
    {
        ...
        if (pHWHead->dcb.fOutxCtsFlow && ((INREG32(&pHWHead->pUartappReg->STAT) & BM_UARTAPP_STAT_CTS) == 0))
        {
    ...
    try
    {
        if (pSerHead->useDMA == FALSE)
        {
            ...
            while(INREG32(&pHWHead->pUartappReg->STAT) & BM_UARTAPP_STAT_TXFF);

            for (*pByteNumber = 0; NumberOfBytes/* && byteCount*/; NumberOfBytes--/*, byteCount--*/)
            {

                OUTREG32(&pHWHead->pUartappReg->DATA,*pSourceBuffer);

                // Check again if FIFO is FULL
                if(INREG32(&pHWHead->pUartappReg->STAT) & BM_UARTAPP_STAT_TXFF)
                {
                    ++pSourceBuffer;
                    (*pByteNumber)++;
                    break;
            ...
            OUTREG32 ( &pHWHead->pUartappReg->INTR[SETREG], BM_UARTAPP_INTR_TXIEN);
...
VOID SL_ModemIntrHandler(PVOID pContext)
{
    ...
    if (pHWHead->CTSFlowOff && 
        (INREG32(&pHWHead->pUartappReg->STAT) & BM_UARTAPP_STAT_CTS))
    {
...
VOID SL_ClearRTS(PVOID pContext)
{
    ...
    try {
        ...
        OUTREG32 ( &pHWHead->pUartappReg->CTRL2[CLRREG], BM_UARTAPP_CTRL2_RTS);
        ...
        {                
            ...
            OUTREG32(&pHWHead->pUartappReg->CTRL2[CLRREG], BM_UARTAPP_CTRL2_RTSEN);
...
VOID SL_SetRTS(PVOID pContext)
{
    ...
    try {
        ...
        {                
            ...
            OUTREG32(&pHWHead->pUartappReg->CTRL2[SETREG],BM_UARTAPP_CTRL2_RTSEN);
        }

        //Set RTS to logic "1":low;
        OUTREG32 ( &pHWHead->pUartappReg->CTRL2[SETREG],BM_UARTAPP_CTRL2_RTS);
...
VOID SL_ClearBreak(PVOID pContext)
{
    ...
    try {
        ...
        OUTREG32 ( &pHWHead->pUartappReg->LINECTRL[CLRREG], BM_UARTAPP_LINECTRL_BRK);
...
VOID SL_SetBreak(PVOID pContext)
{
    ...
    try {
        ...
        OUTREG32 ( &pHWHead->pUartappReg->LINECTRL[SETREG], BM_UARTAPP_LINECTRL_BRK);
...
BOOL SL_XmitComChar(PVOID pContext, UCHAR ComChar)
{
    ...
    try {
        ...
        {   // We know THR will eventually empty
            ...
            SL_ReadLineStatus(pHWHead);
            if(INREG32(&pHWHead->pUartappReg->STAT) & BM_UARTAPP_STAT_TXFE)
            {
                EnterCriticalSection(&(pHWHead->RegCritSec));
                // FIFO is empty, send this character
                OUTREG32(&pHWHead->pUartappReg->DATA,ComChar);
                LeaveCriticalSection(&(pHWHead->RegCritSec));
...
VOID SL_GetModemStatus(PVOID pContext, PULONG pModemStatus)
{
    ...
    if ( INREG32(&pHWHead->pUartappReg->STAT) & BM_UARTAPP_STAT_CTS) 
    {
...
BOOL SL_SetDCB(PVOID pHead,LPDCB lpDCB)
{
    ...
    if(((PSER_INFO)pHead)->cOpenCount)
    {
        ...
        if (bRet && (lpDCB->fOutxCtsFlow != pHWHead->dcb.fOutxCtsFlow))
        {
            ...
            {
                OUTREG32((&pHWHead->pUartappReg->CTRL2[SETREG]),BM_UARTAPP_CTRL2_CTSEN);
            }
            else
            {
                OUTREG32((&pHWHead->pUartappReg->CTRL2[CLRREG]),BM_UARTAPP_CTRL2_CTSEN);
            }
        }
