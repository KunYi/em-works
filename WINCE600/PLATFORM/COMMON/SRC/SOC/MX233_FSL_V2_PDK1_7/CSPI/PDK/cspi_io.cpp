...
DWORD SPI_Init(LPCTSTR pContext)
{
    ...
    if (!pCspi)
    {
        DEBUGMSG(ZONE_ERROR,(L"ERROR: m_pCSPI null pointer!\r\n"));
        return NULL;
    }
    rc = pCspi->CspiInitialize(dwDevIndex);

    if(rc == FALSE)
    {  
        pCspi->CspiRelease();
        delete pCspi;
        ERRORMSG(1, (_T("SPI_Init:pCSPI->CspiInitialize failed!\r\n")));
        return NULL;
    }
    else
    {
        RETAILMSG(1, (TEXT("SPI_Init: CspiInitialize Sucess: pCspi  0x%x !!\r\n"),pCspi));
    }
                   ...
                   PDWORD pdwActualOut)
{
    ...
    // hOpenContext is a pointer to cspiClass instance!
    ...
    if (pCspi != NULL)
    {
        ...
        {
//#ifdef POLLING_MODE
        //DEBUGMSG(1, (TEXT("SPI_IOControl: case CSPI_IOCTL_EXCHANGE:\r\n")));
...
//#endif
        ...
        case CSPI_IOCTL_SSPCONFIGURE:
        {
             bRet = pCspi->ConfigureSSP((SSP_INIT *) pBufIn);
        }
            break;

        case CSPI_IOCTL_SSPRESETAFTERERROR:
        {
             bRet = pCspi->SSPResetAfterError((SSP_RESETCONFIG *) pBufIn);
        }
            break;

        case CSPI_IOCTL_SSPCHECKERRORS:
        {
             bRet = pCspi->SSPCheckErrors();
        }
            break;

        case CSPI_IOCTL_SSPGETIRQSTATUS:
        {
             bRet = pCspi->SSPGetIrqStatus(*(SSP_IRQ *)pBufIn) ;
        }
            break;

        case CSPI_IOCTL_SSPCLEARIRQ:
        {
             bRet = pCspi->SSPClearIrq(*(SSP_IRQ *)pBufIn) ;
        }
            break;

        case CSPI_IOCTL_SSPCONFIGTIMING:
        {
            bRet = pCspi->SSPConfigTiming(*(SSP_SPEED *)pBufIn) ;
        }
            break;
        case CSPI_IOCTL_SSP_EN_ERROR:
            bRet = pCspi->SSPEnableErrIrq(*(BOOL *)pBufIn) ;
            break;

        case CSPI_IOCTL_SSP_DIS_ERROR:
            bRet = pCspi->SSPDisableErrIrq() ;
            break;
