...
extern void BSPSimSelect3Volts(void);
...
HANDLE g_SIMInterrupt = NULL;
DWORD g_dwSysIntr_SIMData;
DWORD g_dwSiVer;  //0x10 TO2, 0x01 TO1.0, 0x02 TO1.1 
//------------------------------------------------------------------------------
// Local Variables
...
VOID SIM_InternalUnMapRegisterAddresses(PCSP_SIM_REG pSIMReg)
{
    ...
    if (pSIMReg != NULL)
    {
        MmUnmapIoSpace(pSIMReg, sizeof(CSP_SIM_REG));
        pSIMReg = NULL;        
    }
...
void SIM_Init(PCSP_SIM_REG pSIMReg)
{
    ...
    if (!KernelIoControl(IOCTL_HAL_QUERY_SI_VERSION, NULL, 0,
        &g_dwSiVer, sizeof(g_dwSiVer), NULL))
    {
        ERRORMSG(1, (_T("Cannot obtain the silicon version!\r\n")));
        return;
    }
    DEBUGMSG(1, (TEXT("Silcon verison = %d\r\n"),g_dwSiVer));
    ...
    g_SIMInterrupt = CreateEvent(NULL,FALSE,FALSE,NULL);
    if (g_SIMInterrupt == NULL)
    {
        DEBUGMSG(1, (TEXT("IsrThreadProc: InterruptInitialize\r\n")));
        return;
    }

    //dwIrq_SIMData = IRQ_SIM_DATA;
    dwIrqs[0] = (UINT32)-1;
    dwIrqs[1] = 0;
    dwIrqs[2] = IRQ_SIM_IPB;
    dwIrqs[3] = IRQ_SIM_DATA;
    
    // Call the OAL to translate the IRQ into a SysIntr value.
    if (!KernelIoControl(IOCTL_HAL_REQUEST_SYSINTR, dwIrqs,
        sizeof(dwIrqs), &g_dwSysIntr_SIMData, sizeof(DWORD), NULL))
    {
        RETAILMSG(1, (TEXT("ERROR: Failed to obtain sysintr value for SIM interrupt.\r\n")));
        g_dwSysIntr_SIMData = (DWORD)SYSINTR_UNDEFINED;
        return;
    }

    // Initialize SIM interrupt
    if (!InterruptInitialize(g_dwSysIntr_SIMData, g_SIMInterrupt, NULL, 0))
    {
        CloseHandle(g_SIMInterrupt);
        g_SIMInterrupt = NULL;
        KernelIoControl(IOCTL_HAL_RELEASE_SYSINTR, &g_dwSysIntr_SIMData, sizeof(DWORD), NULL, 0, NULL);
    ...
    INSREG32BF(&pSIMReg->CLK_PRESCALER, SIM_CLK_PRESCALER, FI_DEFAULT);
...
void SIM_Close(PCSP_SIM_REG pSIMReg)
{
    ...
    // Release SYSINTR
    InterruptDisable(g_dwSysIntr_SIMData);
    KernelIoControl(IOCTL_HAL_RELEASE_SYSINTR, &g_dwSysIntr_SIMData, sizeof(DWORD), NULL, 0, NULL);
    g_dwSysIntr_SIMData = (DWORD)SYSINTR_UNDEFINED;
...
start:
    ...
    {
        if(WaitForSingleObject(g_SIMInterrupt,1000)== WAIT_TIMEOUT)
        {
            RETAILMSG(1,(TEXT("GPCNT time out!0x%x\r\n"),pSIMReg->XMT_STATUS));
            return(STATUS_TIMEOUT);
        }
        InterruptDone(g_dwSysIntr_SIMData);
    }
    ...
    for (;;)
    {
        ...
        if (EXTREG32BF(&pSIMReg->RCV_STATUS, SIM_RCV_STATUS_RFD))
        {
            ...
            if ( flag ==1)
            {
                ...
                if (ATR_RemBytes == 1)
                {
                    if ((g_PortSelect == PORT0)&&(g_dwSiVer<DDK_SI_REV_TO2_0))
...
NTSTATUS SIM_WriteData(PCSP_SIM_REG pSIMReg, UCHAR *Buffer, ULONG Length)
{
    ...
    for ( i = 0; i < Length; i++)
    {
        ...
        if(WaitForSingleObject(g_SIMInterrupt,50)== WAIT_TIMEOUT)
        {
...
BOOL SIM_CheckCard(PCSP_SIM_REG pSIMReg)
{
    ...
    UNREFERENCED_PARAMETER(pSIMReg);
