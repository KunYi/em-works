...
//-----------------------------------------------------------------------------
// Local Variables
static PCSP_IOMUX_TO2_REGS g_pIOMUX;
...
BOOL IomuxAlloc(void)
{
    ...
    if (g_pIOMUX == NULL)
    {
       phyAddr.QuadPart = CSP_BASE_REG_PA_IOMUXC;
        ...
        g_pIOMUX = (PCSP_IOMUX_TO2_REGS) MmMapIoSpace(phyAddr, 
           sizeof(CSP_IOMUX_TO2_REGS), FALSE);
...
BOOL IomuxDealloc(void)
{
    ...
    if (g_pIOMUX != NULL)
    {
        MmUnmapIoSpace(g_pIOMUX, sizeof(CSP_IOMUX_TO2_REGS));
        g_pIOMUX = NULL;
    }
