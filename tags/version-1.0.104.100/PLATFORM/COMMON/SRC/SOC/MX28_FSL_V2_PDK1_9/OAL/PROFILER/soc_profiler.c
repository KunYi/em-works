...
BOOL OALProfileTimerEnable(hw_timer_InputSource_t eClock, UINT32 u32Fixedcnt)
{
    BOOL rc = FALSE;
    ...
    stTimerSetup.bMatchMode         = TIMER_MATCH_MODE_DISABLE;

    //Setup the timer hardware registers
    stTimerSetup.eSelect            = eClock;
    stTimerSetup.u32FixedCount      = u32Fixedcnt;
    ...
    //
    if ((stTimerSetup.bReload) && (stTimerSetup.u32FixedCount))
        BF_WRn(TIMROT_FIXED_COUNTn, TIMER1, FIXED_COUNT, stTimerSetup.u32FixedCount - 1);
    else
        BF_WRn(TIMROT_FIXED_COUNTn, TIMER1, FIXED_COUNT, stTimerSetup.u32FixedCount);
