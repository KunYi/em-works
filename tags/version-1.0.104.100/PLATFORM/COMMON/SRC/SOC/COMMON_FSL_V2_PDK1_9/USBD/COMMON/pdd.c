...
DWORD InterruptHandle(USBFN_PDD *pPdd,DWORD dwErr, DWORD *pTimeout)
{
    ...
    *tOtgsc = INREG32(&pUSBDRegs->OTG.OTGSC);
    ...
    for (;;)
    {
        ...
        {
            RETAILMSG(FALSE, (L"Disconnect Checking, OTGSC is %x, USBSTS is %x\r\n", *tOtgsc, *temp));
                    ...
                    (pPdd->bNeedForceDetach == TRUE))
            {
                ...
                RETAILMSG(FALSE, (L"Disconnect Processing\r\n"));
                if (pPdd->devState == 1)  // We don't process when device has already been disconnected
                {
                    ...
                    if (pPdd->bNeedForceDetach == TRUE)
                    {
                        ...
                        // here change Sleep(1000) to Sleep(2000) to avoid issue that after suspend/resume, device can't
                        // recognized.
                        Sleep(2000);  // The delay is necessary, 500ms is enough, we choose 1000ms just for safety
                    ...
                    {
                        /*
                         * Eric : We need to do some extra clean up work after peripheral detach
                         *        to pull the port to normal status, such as port speed, suspend status
                         *        and so on, close RS and open it again will do the job
                         *        Another work is to reset all endpoint settings
                         */ 
                        DWORD dwEpNum = sizeof(pUSBDRegs->OTG.ENDPTCTRL) / sizeof(UINT32);  // different ICs has different number of EPCTRL registers
                        DWORD epIdx;

                        // get port status back to normal
                        if (BSPUsbfnDetachNeedRSSwitch())
                        {
                            CLRREG32(&pUSBDRegs->OTG.USBCMD, 1<<0); //RS
                            Sleep(50);
                            SETREG32(&pUSBDRegs->OTG.USBCMD, 1<<0);
                        }

                        // clear NAK related bits
                        OUTREG32(&pUSBDRegs->OTG.ENDPTNAKEN, 0x0);
                        OUTREG32(&pUSBDRegs->OTG.ENDPTNAK, INREG32(&pUSBDRegs->OTG.ENDPTNAK));

                        // flush all endpoints
                        OUTREG32(&pUSBDRegs->OTG.ENDPTFLUSH, 0xFFFFFFFF);
                        while (INREG32(&pUSBDRegs->OTG.ENDPTFLUSH));

                        // clear all endpoint settings
                        for (epIdx = 0; epIdx != dwEpNum; epIdx++)
                        {
                            OUTREG32(&pUSBDRegs->OTG.ENDPTCTRL[epIdx], 0);
                        }
                    }
        ...
        if (source.URI)
        {
            RETAILMSG(FALSE, (L"Reset Processing\r\n"));
            HandleUSBReset(pPdd);
        }
        ...
        {
            LOCK();
            RETAILMSG(FALSE, (L"Sleep Processing\r\n"));
        ...
        if (source.UI)
        {
            RETAILMSG(FALSE, (L"UI Processing\r\n"));
        ...
        if (source.NAKI)
        {
            ...
            RETAILMSG(FALSE, (L"NAKI Processing\r\n"));
            OUTREG32(&pUSBDRegs->OTG.ENDPTNAK, 0x10001);
            OUTREG32(&pUSBDRegs->OTG.ENDPTNAKEN, 0x0);
...
)
{
   ...
   // Map the USB registers
    ...
    // Config AHB Burst
    BSPUsbSetBusConfig((PUCHAR)(&pUSBDRegs->OTG));
