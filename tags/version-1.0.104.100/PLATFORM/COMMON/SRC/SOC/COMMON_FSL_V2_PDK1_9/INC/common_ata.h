...
//  Header: common_ata.h
//
//  Provides definitions for ATA module based on Freescale SoC.
//
//------------------------------------------------------------------------------
#ifndef __COMMON_ATA_H
#define __COMMON_ATA_H
...
typedef struct{
    ...
    UCHAR    InterruptClear;
    UCHAR    _FillBytes4[3];
    UCHAR    FIFOAlarm; //0x34
    UCHAR    _FillBytes5[3];
    UCHAR    ADMAErrStatus; // 0x38
    UCHAR    _FillBytes51[3];
    UINT32   SysDMABAddr; // 0x3c
    UINT32   ADMASysAddr; // 0x40
    UCHAR    _FillBytes511[4];
    UINT16   BlockCNT; // 0x48
    UCHAR    _FillBytes52[2];
    UCHAR    BurstLength; // 0x4c
    UCHAR    _FillBytes53[3];
    UINT16    SectorSize; // 0x50
    UCHAR    _FillBytes54[2];
    UCHAR    _FillBytes6[0x4c]; // 0x54
