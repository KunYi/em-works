...
//------------------------------------------------------------------------------
// Marley and Ringo SDAM scripts require 32 words for scratch RAM 
// for each channel context mandatorily.
...
typedef struct 
{
    UINT32 MC0PTR;                 //00: AP (MCU) Channel 0 Pointer
    UINT32 INTR;                   //04: Channel Interrupts
    UINT32 STOP_STAT;              //08: Channel Stop/Channel Status
    UINT32 HSTART;                 //0c: Channel Start
    UINT32 EVTOVR;                 //10: Channel Event Override
    UINT32 DSPOVR;                 //14: Channel BP Override
    UINT32 HOSTOVR;                //18: Channel AP Override
    UINT32 EVTPEND;                //1C: Channel Event Pending
    UINT32 reserved0;              //20: 
    UINT32 RESET;                  //24: Reset Register
    UINT32 EVTERR;                 //28: DMA Request Error Register
    UINT32 INTRMASK;               //2C: Channel AP Interrupt Mask
    UINT32 PSW;                    //30: Schedule Status
    UINT32 EVTERRDBG;              //34: DMA Request Error Register
    UINT32 CONFIG;                 //38: Configuration Register
    UINT32 SDMA_LOCK;              //3C: SDMA LOCK for MX37, MX51
    UINT32 ONCE_ENB;               //40: OnCE Enable
    UINT32 ONCE_DATA;              //44: OnCE Data Register
    UINT32 ONCE_INSTR;             //48: OnCE Instruction Register
    UINT32 ONCE_STAT;              //4C: OnCE Status Register
    UINT32 ONCE_CMD;               //50: OnCE Command Register
    UINT32 reserved1;              //54:
    UINT32 ILLINSTADDR;            //58: Illegal Instruction Trap Address
    UINT32 CHN0ADDR;               //5C: Channel 0 Boot Address
    UINT32 EVT_MIRROR;             //60:  DMA Requests 
    UINT32 EVT_MIRROR2;            //64:  DMA Requests 2
    UINT32 reserved2[2];           //68, 6C
    UINT32 XTRIG_CONF1;            //70: Cross-Trigger Events Configuration register1
    UINT32 XTRIG_CONF2;            //74: Cross-Trigger Events Configuration register2
    UINT32 OTB;                    //78:
    UINT32 PRF_CNTX[6];            //7C~90:
    UINT32 PRF_CFG;                //94:
    UINT32 reserved3[26];          //98~FC: reserverd
    UINT32 CHNPRI[SDMA_NUM_CHANNELS]; //100: 
    UINT32 reserved4[32];
    UINT32 CHNENBL[SDMA_NUM_REQUESTS];  //200:
    UINT32 reserved5[16];
...
#define SDMA_ONCE_ENB_OFFSET            0x0040
#define SDMA_ONCE_DATA_OFFSET           0x0044
#define SDMA_ONCE_INSTR_OFFSET          0x0048
#define SDMA_ONCE_STAT_OFFSET           0x004C
#define SDMA_ONCE_CMD_OFFSET            0x0050
#define SDMA_ILLINSTADDR_OFFSET         0x0058
#define SDMA_CHN0ADDR_OFFSET            0x005C
#define SDMA_EVT_MIRROR_OFFSET          0x0060
#define SDMA_EVT_MIRROR2_OFFSET         0x0064
#define SDMA_XTRIG_CONF1_OFFSET         0x0070
#define SDMA_XTRIG_CONF2_OFFSET         0x0074
#define SDMA_OTB_OFFSET                 0x0078
#define SDMA_PRF_CNTX_OFFSET            0x007C
#define SDMA_PRF_CFG_OFFSET             0x0094 
#define SDMA_CHNPRI_OFFSET              0x0100
#define SDMA_CHNENBL_OFFSET             0x0200
