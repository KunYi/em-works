...
CESDHCBase::CESDHCBase()
{
    ...
    m_fSDIOReset4CMD53 = FALSE;
    m_fDDRMode = FALSE;
    m_f8BitMode = FALSE;
    
    pBspESDHCDisableDDRMode = NULL;
    pBspESDHCEnableDDRMode = NULL;
}
...
VOID CESDHCBase::SoftwareResetSDIO( void)
{
    ...
    OUTREG32(&m_pESDHCReg->XFERTYP, dwXferTypReg | (m_fDDRMode << ESDHC_XFERTYP_DDREN_LSH));
...
{            
    ...
    {
        RETAILMSG(1, (L"CESDHCBase::SetInterface: Setting for 8 bit mode \r\n"));

        INSREG32BF(&m_pESDHCReg->PROCTL, ESDHC_PROCTL_DTW, ESDHC_DTW_8BIT);
        RETAILMSG(1, (L"CESDHCBase::SetInterface: PROCTL value = 0x%X\r\n", INREG32(&m_pESDHCReg->PROCTL) ));
    } 
    else 
    {
...
VOID CESDHCBase::EnableSDIOInterrupts()
{
    ...
    //workaround for losing SDIO interrupt
    CLRREG32(&m_pESDHCReg->PROCTL, CSP_BITFMASK(ESDHC_PROCTL_D3CD));
    SETREG32(&m_pESDHCReg->PROCTL, CSP_BITFMASK(ESDHC_PROCTL_D3CD));
...
VOID CESDHCBase::SetClockRate(PDWORD pdwRate)
{
    ...
    {
        for (j = (m_dwVendorVersion == ESDHC_VVN_FSL23 ? 2 : 1); j < ulMaxPrescaler; j <<= 1)
        {
...
SD_API_STATUS CESDHCBase::SendCommand( PSD_BUS_REQUEST pRequest )
{
    ...
    dwXferTypReg |= (m_fDDRMode << ESDHC_XFERTYP_DDREN_LSH);
    ...
    if (pRequest) {
            ...
            (pRequest->CommandCode == SD_CMD_WRITE_MULTIPLE_BLOCK && m_fAutoCMD12Success)) 
        {
            ...
            {   
                ...
                OUTREG32(&m_pESDHCReg->XFERTYP, 0x0D1A0000 | (m_fDDRMode << ESDHC_XFERTYP_DDREN_LSH));                    
    ...
    ULONG OptionSize )
{
        ...
        case SDHCDGetSlotInfo:
            ...
            else
            {
                ...
                DWORD dwCaps = SD_SLOT_SD_1BIT_CAPABLE |
                    ...
                    (m_f8BitMode ? SD_SLOT_MMC_8BIT_CAPABLE : 0) |
    ...
    {
        if(pBspESDHCEnableDDRMode && pBspESDHCEnableDDRMode(this))
            status = SD_API_STATUS_SUCCESS;
    }
    
    if((SD_SLOT_OPTION_CODE_EXT)Option == SDHCDDisableDDRMode)
    {
        if(pBspESDHCDisableDDRMode && pBspESDHCDisableDDRMode(this))
            status = SD_API_STATUS_SUCCESS;
    }
...
SD_API_STATUS CESDHCBase::BusRequestHandler( PSD_BUS_REQUEST pRequest ) 
{
    ...
    if(pRequest->CommandCode == 53 && m_fSDIOReset4CMD53) 
    {
        //for cmd53, we need to reset data line on i.MX50
        DWORD dwProctlReg = INREG32(&m_pESDHCReg->PROCTL);
        DWORD dwIrqStatEnReg = INREG32(&m_pESDHCReg->IRQSTATEN); 
        DWORD dwIrqSigEnReg = INREG32(&m_pESDHCReg->IRQSIGEN); 
