...
//
// This method is used to set PXP block size.
//
// Parameters:
//      hPXP
//          [in] Handle to PXP driver.
//
//      pBlocksize
//          [in/out] Pointer to PXP block size value.
//
// Returns:
//      TRUE if success.
//      FALSE if failure.
...
BOOL PXPSetBlockSize(HANDLE hPxP, UINT32 *pBlocksize)
{
    ...
    // issue the IOCTL to get PXP block size info
    if (!DeviceIoControl(hPxP,     // file handle to the driver
        PXP_IOCTL_SET_BLOCK_SIZE,   // I/O control code
        pBlocksize,                // in buffer
        sizeof(UINT32),             // in buffer size
        pBlocksize,                // out buffer
...
//
// This method is used to get PXP down scaling factor exponent.
//
// Parameters:
//      hPXP
//          [in] Handle to PXP driver.
...
BOOL PXPGetDownScaleFactorExponent(HANDLE hPxP, UINT32 *pDownScaleExp)
{
    ...
    // issue the IOCTL to get PXP down scaling factor exponent
    if (!DeviceIoControl(hPxP,     // file handle to the driver
        PXP_IOCTL_GET_DOWNSCALE_EXPONENT,   // I/O control code
        NULL,                      // in buffer
        0,                         // in buffer size
        pDownScaleExp,            // out buffer
        sizeof(UINT32),         // out buffer size
        NULL,                  // number of bytes returned
        NULL))                     // ignored (=NULL)
    {
