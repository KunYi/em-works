...
void PrpClass::PrpDeinit(void)
{
    ...
    if(m_hExitPrpEncRotThread != NULL)
    {
...
void PrpClass::PrpISRLoop(UINT32 timeout)
{
    ...
    hLoopEvent[0] = m_hExitPrpISRThread;
    hLoopEvent[1] = m_hPrpIntrEvent;
...
void PrpClass::PrpEncBufferWorkerRoutine(UINT32 timeout)
{
    ...
    HANDLE hLoopEvent[prpMaxBufRequestEvents + 1];
    DWORD result = 0;

    hLoopEvent[0] = m_hExitPrpEncThread;

    for (UINT32 i = 0; i<prpMaxBufRequestEvents; i++)
    {
        hLoopEvent[i + 1] = m_hEncBufWaitList[i];
    }

    PRP_FUNCTION_ENTRY();

    // loop here
    while((result = WaitForMultipleObjects (prpMaxBufRequestEvents + 1, hLoopEvent, FALSE, timeout )) != WAIT_TIMEOUT)
    {
        if ( result == WAIT_OBJECT_0 + 0 )
        {
        ...
        else
        {
            ...
            switch(result)
            {
                ...
                case (WAIT_OBJECT_0 + 1 + prpBuf0RequestEvent):
                ...
                case (WAIT_OBJECT_0 + 1 + prpBuf1RequestEvent):
                    // Program buffer into task parameter memory
...
void PrpClass::PrpEncRotBufferWorkerRoutine(UINT32 timeout)
{
    ...
    hLoopEvent[0] = m_hExitPrpEncRotThread;

    for (UINT32 i = 0; i<prpMaxBufRequestEvents; i++)
    {
        hLoopEvent[i + 1] = m_hEncRotBufWaitList[i];
    }
...
void PrpClass::PrpVfBufferWorkerRoutine(UINT32 timeout)
{
    ...
    hLoopEvent[0] = m_hExitPrpVfThread;

    for (UINT32 i = 0; i<prpMaxBufRequestEvents; i++)
    {
        hLoopEvent[i + 1] = m_hVfBufWaitList[i];
    }
...
void PrpClass::PrpVfRotBufferWorkerRoutine(UINT32 timeout)
{
    ...
    hLoopEvent[0] = m_hExitPrpVfRotThread;

    for (UINT32 i = 0; i<prpMaxBufRequestEvents; i++)
    {
        hLoopEvent[i + 1] = m_hVfRotBufWaitList[i];
    }
