...
--*/
#pragma warning(push)
#pragma warning(disable: 4100 4127 4189 4201 4214 4512 4245 6001 6385 28146 28125)
...
#pragma warning(pop)
#pragma warning(disable: 4245 4189 4701 28159 28132 28146)
...
retry:            
            ...
            if (SD_API_SUCCESS_RESPONSE_TIMEOUT_OK(status)) {
                if (SD_API_SUCCESS(status)) {
                    if(response.ResponseBuffer[4] & 0x80)
                    {
                        DEBUGMSG(SDBUS_ZONE_DEVICE,(_T("powered up\r\n")));
                    }
                    else
                    {
                        DEBUGMSG(SDBUS_ZONE_DEVICE,(_T("not powered up\r\n")));
                        goto retry;
                    }
...
BOOL CSDDevice::MMCSwitchtoDDRMode()
{
    SD_API_STATUS  status = SD_API_STATUS_DEVICE_UNSUPPORTED;
    SD_COMMAND_RESPONSE response;  // response
    BYTE TestBuffer[512];
    ...
    //Spec version must be 4 or beyond to support ddr mode test mechanism
    if (uSpecVer < MMC_SPEC_VERSION_4_PLUS) {
        DEBUGMSG(SDBUS_ZONE_REQUEST, (TEXT("CSDDevice::MMCSwitchtoDDRMode: SpecVersion = %d, can not perform bus detection"), uSpecVer));
        return FALSE;
    }
    
    status = SDSynchronousBusRequest_I(SD_CMD_MMC_SEND_EXT_CSD, 0 , SD_READ,  ResponseR1,&response,
                1,sizeof(TestBuffer),(PBYTE)TestBuffer, 0);  
    if (SD_API_SUCCESS(status) ) 
    {
        RETAILMSG(1, (_T("MMC_EXT_CSD_BOOTCONF=0x%x\r\n"),TestBuffer[MMC_EXT_CSD_BOOTCONF]));
        
        //if current access mode points to boot partition, change it to user area
        if((TestBuffer[MMC_EXT_CSD_BOOTCONF] & 7) != 0x0)
        {
            RETAILMSG(1, (_T("change access mode to user partition\r\n")));
            status = MMCSwitch(3, // Write Byte
                    MMC_EXT_CSD_BOOTCONF, // Offset to  boot config
                    TestBuffer[MMC_EXT_CSD_BOOTCONF] & (~7),
                    0);
        ...
        //if the card does not support DDR mode, return
        if(TestBuffer[MMC_EXT_CSD_CARDTYPE] != 0x7)
        {
            return FALSE;
	    }
	    
	    //if the card is not hstiming mode, return
        if(TestBuffer[MMC_EXT_CSD_HSTIMING] != 0x1)
        {
            return FALSE;
	    }
	}
    ...
    //card only supports ddr mode in 4 or 8 bit mode
    if(m_CardInterfaceEx.InterfaceModeEx.bit.mmc8Bit != 1 && m_CardInterfaceEx.InterfaceModeEx.bit.sd4Bit != 1)
    {
        return FALSE;
    }
    
    //if host does not support ddr mode, then return
    status = m_sdSlot.GetHost().SlotOptionHandler(m_sdSlot.m_dwSlotIndex, (SD_SLOT_OPTION_CODE)SDHCDEnableDDRMode, 0, 0);
    ...
    status = MMCSwitch(3, // Write Byte
                    0xb7, // Offset to  BUS_WIDTH
                    (m_CardInterfaceEx.InterfaceModeEx.bit.mmc8Bit == 1) ? 6 : 5, // 8bit BUS_WIDTH value.
                    0);
    //preivous action failed, try a simple recovery by deselecting and reselecting card
    if(!SD_API_SUCCESS(status))
    {
    ...
    RETAILMSG(TRUE, (_T("MMC card set bus width to ddr mode\r\n")));
    return TRUE;
    
exit:
    m_sdSlot.GetHost().SlotOptionHandler(m_sdSlot.m_dwSlotIndex, (SD_SLOT_OPTION_CODE)SDHCDDisableDDRMode, 0, 0);
    return FALSE;
}
