...
// External Functions
extern BOOL BSPGetDefaultCameraFromRegistry(UINT32 camer_num);
...
CSensorPdd::CSensorPdd()
{
    ...
    m_hCamera1HWStop = CreateEvent ( NULL,FALSE,FALSE,CAMERA1_HW_STOP);
    if(NULL == m_hCamera1HWStop)
    {
        ERRORMSG(TRUE,
            (TEXT("%s: CreateEvent failed for m_hCamera1HWStop\r\n"), __WFUNCTION__));
    }

    m_hCamera2HWStop = CreateEvent ( NULL,FALSE,FALSE,CAMERA2_HW_STOP);
    if(NULL == m_hCamera2HWStop)
    {
        ERRORMSG(TRUE,
            (TEXT("%s: CreateEvent failed for m_hCamera2HWStop\r\n"), __WFUNCTION__));
    }

    m_hDualCameraAsycExit = CreateEvent ( NULL,FALSE,FALSE,NULL ) ;
    if(NULL == m_hDualCameraAsycExit)
    {
        ERRORMSG(TRUE,
            (TEXT("%s: CreateEvent failed for m_hDualCameraAsycExit\r\n"), __WFUNCTION__));
    }

    m_hDualCameraAsyc = ::CreateThread(NULL, 0, (LPTHREAD_START_ROUTINE)DualCameraAsycThread, this, 0, NULL);
    if (m_hDualCameraAsyc == NULL)
    {
        ERRORMSG(TRUE,(TEXT("%s: CreateThread failed for DualCameraAsyc!\r\n"), __WFUNCTION__));
    }
    else
    {
        CeSetThreadPriority(m_hDualCameraAsyc, 251);
        DEBUGMSG(ZONE_FUNCTION, (TEXT("%s: create DualCameraAsyc thread success\r\n"), __WFUNCTION__));
    }
}
...
CSensorPdd::~CSensorPdd()
{
    ...
    if (m_hPinVfThread)
    {    
        // Set PinVfWorkerThread end
        SetEvent ( m_hPinVfEvent[1] ); 
        // Wait for PinVfWorkerThread end
    ...
    if (m_hDualCameraAsyc)
    {    
        // Set DualCameraAsyc Thread end
        SetEvent ( m_hDualCameraAsycExit ); 

        CloseHandle(m_hDualCameraAsyc);
        m_hDualCameraAsyc = NULL;        
    }
    ...
    if (m_hCamera1HWStop != NULL)
    {
        CloseHandle(m_hCamera1HWStop);
        m_hCamera1HWStop = NULL;
    }

    if (m_hCamera2HWStop != NULL)
    {
        CloseHandle(m_hCamera2HWStop);
        m_hCamera2HWStop = NULL;
    }

    if (m_hDualCameraAsycExit != NULL)
    {
        CloseHandle(m_hDualCameraAsycExit);
        m_hDualCameraAsycExit = NULL;
    }
...
void CSensorPdd::PinEncWorkerLoop(UINT32 timeout)
{
    ...
    {
        DEBUGMSG(ZONE_FUNCTION,(TEXT("CSensorPdd::PinEncWorkerLoop get event 0x%x NO.%d\r\n"),result,m_uCameraNum));
        ...
        if(result == WAIT_OBJECT_0 + 0)
        {
            ...
            while ( m_CsState[CAPTURE] == CSSTATE_RUN )
            {
                ...
                if(dwRet != WAIT_TIMEOUT)
                {
                    ...
                    {
                        DEBUGMSG(ZONE_FUNCTION,(TEXT("CSensorPdd::PinEncWorkerLoop call handlePINIO NO.%d\r\n"),m_uCameraNum));
        ...
        {
            DEBUGMSG(ZONE_FUNCTION,(TEXT("CAPTURE thread killed\r\n")));
            break;
        }
    }
...
//------------------------------------------------------------------------------
//
// Function: DualCameraAsycThread
//
// This function is the DualCameraAsycThread thread.
//
// Parameters:
//      lpParameter
//          [in] PDD  handler
...
//
//------------------------------------------------------------------------------
void CSensorPdd::DualCameraAsycThread(LPVOID lpParameter)
{
    CSensorPdd *pDD = (CSensorPdd *)lpParameter;

    pDD->DualCameraAsycLoop();
}

//------------------------------------------------------------------------------
//
// Function: DualCameraAsycLoop
//
// This function is the loop for DualCameraAsycThread.
// Because SMFC only one model, so when one camera driver stop the smfc IDMAC channal, 
// The sametiem, if another camera driver is using smfc, the smfc will not disable, then, the 
// Smfc will not be reset and clearly fully, and since then the data form smfc will wrong.
// So In tha case, if one camera driver is stop smfc IDMAC and the other is running, 
// We will stop the other also, after reset the smfc, we will enable the other channl again.
//
// Parameters:
//      None
//
...
//
//------------------------------------------------------------------------------
void CSensorPdd::DualCameraAsycLoop()
{
    DWORD result;
    HANDLE hCameraHWStop;
    
    // loop here
    while( (result = WaitForSingleObject (m_hDualCameraAsycExit, 100 )) == WAIT_TIMEOUT)
    {
        if( m_uCameraNum == 1)
        {
            hCameraHWStop = m_hCamera2HWStop;
        }
        else
        {
            hCameraHWStop = m_hCamera1HWStop;
        }

        WaitForSingleObject (hCameraHWStop, INFINITE );
        ...
        if(m_CsState[PREVIEW] == CSSTATE_RUN)
        {
            ...
            if(m_CsState[PREVIEW] == CSSTATE_RUN)
            {
                ...
                DEBUGMSG(ZONE_FUNCTION, (TEXT("DualCameraAsyc start preview pin again\r\n")));
            }

            DEBUGMSG(ZONE_FUNCTION, (TEXT("DualCameraAsyc Worked for camera%d preview pin\r\n"),m_uCameraNum));
        }
        else if(m_CsState[CAPTURE] == CSSTATE_RUN)
        {
            ...
            {
                SensorStartChannel(CAPTURE);
                DEBUGMSG(ZONE_FUNCTION, (TEXT("DualCameraAsyc start capture pin again\r\n")));
            }

            DEBUGMSG(ZONE_FUNCTION, (TEXT("DualCameraAsyc Worked for camera%d capture pin\r\n"),m_uCameraNum));
        }
    }
...
DWORD CSensorPdd::PDDInit( PVOID MDDContext, PPDDFUNCTBL pPDDFuncTbl )
{
    ...
    DEBUGMSG(ZONE_FUNCTION, (TEXT("%s: %s \r\n"),__WFUNCTION__,MDDContext));

    if( !ReadCameraNumFromRegister() )
    {
        ERRORMSG(TRUE, (TEXT("%s: Read Camera Num From Register Failed! \r\n"),__WFUNCTION__));
    }

    if( m_uCameraNum == 1 )
        m_ulCTypes = NUM_SUPPORTED_PINS; 
    else
        m_ulCTypes = NUM_SUPPORTED_PINS - 1;
    ...
    BSPGetDefaultCameraFromRegistry(m_uCameraNum);
    ...
    if(m_pSMFC == NULL)
    {
        m_pSMFC = new SMFCClass(m_uCameraNum,m_CSIInterface);            
    }
    else
    {
    ...
    // Only Camera1 can support preview pin
    if( m_uCameraNum == 1 )
    {
    ...
    m_pModeVideoFormat[CAPTURE].pCsDataRangeVideo[5] = &DCAM_StreamMode_RGB565_D1_NTSC;
    m_pModeVideoFormat[CAPTURE].pCsDataRangeVideo[6] = &DCAM_StreamMode_RGB565_D1_PAL;
    ...
    m_pModeVideoFormat[CAPTURE].pCsDataRangeVideo[12] = &DCAM_StreamMode_YV12_D1_NTSC;
    m_pModeVideoFormat[CAPTURE].pCsDataRangeVideo[13] = &DCAM_StreamMode_YV12_D1_PAL;    
    ...
    m_pModeVideoFormat[CAPTURE].pCsDataRangeVideo[19] = &DCAM_StreamMode_UYVY_D1_NTSC;
    m_pModeVideoFormat[CAPTURE].pCsDataRangeVideo[20] = &DCAM_StreamMode_UYVY_D1_PAL;
    ...
    m_pModeVideoFormat[CAPTURE].pCsDataRangeVideo[26] = &DCAM_StreamMode_NV12_D1_NTSC;
    m_pModeVideoFormat[CAPTURE].pCsDataRangeVideo[27] = &DCAM_StreamMode_NV12_D1_PAL;
    ...
    switch(ulModeType)
    {
        ...
        case STILL:
            if(!m_bAllocateBufferForSTILL)
            {
                ...
                DEBUGMSG(ZONE_FUNCTION,(TEXT("%s: SMFC AllocateBuffers for Still finished!\r\n"),__WFUNCTION__));
            ...
            if( m_bCameraPPEnable )
            {
                ...
                if(m_bSensorBufferAllocByDriver)
                {
                    ...
                    if(!m_bAllocateBufferForSMFC)
                    {
                        ...
                        {
                            ERRORMSG(TRUE,(_T("%s: SMFC Buffer Deallocation failed.\r\n"), __WFUNCTION__));
                            return FALSE;
                        }
                    }
                    else
                    ...
                    DEBUGMSG(ZONE_FUNCTION,(TEXT("%s: PREVIEW Call SMFCDeleteBuffers finished!\r\n"),__WFUNCTION__));        
                    m_bSensorBufferAllocByDriver = FALSE;
...
BOOL CSensorPdd::ReadCSIInterfaceFromRegistry()
{
    ...
    hKey = OpenDeviceKey((LPCTSTR)m_hContext);
    if( !hKey )
    {
        RETAILMSG(1,(TEXT("ReadCSIInterfaceFromRegistry : OpenDeviceKey failed!\r\n")));
        return FALSE;
    }
...
BOOL CSensorPdd::ReadCameraNumFromRegister()
{
    HKEY  hKey = 0;
    DWORD dwType  = 0;
    DWORD dwSize  = sizeof ( DWORD );
    ...
    if( !hKey )
    {
        RETAILMSG(1,(TEXT("ReadCameraNumFromRegister : OpenDeviceKey failed!\r\n")));
        return FALSE;
    }

    if( ERROR_SUCCESS == RegQueryValueEx( hKey, L"Index", 0, &dwType, (BYTE *)&dwValue, &dwSize ) )
    {
        if((REG_DWORD == dwType) && ( sizeof( DWORD ) == dwSize ))
        {
            if( dwValue == 1)
            {
                m_uCameraNum = 1;
                DEBUGMSG(ZONE_FUNCTION, (TEXT("ReadCameraNumFromRegister : Current camera drvier is N0.1 \r\n")));
            }

            if( dwValue == 2)
            {
                m_uCameraNum = 2;
                DEBUGMSG(ZONE_FUNCTION, (TEXT("ReadCameraNumFromRegister : Current camera drvier is N0.2 \r\n")));
            }
        }
    }
    else
    {
        RegCloseKey( hKey );
        RETAILMSG(1,(TEXT("ReadCameraNumFromRegister : Get camera driver key failed! \r\n")));
        return FALSE;
    }
...
BOOL CSensorPdd::ReadMemoryModelFromRegistry()
{
    ...
    if( m_uCameraNum == 1 )
    {
        if( ERROR_SUCCESS != RegOpenKeyEx( HKEY_LOCAL_MACHINE, L"Software\\Microsoft\\DirectX\\DirectShow\\Capture1", 0, 0, &hKey ))
        {
            return FALSE;
        }
    }
    else if(m_uCameraNum == 2)
    {
        if( ERROR_SUCCESS != RegOpenKeyEx( HKEY_LOCAL_MACHINE, L"Software\\Microsoft\\DirectX\\DirectShow\\Capture2", 0, 0, &hKey ))
        {
            return FALSE;
        }
    }
    ...
    {
        RETAILMSG(1,(TEXT("ReadMemoryModelFromRegistry : Cannot support too many camera, NO.%d! \r\n"),m_uCameraNum));
    }
    ...
    {
        RETAILMSG(1,(TEXT("ReadMemoryModelFromRegistry : Get MemoryModel failed! \r\n")));
    }
    ...
    {
        RETAILMSG(1,(TEXT("ReadMemoryModelFromRegistry : Get PinCount failed! \r\n")));
    }
...
BOOL CSensorPdd::SensorEncodingConfig()
{
    ...
    switch(m_CSIInterface)
    {
        ...
        case CSI_SELECT_CSI1:
            if (!m_pSMFC->SMFCConfigureEncoding(IDMAC_CH_SMFC_CH2,CSI_SELECT_CSI1,CSI_SELECT_FRAME0,&m_SMFCConfig))
            {
                ERRORMSG(TRUE, (TEXT("%s: SMFC viewfinding configure failed.\r\n"), __WFUNCTION__));
                return FALSE;
            }
...
BOOL CSensorPdd::SensorStopChannel(UINT32 iPin)
{
    switch (iPin)
    {
        ...
        case CAPTURE:
            ...
            if( m_uCameraNum == 1)
            {
                SetEvent(m_hCamera1HWStop);
            }
            else
            {
                SetEvent(m_hCamera2HWStop);
            }
