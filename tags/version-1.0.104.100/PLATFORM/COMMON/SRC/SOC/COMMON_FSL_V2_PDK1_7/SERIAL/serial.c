...
BOOL InitChannelDMA(PSER_INFO pSerHead)
{
    ...
    pSerHead->SerialDmaChanRx = DDKSdmaOpenChan(pSerHead->SerialDmaReqRx, 
                            BSPSerGetChannelPriority());
    ...
    // Open virtual DMA channels 
    pSerHead->SerialDmaChanTx = DDKSdmaOpenChan(pSerHead->SerialDmaReqTx, 
                            BSPSerGetChannelPriority());
...
static PVOID SerInit(BOOL bIR, ULONG Identifier, PVOID pMDDContext, PHWOBJ pHWObj)
{
	...
	//
	// CS&ZHL Jun 29-2011: move them to SerOpen( ) function.
	//
    ...
    if (!InitError)
    {
      pSerHead->pBaseAddress = Ser_InternalMapRegisterAddresses(
        ...
        if (pSerHead->useDMA)
        {
            ...
            // Combine TX & RX DMA channel IRQs and UART IRQ together to request one systintr, 
            // so we can handle many interrupts in one thread.
            aIrqs[0] = (UINT32) -1;
            aIrqs[1] = 0;
            aIrqs[2] = GetSdmaChannelIRQ(pSerHead->SerialDmaChanTx);
            aIrqs[3] = GetSdmaChannelIRQ(pSerHead->SerialDmaChanRx);
            aIrqs[4] = pSerHead->irq;
        ...
        RETAILMSG(1, (TEXT("SerInit - Initialization OK\r\n")));
        return (pSerHead);
    }
    else
    {
