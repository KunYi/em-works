...
#define MAXYEAR          (ORIGINYEAR + 135)    // the maximum year supported by mx28 rtc
...
//
// CS&ZHL MAY-30-2012: flag for setup ISL1208 in OEMSetRealTime(..)
//
static BOOL	g_bApplyISL1208 = FALSE;
                        ...
                        VOID *pOutBuffer, UINT32 outSize,UINT32 *pOutSize)
{
    ...
    OALMSG(TRUE, (L"+OALIoCtlHalInitRTC(...)\r\n"));
	...
	memcpy(&lpstget, lpst, sizeof(SYSTEMTIME));
#ifdef	EM9280
	if(!InitializeISL1208(&lpstget))
	{
		OALMSG(TRUE, (L"OALIoCtlHalInitRTC::Init ISL1208 failed!\r\n"));
        goto cleanUp;
	}

	//force On-chip RTC with ISL1208 RTC
	g_bApplyISL1208 = FALSE;
	rc = OEMSetRealTime(&lpstget);
	g_bApplyISL1208 = TRUE;
	//OALMSG(TRUE, (L"OALIoCtlHalInitRTC::Set g_bApplyISL1208 = %d\r\n", g_bApplyISL1208));
#else	// -> iMX28EVK
...
BOOL OEMGetRealTime(SYSTEMTIME *pTime)
{
    ...
    do {
           seconds = HW_RTC_SECONDS_RD();
    } while (seconds != HW_RTC_SECONDS_RD());
...
BOOL OEMSetRealTime(LPSYSTEMTIME pTime)
{
    ...
    if (!g_RTCInitialized)
    {
		OALMSG(OAL_ERROR, (L"OEMSetRealTime: RTC not initialized yet!\r\n"));
        return rc;
    }

	if (!CheckRealTime(pTime))
    {
		OALMSG(OAL_ERROR, (L"OEMSetRealTime: CheckRealTime failed\r\n"));
        return rc;
    }
	...
	if(g_bApplyISL1208)
	{
		rc = ISL1208SetRealTime(pTime);
		if(!rc)
		{
			OALMSG(OAL_ERROR, (L"OEMSetRealTime: Set ISL1208 failed\r\n"));
		}
	}
#endif	//EM9280
    ...
    // write
    HW_RTC_SECONDS_WR(Totalseconds);
