...
{
    KITLOutputDebugString("Microsoft Windows CE Ethernet Bootloader %d.%d for MX25 3DS (%s %s)\r\n",
...
void ResetDefaultBootCFG(BOOT_CFG *pBootCFG)
{
    ...
    pBootCFG->autoDownloadImage	= BOOT_CFG_AUTODOWNLOAD_NK_NAND;			//BOOT_CFG_AUTODOWNLOAD_NONE;
	...
	//
	pBootCFG->dwNandImageStart	= (DWORD)OALPAtoCA(IMAGE_BOOT_NKIMAGE_NAND_PA_START);
	pBootCFG->dwNandImageLength	= 0x2C0000;		// Size of XIPKENEL.BIN
    pBootCFG->dwPhysStart				= 0;
    pBootCFG->dwPhysLen					= 0;
    pBootCFG->mac[0]						= 0x9BD0;				//0xFFFF;
    pBootCFG->mac[1]						= 0x0005;				//0xFFFF;
    pBootCFG->mac[2]						= 0x2100;				//0xFFFF;
    pBootCFG->ConfigMagicNumber	= EBOOT_CFG_MAGIC_NUMBER;
    pBootCFG->numBootMe				= 50;
    pBootCFG->delay							= 3;
    pBootCFG->dwConfigFlags			= CONFIG_FLAGS_DEFAULT;
    pBootCFG->IP								= inet_addr("192.168.201.172");			//inet_addr("0.0.0.0"); 
    pBootCFG->subnetMask				= inet_addr("255.255.255.0");				//inet_addr("0.0.0.0"); 
    pBootCFG->DHCPEnable				= FALSE;											//TRUE;
    pBootCFG->EtherDevice				= ETH_DEVICE_FEC;							//ETH_DEVICE_LAN911x;
