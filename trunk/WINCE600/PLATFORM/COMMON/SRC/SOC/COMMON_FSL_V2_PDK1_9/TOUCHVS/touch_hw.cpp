...
//-----------------------------------------------------------------------------
// Local Variables  FALSE for 4 wire touch ,TRUE for 5 wire touch
static BOOL Use5WireTouch=FALSE;
static DWORD ConfirmRetires;
static int _AD_TORLANCE_Y_,_AD_TORLANCE_X_;
static BOOL fPenRemoved, fValidSample;
static _ST_TP_SAMPLES Sample;


//Get bsp delta X ,Y coordinate samples
...
//
//-----------------------------------------------------------------------------
static void TriggerSampleX(BOOL Use5Wire)
{
    // Enable YMINUS_ENABLE/YPLUS_ENABLE
    LRADCEnableTouchDetectYDrive(g_hLRADCTch,Use5WireTouch,TRUE);

    if(Use5Wire)
    {
        // Enable Interrupt
        LRADCEnableInterrupt(g_hLRADCTch, LRADC_CH6, TRUE);


        // Configure delayTrigger
        LRADCSetDelayTrigger( g_hLRADCTch,
                              LRADC_DELAY_TRIGGER1,                                                     // Trigger Index
                              (1 << LRADC_CH6),    // Lradc channels
...
//
//-----------------------------------------------------------------------------
static void TriggerSampleY(BOOL Use5Wire)
{
    // Enable XMINUS_ENABLE/XPLUS_ENABLE
    LRADCEnableTouchDetectXDrive(g_hLRADCTch,Use5WireTouch,TRUE);
...
BOOL TouchConfig(void)
{
    ...
    RETAILMSG(1, (TEXT("-->TouchConfig()\r\n")));

    Use5WireTouch=BSPTouchUses5WireType();
    // Set Analog powerUP
    LRADCSetAnalogPowerUp(g_hLRADCTch,Use5WireTouch,TRUE);
    ...
    BSPTouchGetDeltaCoord(&_AD_TORLANCE_X_,&_AD_TORLANCE_Y_);
...
//-----------------------------------------------------------------------------
//
// Function: TouchInterruptClear
//
// DESCRIPTION:
//      Interrupt clear for touch Interrupt.
//
// Parameters:
//
// Returns:
...
_EN_TP_INTERRUPT TouchInterruptCapture(_EN_TP_INTERRUPT irqCurrent)
{
    ...
    case _TP_INTERRUPT_PEN_DOWN_:
        ...
        LRADCEnableTouchDetectXDrive(g_hLRADCTch,Use5WireTouch,FALSE);
        LRADCEnableTouchDetectYDrive(g_hLRADCTch,Use5WireTouch,FALSE);
    ...
    case _TP_INTERRUPT_PEN_UP_OR_TIMER_SAMPLE_X_:
        ...
        if(Use5WireTouch)
        {
            Sample.Current.x = LRADCGetAccumValue(g_hLRADCTch,LRADC_CH6);
            LRADCClearAccum(g_hLRADCTch,LRADC_CH6);
        }
        else
        {
    ...
    case _TP_INTERRUPT_PEN_UP_OR_TIMER_SAMPLE_Y_:
        ...
        if(Use5WireTouch)
        {
            Sample.Current.y = LRADCGetAccumValue(g_hLRADCTch,LRADC_CH6);
            LRADCClearAccum(g_hLRADCTch,LRADC_CH6);
        }
        else
        {
