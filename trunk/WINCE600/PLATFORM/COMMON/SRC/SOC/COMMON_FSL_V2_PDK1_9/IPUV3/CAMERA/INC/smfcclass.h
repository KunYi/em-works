...
static const TCHAR IPU_SMFC_EOF_Event[IPU_SMFC_CH_MAX][40] =
{
    {L"SMFC EOF Interrupt1"},
    {L"SMFC EOF Interrupt2"},
    {L"SMFC EOF Interrupt3"},
    {L"SMFC EOF Interrupt4"}
};

static const TCHAR IPU_SMFC_Enable_Event[IPU_SMFC_CH_MAX][40] =
{
    {L"SMFC CH0 Enable"},
    {L"SMFC CH1 Enable"},
    {L"SMFC CH2 Enable"},
...
{
    public:
        SMFCClass(UINT32 SMFCInstance, CSI_SELECT csi_sel);
        ~SMFCClass();
        ...
        BOOL SMFCConfigureEncoding(UINT32 ch,CSI_SELECT csi_sel, CSI_SELECT_FRAME csi_FrameId,pSMFCConfigData configData);
        BOOL SMFCConfigureViewfinding(UINT32 ch,CSI_SELECT csi_sel, CSI_SELECT_FRAME csi_FrameId,pSMFCConfigData configData);
        UINT32* SMFCGetBufFilled();    
        UINT32 SMFCGetMaxBuffers();

        BOOL SMFCConfig(UINT32 ch,CSI_SELECT csi_sel,CSI_SELECT_FRAME csi_FrameId, pSMFCIDMACChannelParams pChannelParams);
        ...
        BOOL SMFCIsBusy(UINT32);

        void SMFCDumpIntrRegs(UINT32 ch);
        DWORD GetMemoryMode();
    ...
    private:
        BOOL SMFCInit(UINT32 SMFCInstance, CSI_SELECT csi_sel);
        void SMFCDeinit(void);

        void SMFCSetChannel(UINT32 ch);
        void SMFCIDMACChannelConfig(UINT32, pSMFCIDMACChannelParams);
        ...
        UINT32  m_uSMFCInstance;
        BOOL    m_bSMFCEnabled;
        BOOL    m_bSMFCConfigured;              
        BOOL    m_bSMFCRestartISRLoop;       
        BOOL    m_bSMFCRunning;
        HANDLE  m_hSMFCEnableEvent[IPU_SMFC_CH_MAX];
