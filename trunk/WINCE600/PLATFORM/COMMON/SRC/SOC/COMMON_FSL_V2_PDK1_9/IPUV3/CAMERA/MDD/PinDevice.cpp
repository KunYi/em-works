...
#define PININTERFACE
#pragma warning(disable: 4100 4115 4189 4201 4204 4214 4245 6244)
#include <windows.h>
#include <ceddk.h>
#include <Devload.h>
#include <ceddk.h>
...
BOOL CPinDevice::InitMsgQueueDescriptor (PCS_MSGQUEUE_BUFFER pCsMsgQBuff, PCS_STREAM_DESCRIPTOR pCsStreamDesc, PVOID pMappedData, PVOID pUnmappedData, BOOL bBufferFill)
{
    ...
    if( bBufferFill )
    {
        ...
        //This PictureNumber is the sum of the number of frames captured plus the number of frames dropped. 
        pCsFrameInfo->PictureNumber = (LONGLONG)++m_ulPictureNumber;
        pCsFrameInfo->DropCount     = (LONGLONG)m_ulFramesDropped;
        DEBUGMSG(ZONE_FUNCTION, (TEXT("%s: PictureNumber: %x, DropCount: %x\r\n"), 
                                __WFUNCTION__, m_ulPictureNumber,m_ulFramesDropped));
    }
...
{    
    ...
    if ( STILL == m_ulPinId )
    {
        ...
        // In microsoft WinCE6.0 help document,the following job was done in application.It is not a good way like this.
        // In WinCE5.0, this job was done by driver.

        m_pCamAdapter->PDDPreStillPicture(STILL);
    }
    #endif
    ...
    // Modify ------ Try to change the sequence of getting buffer
    if ( m_dwMemoryModel == CSPROPERTY_BUFFER_DRIVER )
    {
        // if buffer allocated by driver, no change
        if ( FALSE == RemoveBufferFromList( &pCsStreamDesc, &pMappedData, &pUnmappedData ) || NULL == pCsStreamDesc )
        {
            // We dropped a frame
            m_ulFramesDropped++;
            DEBUGMSG(ZONE_FUNCTION, (TEXT("%s: DropCount: %x\r\n"), __WFUNCTION__, m_ulFramesDropped));
            m_fDiscontinuity = TRUE;
            bOK = FALSE;
    ...
    {
        // if buffer allocated by client
        // first, we get the buffer(pointer) from Low level module, 
        PUCHAR pBuffer;
        DWORD dwBufferLen;
        
        // we need to check if any buffer enqueued, otherwise PDDFillPinBufferEx will loop forever
        if(!CheckBufferAvailable())
        {
            RETAILMSG(1, (TEXT("%s:No buffer enqueued!\r\n"), __WFUNCTION__));
            bOK = FALSE;
            dwRet = ERROR_INVALID_DATA;
            goto exit;
        }
        ...
        LeaveCriticalSection(&m_csStreamIO);
        dwBufferLen = m_pCamAdapter->PDDFillPinBufferEx( m_ulPinId,  &pBuffer ) ;
        if(dwBufferLen == 0)
        {
            EnterCriticalSection(&m_csStreamIO);
            RETAILMSG(1, (TEXT("%s: PDDFillPinBufferEx error\r\n"), __WFUNCTION__));
            bOK = FALSE;
            dwRet = ERROR_INVALID_DATA;
            goto exit;
        }
        ...
        // got the buffer Addr, according to it to find the buffer and remove from list
        if ( !RemoveBufferFromList( pBuffer, &pCsStreamDesc, &pMappedData, &pUnmappedData) )
        {
            // We dropped a frame
            m_ulFramesDropped++;
            DEBUGMSG(ZONE_FUNCTION, (TEXT("%s: DropCount: %x\r\n"), __WFUNCTION__, m_ulFramesDropped));
        ...
        if (!InitMsgQueueDescriptor (&CsMsgQBuff, pCsStreamDesc, pMappedData, pUnmappedData, FALSE))
        {        
            RETAILMSG(1, (TEXT("%s: InitMsgQueueDescriptor error: %x\r\n"), __WFUNCTION__));
            bOK = FALSE;
            dwRet = ERROR_INVALID_DATA;
            goto exit;
        }
...
exit:
    ...
    if ( STILL == m_ulPinId )
    {
        ...
        #if 0
        m_pCamAdapter->PDDPostStillPicture(STILL);
        #endif
        ...
        )
{
    ...
    case CSPROPERTY_CONNECTION_ALLOCATORFRAMING:
        ...
        case CSPROPERTY_TYPE_SET:
            ...
            m_dwMemoryModel = pCsAllocatorFraming->RequirementsFlags;

            m_ulMaxNumOfBuffers = pCsAllocatorFraming->Frames ;

            if( pCsAllocatorFraming->RequirementsFlags == CSPROPERTY_BUFFER_CLIENT_UNLIMITED )
            {
                if( pCsAllocatorFraming->Frames > NUM_PIN_BUFFER_CLIENT )
                    m_ulMaxNumOfBuffers = NUM_PIN_BUFFER_CLIENT;
            }
    ...
    switch( dwCommand )
    {
        case CS_ALLOCATE:
            ...
            if( m_pStreamDescriptorList == NULL )
            {
                ...
                if( NULL == m_pStreamDescriptorList )
                    return ERROR_OUTOFMEMORY;
                DEBUGMSG(ZONE_FUNCTION, (TEXT("%s:CS_ALLOCATE: m_ulMaxNumOfBuffers:%x, size of m_pStreamDescriptorList: %x\r\n"), 
                          __WFUNCTION__, m_ulMaxNumOfBuffers,sizeof( CS_STREAM_DESCRIPTOR_SHADOW ) * m_ulMaxNumOfBuffers)); 
            }
        ...
        case CS_DEALLOCATE:
            DEBUGMSG(ZONE_IOCTL,(TEXT("CPinDevice CS_DEALLOCATE \r\n")));
            dwError = DeallocateBuffer( csBufferInfo.pStreamDescriptor );
            break;
#ifdef PHY_ALLOC_TEST
        case 3:
            {
                DEBUGMSG(ZONE_IOCTL,(TEXT("CPinDevice CS_ALLOC_PHY \r\n")));
                CAMMemAlloc *pCammem = (CAMMemAlloc*)pOutBuf;

                if (pOutBuf == NULL || OutBufLen < sizeof(CAMMemAlloc))
                {
                    ERRORMSG(TRUE, (TEXT("Invalid parameter for CS_ALLOC_PHY!\r\n")));
                    break;                
                }

                HANDLE hCallerVMPId = (HANDLE) GetCallerVMProcessId();
                m_ulFrameSize = abs(CS__DIBSIZE (m_CsDataRangeVideo.VideoInfoHeader.bmiHeader));
                pCammem->Reserved = (UINT) AllocPhysMem(m_ulFrameSize, PAGE_READWRITE, 0, 0, &pCammem->PhysAdd);
                if(pCammem->Reserved == NULL)
                {
                    pCammem->PhysAdd = NULL;
                    pCammem->VirtAdd = NULL;
                    ERRORMSG(TRUE, (TEXT("AllocPhysMem failed!\r\n")));
                    break;
                }
                pCammem->VirtAdd = (UINT) VirtualAllocEx(hCallerVMPId, 0, m_ulFrameSize, MEM_RESERVE,PAGE_NOACCESS);
                if(pCammem->VirtAdd == NULL)
                {
                    FreePhysMem((LPVOID ) pCammem->Reserved);
                    pCammem->PhysAdd = NULL;
                    ERRORMSG(TRUE, (TEXT("VirtualAllocEx failed!\r\n")));
                    break;
                }

                if (!VirtualCopyEx(hCallerVMPId, (PVOID)pCammem->VirtAdd, GetCurrentProcess(), (PVOID)(pCammem->PhysAdd >> 8), 
                                   m_ulFrameSize, PAGE_PHYSICAL | PAGE_READWRITE | PAGE_NOCACHE))
                {
                    FreePhysMem((LPVOID ) pCammem->Reserved);
                    VirtualFreeEx(hCallerVMPId, (PVOID)pCammem->VirtAdd, 0, MEM_RELEASE);
                    pCammem->PhysAdd = NULL;
                    pCammem->VirtAdd = NULL;
                ...
                DEBUGMSG(ZONE_IOCTL,(TEXT("PIN Device alloc phy mem: vir = 0x%x, phy = 0x%x\r\n"),
                                    pCammem->VirtAdd,pCammem->PhysAdd));
                dwError = ERROR_SUCCESS;
                break;
            }
        case 4:
            {
                ...
                if (pOutBuf == NULL || OutBufLen < sizeof(CAMMemAlloc))
                {
                    ERRORMSG(TRUE, (TEXT("Invalid parameter for CS_DEALLOC_PHY!\r\n")));
                    break;                
                }

                if(!FreePhysMem((LPVOID )pCammem->Reserved))
                {
                    ERRORMSG(TRUE, (TEXT("FreePhysMem failed!\r\n")));
                    break;
                }
                if (!VirtualFreeEx((HANDLE)GetCallerVMProcessId(), (PVOID)(pCammem->VirtAdd), 0, MEM_RELEASE)) 
                {
                    ERRORMSG(TRUE, (TEXT("VirtualFreeEx failed!\r\n")));
                    break;
                }
                
                DEBUGMSG(ZONE_IOCTL,(TEXT("PIN Device dealloc phy mem OK: vir = 0x%x, phy = 0x%x\r\n"),
                                    pCammem->VirtAdd,pCammem->PhysAdd));
                dwError = ERROR_SUCCESS;
                break;
            }
...
DWORD 
CPinDevice::PinHandlePowerRequests(
    DWORD  dwCode,
    PUCHAR pInBuf,
    DWORD  InBufLen,
    PUCHAR pOutBuf,
    ...
    )
{
    ...
    DEBUGMSG( ZONE_IOCTL, ( _T("PIN_IOControl(%08x): HandlePowerRequests\r\n"), this ) );

    dwErr = m_pCamAdapter->AdapterHandlePowerRequests(dwCode,pInBuf,InBufLen,pOutBuf,OutBufLen,pdwBytesTransferred);
...
CPinDevice::AllocateBuffer( PCS_STREAM_DESCRIPTOR pCsDescriptor, LPVOID pOutBuf, DWORD  OutBufLen, DWORD *pdwBytesTransferred )
{
    ...
    if( m_dwMemoryModel == CSPROPERTY_BUFFER_DRIVER )
    {
        ...
        DEBUGMSG(ZONE_FUNCTION, (TEXT("%s: m_dwBufferCount: %x\r\n"), 
                                  __WFUNCTION__, m_dwBufferCount)); 
    }
...
CPinDevice::DeallocateBuffer( PCS_STREAM_DESCRIPTOR pCsDescriptor )
{
    ...
    DEBUGMSG(ZONE_FUNCTION,(TEXT("CPinDevice::DeallocateBuffer m_pStreamDescriptorList[ %d ].m_fBusy = %d\r\n"),
                                lIndex, m_pStreamDescriptorList[ lIndex ].m_fBusy));
...
)
{
    ...
    {
        DEBUGMSG(ZONE_FUNCTION,(TEXT("GetIndexFromHandle 1 dwHandle = 0x%x, pBuffer = 0x%x,lIndex = 0x%x\r\n"),
                                dwHandle,pBuffer,lIndex));
    ...
    {
        DEBUGMSG(ZONE_IOCTL,(TEXT("GetIndexFromHandle 2 dwHandle = 0x%x, pBuffer = 0x%x,lIndex = 0x%x\r\n"),
                                dwHandle,pBuffer,lIndex));
...
CPinDevice::EnqueueDescriptor( PCS_STREAM_DESCRIPTOR pUnMarshalCsDescriptor )
{
    ...
    if( lIndex == -1 )
    {
        DEBUGMSG(ZONE_FUNCTION,(TEXT("CPinDevice::EnqueueDescriptor lIndex = %x\r\n"),lIndex));
        goto Cleanup;
    }
    DEBUGMSG(ZONE_FUNCTION,(TEXT("Get lIndex = 0x%x lpBuffer = 0x%x dwHandle = 0x%x\r\n"),
                                lIndex, lpBuffer, dwHandle));
    // Is the row in use?
    ...
    if(m_pStreamDescriptorList[ lIndex ].m_pMarshalledDataBuffer != NULL)
        DEBUGMSG(ZONE_FUNCTION,(TEXT("before unmarshal m_pMarshalledDataBuffer = 0x%x m_pMarshalledDataBuffer->ptr = 0x%x\r\n"),
                                m_pStreamDescriptorList[ lIndex ].m_pMarshalledDataBuffer,
                                m_pStreamDescriptorList[ lIndex ].m_pMarshalledDataBuffer->ptr()));
        ...
        NULL != m_pStreamDescriptorList[ lIndex ].m_pMarshalledDataBuffer->ptr() )
    {
        ...
        DEBUGMSG(ZONE_FUNCTION,(TEXT("after unmarshal m_pMarshalledDataBuffer = 0x%x m_pMarshalledDataBuffer->ptr = 0x%x\r\n"),
                                m_pStreamDescriptorList[ lIndex ].m_pMarshalledDataBuffer,
                                m_pStreamDescriptorList[ lIndex ].m_pMarshalledDataBuffer->ptr()));
    }
    ...
    if( NULL == m_pStreamDescriptorList[ lIndex ].m_pMarshalledDataBuffer )
    {
        ...
        DEBUGMSG(ZONE_FUNCTION, (TEXT("%s: lIndex: %x , m_pMarshalledDataBuffer: %x\r\n"), 
                                __WFUNCTION__,lIndex, m_pStreamDescriptorList[ lIndex ].m_pMarshalledDataBuffer));
    }
    ...
    DEBUGMSG(ZONE_FUNCTION, ( _T("after marshal lpMappedBuffer = 0x%x\r\n"), lpMappedBuffer) );
    ...
    if( ( m_dwMemoryModel == CSPROPERTY_BUFFER_CLIENT_LIMITED ) ||
        ( m_dwMemoryModel == CSPROPERTY_BUFFER_CLIENT_UNLIMITED ) )
    {
        dwError = m_pCamAdapter->PDDEnqueueBuffer(m_ulPinId, lpBuffer);
        if( dwError != ERROR_SUCCESS )
        {
            DEBUGMSG(ZONE_FUNCTION, (TEXT("%s: lIndex:%x, lpBuffer:%x, lpMappedBuffer: %x enqueue error !\r\n"), 
                                    __WFUNCTION__, lIndex, lpBuffer, lpMappedBuffer));
        }
    }
    
    dwError = ERROR_SUCCESS;
    DEBUGMSG(ZONE_FUNCTION, (TEXT("%s: lIndex:%x, lpBuffer:%x, lpMappedBuffer: %x\r\n"), 
                            __WFUNCTION__, lIndex, lpBuffer, lpMappedBuffer));
Cleanup:
...
BOOL
CPinDevice::CheckBufferAvailable()
{
    DWORD dwCounter = 0;
    BOOL  RetVal = FALSE;

    // Let's look in the list of buffers for the first buffer that has a non null external stream descriptor
    DEBUGMSG( ZONE_IOCTL, ( _T("CPinDevice(%08x): CheckBufferAvailable\r\n"), this ) );

    EnterCriticalSection( &m_csStreamBuffer );
    while( dwCounter < m_dwBufferCount )
    {
        if( m_pStreamDescriptorList[ dwCounter ].pCsStreamDescriptorExternal != NULL )
        {
            // when buffer enqueue pCsStreamDescriptorExternal will be set, so if pCsStreamDescriptorExternal != NULL
            // then it means a buffer enqueued, otherwise no buffer enqueued
            RetVal = TRUE;
            break;
        }
    ...
    while(( dwCounter < m_dwBufferCount ) && ( *ppCsStreamDesc == NULL ))
    {
        if( m_pStreamDescriptorList[ dwCounter ].pCsStreamDescriptorExternal != NULL )
        {
            ...
            *ppUnmappedData = m_pStreamDescriptorList[ dwCounter ].pCsStreamDescriptorExternal->CsStreamHeader.Data;
            ...
            DEBUGMSG(ZONE_FUNCTION, (TEXT("%s: dwCounter:%x, m_dwBufferCount: %x\r\n"),
                                        __WFUNCTION__,dwCounter, m_dwBufferCount));
            break;
            }
    ...
    )
{
    ...
    *ppUnmappedData = NULL;
    ...
    {
        if( (m_pStreamDescriptorList[ dwCounter ].pCsStreamDescriptorExternal != NULL ) &&
            (pBuffer == m_pStreamDescriptorList[ dwCounter ].pCsStreamDescriptorExternal->CsStreamHeader.Data ) )
        {
            //REVIEW: All buffers accessed here should have been marshalled, check if try/except is needed
            __try
            {
...
BOOL
CPinDevice::ResetMarshal()
{
    // this function is a workaround for the m_pMarshalledDataBuffer call Marshal failed issue
    // this workaround also has potential issue, that is frequence new & delete will cause the fragmentation
    // if this potential issue occur, maybe we should modify the enqueue function, when using client allocate
    // buffer mode, we need not to do marshal operation at all.
    EnterCriticalSection( &m_csStreamBuffer );
...
ULONG CPinDevice :: PictureNumber( ) const
{
    ...
    // Now PictureNumber is equal to framescaptured,so return picturenumber + framesdropped.
    return m_ulPictureNumber + m_ulFramesDropped;
}
