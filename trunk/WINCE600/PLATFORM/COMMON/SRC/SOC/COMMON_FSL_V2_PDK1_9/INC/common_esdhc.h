...
typedef struct {
    ...
    REG32 pad2;                     // 0x5c: resevered
    REG32 DLLCTL;                   // 0x60: DDR CTL
    REG32 DLLSTATUS;                // 0x64: DDR CTL
    REG32 pad3[22];                   // 0x68 - 0xBC: reserved area
    REG32 VENDOR;                    // 0xc0: Vendor specific register
    REG32 pad4[14];                   // 0xc4 - 0xfc: reserved area
...
#define ESDHC_XFERTYP_DDREN_LSH             3
...
#define ESDHC_XFERTYP_DDREN_WID            1
...
typedef enum
{
    ...
    ESDHC_VVN_FSL23 = 0x13  //Freescale eSDHC V2.3
}ESDHCVendorVersion;    
