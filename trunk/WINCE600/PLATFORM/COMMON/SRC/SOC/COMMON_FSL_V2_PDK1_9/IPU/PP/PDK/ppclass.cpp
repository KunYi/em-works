...
BOOL PpClass::PpInit(void)
{
    ...
    // Events to exit PPBufThread
    m_hExitPpBufThread = CreateEvent ( NULL,TRUE,FALSE,NULL ) ;
    if(NULL == m_hExitPpBufThread)
    {
        ERRORMSG(TRUE,
            (TEXT("%s: CreateEvent failed for Exit PPBufThread\r\n"), __WFUNCTION__));
        goto Error;
    }

    // Events to exit PPRotBufThread
    m_hExitPpRotBufThread = CreateEvent ( NULL,TRUE,FALSE,NULL ) ;
    if(NULL == m_hExitPpRotBufThread)
    {
        ERRORMSG(TRUE,
            (TEXT("%s: CreateEvent failed for Exit PPRotBufThread\r\n"), __WFUNCTION__));
        goto Error;
    }
...
void PpClass::PpDeinit(void)
{
    ...
    {
        // Set PpBufThread end
        SetEvent ( m_hExitPpBufThread ); 
        // Wait for PpBufThread end
        if (WAIT_OBJECT_0 == WaitForSingleObject (m_hPpBufThread,INFINITE))
            RETAILMSG(0,(TEXT("Wait for PpBufThread end")));

        CloseHandle(m_hPpBufThread);
        m_hPpBufThread = NULL;
    }

    if (m_hExitPpBufThread != NULL)
    {
        CloseHandle(m_hExitPpBufThread);
        m_hExitPpBufThread = NULL;
    }

    if (m_hPpRotBufThread)
    {
        // Set PpRotBufThread end
        SetEvent ( m_hExitPpRotBufThread ); 
        // Wait for PpRotBufThread end
        if (WAIT_OBJECT_0 == WaitForSingleObject (m_hPpRotBufThread,INFINITE))
            RETAILMSG(0,(TEXT("Wait for PpRotBufThread end")));

        CloseHandle(m_hPpRotBufThread);
        m_hPpBufThread = NULL;
    }

    if (m_hExitPpRotBufThread != NULL)
    {
        CloseHandle(m_hExitPpRotBufThread);
        m_hExitPpRotBufThread = NULL;
    }
}
...
void PpClass::PpBufferWorkerRoutine(UINT32 timeout)
{
    ...
    hLoopEvent[0] = m_hExitPpBufThread;
    hLoopEvent[1] = m_hRequestBuffer;
...
void PpClass::PpRotBufferWorkerRoutine(UINT32 timeout)
{
    ...
    hLoopEvent[0] = m_hExitPpRotBufThread;
    hLoopEvent[1] = m_hRequestRotBuffer;
