...
BOOL FECInit(UINT8 *pAddress, UINT32 offset, UINT16 mac[3])
{
    ...
    {
        // Disable MII gasket
        INSREG32BF(&g_FEC.REG->MIIGSK_ENR, FEC_MIIGSK_ENR_EN, FEC_MIIGSK_ENR_EN_DISABLED);

        // Configure MIIGSK for  RMII mode    
        INSREG32BF(&g_FEC.REG->MIIGSK_CFGR, FEC_MIIGSK_CFGR_IF_MODE, FEC_MIIGSK_CFGR_IF_MODE_RMII);

        // Enable MII gasket
        INSREG32BF(&g_FEC.REG->MIIGSK_ENR, FEC_MIIGSK_ENR_EN, FEC_MIIGSK_ENR_EN_ENABLED);
        
        // Wait for MIIGSK to become ready
        while(EXTREG32BF(&g_FEC.REG->MIIGSK_ENR, FEC_MIIGSK_ENR_READY) != FEC_MIIGSK_ENR_IS_READY);
    }
	...
	// Enable FEC after basic configuration complete
    ...
    // MII_SPEED must be programmed to provide MDC frequency <= 2.5 MHz.  
    // Assume FEC is clocked with IPG_CLK of 66.5 MHz.
    //      66.5 MHz / 2.5 MHz => Divider must be >= 26.6
    //      MDC divider = MII_SPEED * 2 => MII_SPEED must be set to >= 14
    //
    // Platform code can override this divider by setting MIISpeed member
    // of global FEC data structure.
    if (g_FEC.MIISpeed)
    {
        // Set MII speed 
        OUTREG32(&g_FEC.REG->MSCR, CSP_BITFVAL(FEC_MSCR_MIISPEED, g_FEC.MIISpeed));
    }
    else
    {
        // Set MII speed to 2.5MHz        
        // OUTREG32(&g_FEC.REG->MSCR, CSP_BITFVAL(FEC_MSCR_MIISPEED, 14));
        // TODO:  MX53_BRINGUP:  Use 1/4 max MII speed.  PHY communication not reliable.
        OUTREG32(&g_FEC.REG->MSCR, CSP_BITFVAL(FEC_MSCR_MIISPEED, 14*4));
    }
    ...
    else
    {
        ...
        EdbgOutputDebugString( "PHY ID = 0x%x\r\n", MIIPhyId);
        rc = FALSE;
        goto CleanUp;
    }

    // MIIWRITE(MII_REG_CR, 0x8000);
    // OALStall(500000);
