...
//
//------------------------------------------------------------------------------
SCODE DDLcdif::SetPointerShape(GPESurf * pMask,
                                 GPESurf * pColorSurf,
                                 int xHot,
                                 int yHot,
                                 int cx,
        ...
        for (row = 0; row < cy; row++)
        {
            ...
            for (col = 0; col < cx / 8; col++)
            {
                ...
                for (bitMask = 0x0080, i = 0; i < 8; bitMask >>= 1, i++)
                {
                    ...
                    xorLine[(col * 8) + i] = ((bXor & bitMask ? 0xFF : 0x00) ^ xorLine[(col * 8) + i])?~xorLine[(col * 8) + i]:xorLine[(col * 8) + i];
                }
            }
        }
    }
...
//
//------------------------------------------------------------------------------
SCODE DDLcdif::MovePointer(int x, int y)
{
    CursorOff();
...
//
//------------------------------------------------------------------------------
VOID DDLcdif::CursorOn(VOID)
{
    UCHAR   *ptrScreen = (UCHAR*)m_pBackgroundSurface->Buffer();
    UCHAR   *ptrLine;
    UCHAR   *cbsLine;
    int     x, y;
    ...
    if (!m_CursorForcedOff && !m_CursorDisabled && !m_CursorVisible)
    {
        ...
        for (y = m_CursorRect.top; y < m_CursorRect.bottom; y++)
        {
            ...
            ptrLine = &ptrScreen[y * m_pBackgroundSurface->Stride()];
            ...
            for (x = m_CursorRect.left; x < m_CursorRect.right; x++)
            {
                ...
                switch (m_iRotate)
                {
                    case DMDO_0:
                        ...
                        break;
                    case DMDO_90:
                        iRotate = (x - m_CursorRect.left)*m_CursorSize.x + m_CursorSize.y - 1 -
                                  (y - m_CursorRect.top);
                        break;
                    case DMDO_180:
...
//
//------------------------------------------------------------------------------
VOID DDLcdif::PointerBltPrepare(GPEBltParms * pBltParms)
{
    RECTL   rectl;

    if (pBltParms->pDst == m_pBackgroundSurface)   // only care if dest is main display surface
    {
    ...
    if (pBltParms->pSrc == m_pBackgroundSurface)   // only care if source is main display surface
    {
        if (m_CursorVisible && !m_CursorDisabled)
        {
