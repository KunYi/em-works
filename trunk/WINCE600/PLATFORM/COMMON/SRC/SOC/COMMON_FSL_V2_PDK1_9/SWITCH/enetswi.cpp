...
//
//  This file implements NDIS interface for ENET SWITCH Driver.
//
//-----------------------------------------------------------------------------
#pragma warning(push)
...
PVOID pv_HWregENETSWI=NULL ;
PVOID pv_HWLMENETSWI=NULL ;
...
int GetEnentIndexFromReg(LPCTSTR RegistryPath)
{
    ...
    if (Result != ERROR_SUCCESS) {
        DEBUGMSG(ZONE_ERROR, (TEXT("ENET SWITCH Driver:  OpenDeviceKey failed!!! %s\r\n"),RegistryPath));
        return -1;
    } 
    ...
    {
       DEBUGMSG(ZONE_ERROR, (TEXT("ENET SWITCH Driver:  RegQueryValueEx failed!!!Err:%d  dwIndex :%x\r\n"),GetLastError(),dwIndex));
       return -1;
    }
    ...
    RETAILMSG(1, (TEXT("ENET SWITCH Driver:  RegQueryValueEx dwIndex :%x\r\n"),dwIndex));  
...
NTSTATUS DriverEntry( IN PDRIVER_OBJECT DriverObject, IN PUNICODE_STRING RegistryPath)
{
    ...
    DEBUGMSG (ZONE_INIT, (TEXT("ENET SWITCH Driver +:%x %s\r\n"),DriverObject,RegistryPath->Buffer));
