...
{
    BOOL rc = FALSE;
    WORD ms, sec[2], min[2], hour[2], day[2], dayofweek, month, year;
    BOOL isleap;
    UINT8 *month_tab;
    ...
    for (i =0; i<2; i++)
    {
        ...
        sec[i] = (WORD)(pRtc->RTCSecCnt & RTC_SECOND_MASK);

        //Get value of minute from RTC register
        min[i] = (WORD)(pRtc->RTCHMCnt & RTC_MINUTE_MASK);

        //Get value of hour from RTC register
        hour[i] = (WORD)(pRtc->RTCHMCnt & RTC_HOUR_MASK) >> RTC_HOUR_OFFSET;

        //Get value of day from RTC register
        day[i] = (WORD)(pRtc->RTCDayCnt & RTC_DAY_MASK);
    }

    while ( (sec[0] != sec[1] ) || (min[0] != min[1] ) ||(hour[0] != hour[1] ) || (day[0] != day[1] ) )
    {
        OALMSG(OAL_ERROR, (TEXT("MX35GetRealTime  !!!:  [0] = %u : %u : %u : %u, [1] = %u : %u : %u : %u,  "),
               day[0], hour[0], min[0] , sec[0], day[1], hour[1], min[1] , sec[1] ));
        ...
        OALMSG(OAL_ERROR, (TEXT(" [2] = %u : %u : %u : %u  \r\n"),
                day[0], hour[0], min[0] , sec[0]));

    }
        
    //Calculate current day of the week
    dayofweek = GetDayOfWeek(day[0]);
    ...
    while (day[0] > 365)
    {
        if (IsLeapYear(year))
        {
            ...
            {
                OALMSG(OAL_VERBOSE&&OAL_ERROR, (TEXT("ERROR calculate day\r\n")));
                break;
            }
        }
        else
    ...
    {
        if (day[0] <= month_tab[month])
            break;
        day[0] = day[0] - month_tab[month];
    }
    month +=1;
...
BOOL OEMSetRealTime(LPSYSTEMTIME pTime)
{
    ...
    {
        OALMSG(OAL_VERBOSE&&OAL_ERROR, (TEXT("Time to be set is INVALID!!!:  %u/%u/%u, %u:%u:%u  Day of week:%u\r\n"),
...
//  Function:  InitRTC
//
//  This function is used to initialize the real time clock.
//
//  Parameters:
//      None.
//
...
//
//  This function is used to convert the year, month and days of 
//  SYSTEMTIME to number of days which is compatible with the RTC.
//
//  Parameters:
//      lpst
//          [in] Long pointer to the buffer containing the time to be 
