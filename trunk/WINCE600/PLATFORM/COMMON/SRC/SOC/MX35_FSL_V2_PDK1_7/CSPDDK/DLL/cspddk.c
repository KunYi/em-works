...
BOOL Init(void)
{
    ...
    // Query silicon revision
    if (!KernelIoControl(IOCTL_HAL_QUERY_SI_VERSION, NULL, 0, 
        &g_SiRev, sizeof(g_SiRev), NULL))
    {
        ERRORMSG(1, (_T("CSPDDK: query silicon revision failed!")));
        goto cleanUp;
    }

    if (!SocSdmaInit())
    {
        ERRORMSG(1, (_T("SocSdmaInit failed!")));
        goto cleanUp;
    }

    if (!SdmaInit())
    {
        ERRORMSG(1, (_T("SdmaInit failed!")));
        goto cleanUp;
    }
