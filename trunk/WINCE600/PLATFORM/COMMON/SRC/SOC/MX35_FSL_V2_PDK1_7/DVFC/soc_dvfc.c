...
BOOL DVF_Init(void)
{
    ...
    g_bDvfcTerminate = FALSE; 
    g_dwDvfcSysIntr = (DWORD)SYSINTR_UNDEFINED;
...
BOOL DVF_Deinit(void)
{
    BOOL rc;
     ...
     // Disable DVFS and DPTC interrupts
    ...
    g_bDvfcTerminate = TRUE;
    if (g_hDvfcIntrServThread && g_hDvfcIntrEvent)
    {
        SetEvent(g_hDvfcIntrEvent);
        CloseHandle(g_hDvfcIntrServThread);
        g_hDvfcIntrServThread = NULL;
    }

    rc = BSPDvfcDeinit();

    // Release SYSINTR
    if (g_dwDvfcSysIntr != (DWORD)SYSINTR_UNDEFINED)
    {
        InterruptDisable(g_dwDvfcSysIntr);
        KernelIoControl(IOCTL_HAL_RELEASE_SYSINTR, &g_dwDvfcSysIntr, sizeof(DWORD), NULL, 0, NULL);
        g_dwDvfcSysIntr = (DWORD)SYSINTR_UNDEFINED;
    }

    if (g_hDvfcIntrEvent)
    {
        CloseHandle(g_hDvfcIntrEvent);
        g_hDvfcIntrEvent = NULL;
    }

    // free the virtual space allocated for CCM memory map
    if (g_pCCM != NULL)
    {
