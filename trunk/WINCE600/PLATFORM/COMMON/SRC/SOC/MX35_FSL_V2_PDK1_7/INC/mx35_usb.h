...
//  Header:  mx35_usb.h
//
//  Provides definitions for usb module based on Freescale MX35 SoC.
//
//------------------------------------------------------------------------------
#ifndef __MX35_USB_H
#define __MX35_USB_H
...
typedef struct _USB_CTRL {
    ...
    unsigned int OCPOL_OH2:1;     // HOST2 OverCurrent Enable
                              // This bit selects the polarity of OverCurrent
                              // 1 = High Active(default)
                              // 0 = Low Active
    unsigned int OCPOL_OTG:1;     // OTG OverCurrent Polarity
                              // This bit selects the polarity of OverCurrent
                              // 1 = High Active(default)
                              // 0 = Low Active
    ...
    unsigned int XCSO:1;      // Xcvr Clock Select for OTG Port 2
                              ...
                              // 1 = Force to use the internal 60MHz clock
                              // 0 = No need to use the internal 60MHz clock(default)              
    unsigned int PP:1;        // Power Polarity
                              // The power polarity bit controls the polarity of pwr output signal
                              // 1 = High active
                              // 0 = Low active
    unsigned int R1:4;        // Reserved. These bits are reserved and should be set to zero.
    ...
    unsigned int R3:2;        // Reserved. These bits are reserved and should be set to zero.
