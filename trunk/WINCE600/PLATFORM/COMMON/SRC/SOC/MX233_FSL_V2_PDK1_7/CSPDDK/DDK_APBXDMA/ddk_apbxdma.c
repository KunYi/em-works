...
BOOL DDKApbxDmaInitChan(UINT8 Channel,BOOL bEnableIrq)
{
    ...
    {
        ERRORMSG(1, (_T("DDKApbxDmaInitChan..Invalid channel!\r\n")));
        goto cleanUp;
    }
...
//-----------------------------------------------------------------------------
//
//  Function:  DDKApbxDmaEnableErrorIrq
//
//  This function Enable the Interrupt for respective channel
//
//  Parameters:
...
BOOL DDKApbxDmaEnableErrorIrq(UINT8 Channel,BOOL bEnable)
{
    ...
    if( bEnable)
        HW_APBX_CTRL2_SET(1 << (16 + Channel));          //Enable interrupt
    else
        HW_APBX_CTRL2_CLR(1 << (16 + Channel) );                 //Disable interrupt
...
BOOL ApbxDmaDeInit(void)
{
    ...
    // Deallocate APBX DMA driver data structures
    ApbxDmaDealloc();
