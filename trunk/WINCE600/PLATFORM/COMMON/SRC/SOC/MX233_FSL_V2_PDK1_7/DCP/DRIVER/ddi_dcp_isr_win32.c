...
HRESULT ddi_dcp_ServiceChannelIRQ(UINT32 Channel);

DWORD WINAPI ddi_dcp_isr_thread(LPVOID lpParameter);

extern DCPChannel_t* g_DCPChannel;

extern BOOL             g_cscCallbackEnable;
extern DCPCallback_t    g_cscCallback;
extern void *           g_cscCallbackData;
...
DWORD WINAPI ddi_dcp_isr_thread(LPVOID lpParameter)
{
    ...
    for (;;) {
        ...
        if (Channel & 0x100) {
            ...
            if (g_cscCallbackEnable == TRUE)
                g_cscCallback(g_cscCallbackData);
