...
BOOL SpdifTxHwContext::InitOutputDMA(PHYSICAL_ADDRESS *pAddress)
{
    ...
    DDKApbxDmaEnableCommandCmpltIrq(APBX_CHANNEL_SPDIFTX,FALSE);
    
    // Put the DMA in RESET State
    DDKApbxDmaResetChan(APBX_CHANNEL_SPDIFTX,TRUE);
...
BOOL SpdifTxHwContext::StartOutputDMA()
{
    ...
    if(!m_bOutputDMARunning)
    {
        ...
        {
            if(!DDKApbxDmaInitChan(APBX_CHANNEL_SPDIFTX,TRUE))
            {
            ...
            DDKApbxDmaClearCommandCmpltIrq(APBX_CHANNEL_SPDIFTX);
            ...
            DDKApbxStartDma(APBX_CHANNEL_SPDIFTX,(PVOID)m_DMAOutDescriptorPhysAddr, 2);
...
void SpdifTxHwContext::InterruptThread()
{
    ...
    if(!DDKApbxDmaGetActiveIrq(APBX_CHANNEL_SPDIFTX)||!m_bOutputDMARunning)
    {
        return;
    }
    ...
    if(DDKApbxGetNextCMDAR(APBX_CHANNEL_SPDIFTX) == (DWORD)&m_DMAOutDescriptorPhysAddr[BUFFER_A])
    {
        // The data in DMA buffer A has been transmitted and the
        // buffer may now be reused.
        m_dwOutputDMAStatus |= DMA_DONEA;
    }
    if(DDKApbxGetNextCMDAR(APBX_CHANNEL_SPDIFTX) == (DWORD)&m_DMAOutDescriptorPhysAddr[BUFFER_B])
    {
