...
BOOL CheckStatus(WORD Status, PBYTE pBusy)
{
    ...
    {
        *pBusy = TRUE;
        DEBUGMSG(TRUE, (_T("StatusBusy\r\n")));
        return TRUE;   
    }
    if(Status & (1 << g_CSPfi.StatusErrorBit))
    {
        DEBUGMSG(TRUE, (_T("StatusError\r\n")));
        return FALSE;   
    }
    *pBusy = FALSE;
    return TRUE;
...
void UnMapRegister(VOID)
{
    ...
    if(pv_HWregOTP)
    {
        BSPNAND_UnmapRegister(pv_HWregOTP, 0x1000);
    }
...
BOOL MapRegister(VOID)
{
    ...
    pv_HWregOTP = (PVOID) BSPNAND_RemapRegister(CSP_BASE_REG_PA_OCOTP, 0x1000);
    // Check if virtual mapping failed
    if (pv_HWregOTP == NULL)
    {
        RETAILMSG(1, (_T("MapRegister:  BSPNAND_RemapRegister for OCOTP registers failed!\r\n")));
        goto cleanup;
    }
...
VOID CSPNAND_PostInit(PFlashInfoExt pFlashInfo)
{
    ...
    pFlashPageBuf   = (UINT16*)DMA_MemAlloc(pFlashInfo->fi.wDataBytesPerSector + FLASH_SECTOR_INFO_BUFFER_SIZE);
    pFlashAuxBuf    = (UINT16*)((PBYTE)pFlashPageBuf + pFlashInfo->fi.wDataBytesPerSector);
    
    dwOTP = HW_OCOTP_ROMn_RD(1);
    RETAILMSG(TRUE, (_T("ROM1 = 0x%x\r\n"),dwOTP));
    if(((dwOTP & 0x7) == 0x0) && (g_CSPfi.NumberOfChip > 1))
    {
        //number_of_nand > 1, and fuse not burned
        g_OffsetFactor = 2;
        if(g_CSPfi.fi.wSectorsPerBlock == 64)
        {
            //SLC
            g_OffsetFactor = 1;
            RETAILMSG(1,(_T("g_OffsetFactor = 1\r\n")));
        }
    }
    NANDBootReserved();
}
    ...
    PSectorInfo pSectorInfoBuff)
{
    ...
    if(dwCs == 0 && startSectorAddr < (SECTOR_ADDR)CHIP_LDLB_NAND_OFFSET * g_CSPfi.fi.wSectorsPerBlock / g_OffsetFactor)
    {
        EnableECC = FALSE;
    }
    else
    ...
    if(!pSectorBuff)
    {
        ...
        readSize = sizeof(SectorInfo) + ECC0_2K4K_PAGE * 13 / 4;
    }
    else
    {
    ...
    if(pSectorInfoBuff)
    {
        memcpy(pSectorInfoBuff,pFlashAuxBuf,sizeof(*pSectorInfoBuff));
        if(startSectorAddr < (SECTOR_ADDR)(CHIP_LDLB_NAND_OFFSET + CHIP_LDLB_NAND_BLOCKS) * g_CSPfi.fi.wSectorsPerBlock)
        {
    ...
    PSectorInfo pSectorInfoBuff)
{
    ...
    memset(pFlashAuxBuf,0xFF,sizeof(SectorInfo));
    ...
    if(pSectorInfoBuff)
    {
        memcpy(pFlashAuxBuf,pSectorInfoBuff,sizeof(*pSectorInfoBuff));
    }
