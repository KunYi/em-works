    ...
    const PMIC_AUDIO_OUTPUT_PORT port)
{
         ...
         (extStereoIn.handleState == HANDLE_IN_USE)))
    {
                    ...
                    (port & MONO_LOUDSPEAKER))))
        {
            ...
            if ((port & STEREO_HEADSET_LEFT) && (handle == stDAC.handle && extStereoIn.handleState != HANDLE_IN_USE))   
            {
                reg_mask |= regRX0.AHSLEN.mask;
            }
            if ((port & STEREO_HEADSET_RIGHT) && (handle == stDAC.handle && extStereoIn.handleState != HANDLE_IN_USE))  
            {
