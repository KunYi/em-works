...
static CRITICAL_SECTION g_csPrPLock;    // Critical section for PP
...
void Display_SetupPrP4Overlay(pOverlaySurf_t pOverlaySurfaceInfo);
...
void Display_Top_SetupOverlay()
{
    ...
    // Configure PrP task
    Display_SetupPrP4Overlay(g_ppOverlays[g_iNumOverlays-1]);
...
void Display_SetupDP4Overlay(pOverlaySurf_t pOverlaySurfaceInfo)
{
    ...
    dispConfigData.bGlobalAlpha = pOverlaySurfaceInfo->globalAlpha;
                   ...
                   int iRotate, EDDGPEPixelFormat PixelFormat, int RealWidth, BOOL bPerPixelAlpha)
{
    ...
    {
        ERRORMSG(1, (TEXT("Overlay with per-pixel alpha cannot be resized (it will lose alpha data)!\r\n")));
        return FALSE;     
    }
...
void Display_SetupPrP4Overlay(pOverlaySurf_t pOverlaySurfaceInfo)
{
    ...
    if (prpData_Overlay.inputIDMAChannel.FrameFormat == icFormat_YUV420)
    {
        ...
        g_nOverlayBufferOffset[g_iNumOverlays-1] = pSrcRect->top * pOverlaySurfaceInfo->SrcWidth + pSrcRect->left;
        //Compute u,v buffer offset from source rectangle, the base offset changed
    ...
    else
    {
            ...
            g_nOverlayBufferOffset[g_iNumOverlays-1] = pSrcRect->top * pOverlaySurfaceInfo->SrcLineStride + pSrcRect->left * pOverlaySurfaceInfo->SrcBpp / 8;
    }
...
//    prpData_Overlay.bVFEnable = !g_pDI1PanelInfo->ISACTIVE;
    ...
    prpData_Overlay.outputVFIDMAChannel.LineStride = pOverlaySurfaceInfo->DstLineStride;  //acctually prp didn't use this parameter
