...
UINT8 DDKSdmaOpenChan(DDK_DMA_REQ dmaReq, UINT8 priority)
{
    ...
    if (g_pHostSharedUA->chanDesc[chan].dmaMask[0] || g_pHostSharedUA->chanDesc[chan].dmaMask[1] )
    {
        // Search through first 32 DMA requests specified for this channel
        for (i = 0, dmaMask = 0x1; i < SDMA_NUM_CHANNELS; i++, dmaMask <<= 1)
        {
            if (dmaMask & g_pHostSharedUA->chanDesc[chan].dmaMask[0])
            {
        ...
        // Search through last 16 DMA requests specified for this channel
        for (dmaMask = 0x1; i < SDMA_NUM_REQUESTS; i++, dmaMask <<= 1)
        {
            if (dmaMask & g_pHostSharedUA->chanDesc[chan].dmaMask[1])
            {
                // Bind DMA requests to virtual channel
                SETREG32(&g_pSDMA->CHNENBL[i], CHAN_MASK(chan));
            }
...
BOOL DDKSdmaFreeChain(UINT8 chan)
{
    ...
    {
        ERRORMSG(TRUE, (_T("Invalid channel!\r\n")));
        goto cleanUp;
    }
...
BOOL SdmaInit(void)
{
    ...
    if (g_HostSharedSize < sizeof(SDMA_HOST_SHARED_REGION))
    {
        RETAILMSG(TRUE, (_T("WARNING:  SDMA host shared region = %d bytes, allocation is %d bytes\r\n"), 
            sizeof(SDMA_HOST_SHARED_REGION), g_HostSharedSize));
    }
    ...
    for (dmaEvent = 0; dmaEvent < SDMA_NUM_REQUESTS; dmaEvent++)
    {
