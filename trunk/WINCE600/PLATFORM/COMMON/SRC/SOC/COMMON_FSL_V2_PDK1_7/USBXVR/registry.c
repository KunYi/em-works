...
//
// Read a string from the corresponding registry with a given key (pParam->name)
//
// Parameter:
//     hKey
//         [IN] Handle to registry
//     pBase
//         [IN/OUT] Pointer to PDD context, it also stores the output value into 
//                  its offset depending on the pParam
//     pParam
//         [IN] Pointer to a table contanting DEVICE_REGISTRY_PARAM structure
//
// Return:
...
//
// Read a dword value from the corresponding registry with a given key
// (pParam->name)
//
// Parameter:
//     hKey
//         [IN] Handle to registry
//     pBase
//         [IN/OUT] Pointer to PDD context, it also store sthe output value into 
//                  its offset depending on the pParam
//     pParam
//         [IN] Pointer to a table contanting DEVICE_REGISTRY_PARAM structure
//
...
//
// Read a multiple dwords from the corresponding registry with a given key 
// (pParam->name)
//
// Parameter:
//     hKey
...
//
// Return:
//    The multiple dword result would be output to pBase with offset 
//    pParam->offset If success, ERROR_SUCCESS would be returned, otherwise, 
//    failure
//
//------------------------------------------------------------------------------
...
//
// Read binary format data from the corresponding registry with a given key 
// (pParam->name)
//
// Parameter:
//     hKey
//         [IN] Handle to registry
//     pBase
//         [IN/OUT] Pointer to PDD context, it also store the output value into 
//                  its offset depending on the pParam
//     pParam
//         [IN] Pointer to a table contanting DEVICE_REGISTRY_PARAM structure
//
...
// Parameter:
//     context
//         [IN] Pointer to the active registry path
//     pBase
//         [IN/OUT] Pointer to PDD context, it also stores the output value into
//                  its offset depending on the pParam
//     count
//         [IN] Number of registry key to read, it should match with params[]
//     params
//         [IN] Pointer to arrays of DEVICE_REGISTRY_PARAM structure
//
// Return:
