...
//-----------------------------------------------------------------------------
//
// Function: LRADCEnableVDD5VMeasurement
//
// This function enables the LRADC channel for VDD5V measurement.
//
// Parameters:
//      hLRADC
//          [in] The LRADC device handle retrieved from LRADCOpenHandle().
...
UINT32 LRADCEnableVDD5VMeasurement(HANDLE hLRADC)
{
    UINT32 u32VDD5V = 0;
    if (!DeviceIoControl(hLRADC,                // file handle to the driver
                         LRADC_IOCTL_MEASUREVDD5V,   // I/O control code
                         NULL,                                // in buffer
                         sizeof(STLRADCCONFIGURE),              // in buffer size
                         &u32VDD5V,                         // out buffer
                         sizeof(UINT32),                     // out buffer size
...
//          [in] The LRADC device handle retrieved from LRADCOpenHandle().
//      Use5WireTouch
//          [in] To set the 5 wire type or 4 wire type touch
//      bValue
//          [in] To set or clear the TOUCH_DETECT_IRQ_EN bit
//
// Returns:
...
BOOL LRADCSetAnalogPowerUp(HANDLE hLRADC,BOOL Use5WireTouch,BOOL bValue)
{
    STLRADCCONFIGURE stLRADCConfig;
    stLRADCConfig.bValue = bValue;
     stLRADCConfig.Use5WireTouch = Use5WireTouch;
    if (!DeviceIoControl(hLRADC,                // file handle to the driver
...
//-----------------------------------------------------------------------------
//
// Function: LRADCEnableButtonDetect
//
// This function set or clear the  BUTTON0_DETECT_ENABLE  in HW_LRADC_CTRL0 Register.
//
// Parameters:
//      hLRADC
//          [in] The LRADC device handle retrieved from LRADCOpenHandle().
...
{
    LRADCBUTTONCONFIGURE ButtonConfigure;
    ButtonConfigure.bValue = bValue;
    ButtonConfigure.button= button;

    if (!DeviceIoControl(hLRADC,                // file handle to the driver
                         LRADC_IOCTL_ENABLE_BUTTON_DETECT, // I/O control code
                         &ButtonConfigure,                                // in buffer
                         0,                                             // in buffer size
                         NULL,                                  // out buffer
                         0,                                             // out buffer size
                         NULL,                                  // pointer to number of bytes returned
...
//-----------------------------------------------------------------------------
//
// Function: LRADCEnableButtonDetectInterrupt
//
...
BOOL LRADCEnableButtonDetectInterrupt(HANDLE hLRADC,LRADC_BUTTON button,BOOL bValue)
{
    ...
    if (!DeviceIoControl(hLRADC,                // file handle to the driver
                         LRADC_IOCTL_ENABLE_BUTTON_DETECT_IRQ, // I/O control code
                         &ButtonConfigure,                                // in buffer
                         0,                                             // in buffer size
                         NULL,                                  // out buffer
                         0,                                             // out buffer size
...
//-----------------------------------------------------------------------------
//
// Function: LRADCClearButtonDetect
//
// This function clear BUTTON0_DETECT_IRQ or  BUTTON1_DETECT_IRQ in HW_LRADC_CTRL1 Register.
//s
// Parameters:
//      hLRADC
//          [in] The LRADC device handle retrieved from LRADCOpenHandle().
...
BOOL LRADCClearButtonDetect(HANDLE hLRADC,LRADC_BUTTON button)
{
    ...
    if (!DeviceIoControl(hLRADC,                // file handle to the driver
                     LRADC_IOCTL_CLR_BUTTON_DETECT, // I/O control code
                     &ButtonConfigure,               // in buffer
                     0,                                             // in buffer size
                     NULL,                                  // out buffer
                     0,                                             // out buffer size
