...
DWORD WDT_Open(DWORD hDeviceContext, DWORD AccessCode, DWORD ShareMode)
{
	...
	if(hDeviceContext != 1)
	{
        RETAILMSG(1, (TEXT("WDT_Open: wrong handle 0x%x!\r\n"), hDeviceContext));
		return NULL;
	}
	...
	if( !pWDT )
	{
        RETAILMSG(1, (TEXT("WDT_Open: memory allocation failed!\r\n")));
		return NULL;
	}
	...
	if( !KernelIoControl(IOCTL_HAL_WATCHDOG_GET,
							...
							&dwReturnBytes) )
	{
        RETAILMSG(1, (TEXT("WDT_Open: IOCTL_HAL_WATCHDOG_GET failed!!!\r\n")));
		LocalFree( pWDT );
		return NULL;
	}
	...
	{
        RETAILMSG(1, (TEXT("WDT_Open: invalid watch info!\r\n")));
		LocalFree( pWDT );
		return NULL;
	}
	RETAILMSG(1, (TEXT("WDT_Open: fn-%p Period: %d\r\n"), pWDT->pfnKickWatchDog, pWDT->dwWatchDogPeriod));
