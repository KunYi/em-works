...
extern "C" BOOL SerialReadData (DWORD cbData, LPBYTE pbData)
{
    ...
    static PBYTE pBlock = dataBuffer + sizeof(SERIAL_BLOCK_HEADER);
...
extern "C" BOOL SerialSendBootRequest(const char * platformString)
{
    ...
    // create boot request
    strncpy_s((char *)pBootReq->PlatformId, sizeof(pBootReq->PlatformId), platformString, sizeof(pBootReq->PlatformId) - 1);
