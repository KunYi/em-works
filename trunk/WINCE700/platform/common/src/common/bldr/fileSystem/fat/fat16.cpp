...
{
    BootFileSystemFat16_t* pThis = NULL;

    pThis = new BootFileSystemFat16_t();
    ...
    )
{
    ...
    if (memcmp(
            pBpb->fatType, "FAT16   ", sizeof(pBpb->fatType)
            ) != 0) goto cleanUp;
    ...
    )
{
    ...
    cluster = *((uint16_t*)&m_pSector[offset]);
    if (remove)
        {
        *((uint16_t*)&m_pSector[offset]) = 0;
        m_sectorDirty = true;
        }

    rc = (cluster >= 2) && (cluster <= 0xFFEF);
    ...
    )
{
    ...
    for (sector = 0; sector < m_sectorsPerFat; sector++)
        ...
        while (offset < m_sectorSize)
            {
            if (*(uint16_t*)&m_pSector[offset] == 0) break;
    ...
    *(uint16_t*)&m_pSector[offset] = 0xFFFF;
    m_sectorDirty = true;
    ...
    if (cluster > 1)
        ...
        offset -= sector * m_sectorSize;
        if (!ReadToBuffer(m_fatSector + sector)) goto cleanUp;
        *(uint16_t*)&m_pSector[offset] = (uint16_t)freeCluster;
        m_sectorDirty = true;
        }
