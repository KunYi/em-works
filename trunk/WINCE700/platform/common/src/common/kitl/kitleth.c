...
#define htons(v)    ((UINT16)(((0x00FF&((UINT16)v)) << 8)|(((UINT16)((UINT16)v)) >> 8)))
#define ntohs(v)    htons(v)
#define htonl(v)    ((((ULONG)v) << 24)|((0x0000FF00UL&((ULONG)v)) << 8)| \
                    ((0x00FF0000UL&((ULONG)v))>>8)|(((ULONG)v) >> 24))
#define ntohl(v)    htonl(v)

//------------------------------------------------------------------------------
//
//  Global:  DmaBufferBase, DmaBufferSize
//
//  Thes global variables are for KITL Driver's DMA buffer base and size.
//  This is a fix-up variable that you can optionally override in config.bib
//
UINT32 DmaBufferBase = (UINT32)-1;
UINT32 DmaBufferSize = (UINT32)-1;
...
static struct {
    ...
    UINT32 dhcpServerIP;
    UINT32 dhcpXId;
    UINT32 FailedDHCPIPAddress; // If non-zero then a DHCP Renew failed previously
...
static VOID SendDHCP(DHCP_MESSAGE_TYPE msgType, UINT32 requestIP)
{
    ...
    size = EncodeUDP(
        pFrame, (UINT16)(sizeof(BOOTP_MESSAGE) + offset), NULL, requestIP, 0xFFFFFFFF,
...
// Since the KITL protocol doesn't support changing the IP address once assigned
// report the DHCP failure once a minute so it's possible to see that the IP address
// changed breaking the KITL connection. 
static VOID ReportDHCPRenewFailure(VOID *pParam)
{
    // Schedule this function to run again in 60 seconds
    KITLOutputDebugString(
        "DHCP Renew of %S failed, IP address has changed or is invalid (message repeats every minute)\r\n",
        OALKitlIPtoString(g_kitlEthState.FailedDHCPIPAddress)
        );
    KitlSetTimerCallback(60, ReportDHCPRenewFailure, pParam);  

}    


static BOOL IsIPInUse (void)
{
    UINT32 startTime;
    UINT16 size;
    ...
    while ((OALGetTickCount() - startTime) < ARP_TIMEOUT) 
        ...
        if (g_kitlEthState.pfnRecv(g_kitlEthState.packet, &size)) 
            ...
            if (GetOpARP(g_kitlEthState.packet, size) == ARP_RESPONSE)
                {
                ETH_HEADER *pEth = (ETH_HEADER*)g_kitlEthState.packet;
                ARP_MESSAGE *pARP = (ARP_MESSAGE*)((UINT8*)pEth + sizeof(ETH_HEADER));

                // if received ARP is not intended for us then receive skip
                if(pARP->destip != g_kitlEthState.deviceIP) continue;
    ...
    return !g_kitlEthState.deviceIP;
}

//------------------------------------------------------------------------------

static VOID GetAddressDHCP (void)
{
    ...
    if (g_kitlEthState.deviceIP != 0) {
        ...
        while (attempts++ < 10) {
            ...
            while ((OALGetTickCount() - startTime) < DHCP_TIMEOUT) {
                ...
                // If we are in BOUND state we successfully renewed it...
                if (g_kitlEthState.dhcpState == DHCP_BOUND) goto cleanUp;
            }
            KITLOutputDebugString ("No response from DHCP server for %i seconds, resend DHCP request\r\n",DHCP_TIMEOUT / 1000);
        }
    }
...
cleanUp:
    if(g_kitlEthState.deviceIP != 0)
        KITL_RETAILMSG(ZONE_INIT, ("KITL: DHCP get device IP: %S\r\n",
    ...
    case DHCP_REQUESTING:
        if (msgType == DHCP_ACK) {
            ...
            if (pOption != NULL) {
                ...
                value = ntohl (value);
                rc = KitlSetTimerCallback(value, RenewDHCP, NULL);
            ...
            g_kitlEthState.fDHCPAddressValid = TRUE;
            KITL_RETAILMSG (ZONE_INIT, ("DHCP Renew successful, IP = %S, Lease time = %d seconds\r\n",
                            OALKitlIPtoString(g_kitlEthState.deviceIP),
                            pOption? value : 1800
                            ));
        } else if (msgType == DHCP_NAK) {
            invalidIP = g_kitlEthState.deviceIP;
            g_kitlEthState.deviceIP = 0;
            // Discover DHCP servers
            SendDHCP(DHCP_DISCOVER, 0);
            // Start with discover again
            g_kitlEthState.dhcpState = DHCP_SELECTING;
            g_kitlEthState.fDHCPAddressValid = FALSE;
            if( g_kitlEthState.FailedDHCPIPAddress == 0 ){
                g_kitlEthState.FailedDHCPIPAddress = invalidIP;
                ReportDHCPRenewFailure( NULL );   // Begin reporting this issue regularly
            }                                    
        }
        break;
    }
...
static UINT8* DecodeUDP(UINT8 *pFrame, UINT16 *pLength, BOOL *pUsed)
{
    ...
    *pUsed = FALSE;
    ...
    case KITL_CLIENT_PORT:
        // if we don't have an DHCP address yet, ignore this packet
        if (DHCP_BOUND != g_kitlEthState.dhcpState && 
            g_kitlEthState.fDHCPAddressValid == FALSE) {
            break;
        }
        ...
        *pLength = (UINT16)(udpLength - sizeof(UDP_HEADER));
        break;
    }
...
static UINT8* KitlEthDecode(UINT8 *pFrame, UINT16 *pLength)
{
    ...
    KITL_RETAILMSG(ZONE_RECV, (
        "-KitlEthDecode(pData = 0x%x, length = %d)\r\n", pData, *pLength
    ));
    return pData;
}
...
static BOOL KitlEthSend(UCHAR *pData, USHORT length)
{
    ...
    if (g_kitlEthState.updateFilter) {
        ...
        VBridgeUCurrentPacketFilter((PDWORD)&g_kitlEthState.deviceFilter);
    ...
    do {
        ...
        if (code == 0) {
            if (g_kitlEthState.flags & OAL_KITL_FLAGS_VMINI) {
                ...
                while (VBridgeKGetOneTxBuffer(&pSendData, (UINT *)&sendLength)) {
...
BOOL OALIoCtlVBridge(
    ...
    KITL_RETAILMSG(ZONE_SEND|ZONE_RECV, ("+OALIoCtlVBridge(0x%x,...)\r\n", code));
    ...
    case IOCTL_VBRIDGE_WILD_CARD:
        rc = VBridgeUWildCard(
            pInBuffer, inSize, pOutBuffer, outSize, (LPDWORD)pOutSize
        );
        break;
...
BOOL OALKitlEthInit(
    ...
    if (pDriver->pfnInitDmaBuffer != NULL) {
        KITL_RETAILMSG(ZONE_INIT, ("DMA Buffer [%x, %x]\r\n", DmaBufferBase, DmaBufferSize));
        // Map g_oalKitlBuffer as DMA Buffer if BSP DmaBuffer is not provided.
        if (DmaBufferBase == -1 || DmaBufferSize == -1)
        {
            UINT32 pa, offset = 0;

            // If either DmaBufferBase or DmaBufferSize is not supplied, just remap the g_oalKitlBuffer to uncached DMA buffer.
            // Try OALPAtoUA first, if it does not work (probably because of using new address table) then try NKCreateStaticMapping.
            // Due to uncached static mapping in kernel space is not existed when using new address table,
            // we need to OALVAtoPA to get the PA first and then NKCreateStaticMapping to obtain the UA for DMA buffer.
            // Note that NKCreateStaticMapping requires PA shifted to the right by 8 bit and Size to be page aligned.
            //
            pa = OALVAtoPA(g_oalKitlBuffer);
            DmaBufferBase = (UINT32)OALPAtoUA(pa);
            DmaBufferSize = sizeof(g_oalKitlBuffer);
            if (DmaBufferBase == 0)
            {
                offset = pa & (VM_PAGE_SIZE - 1);
                pa &= ~(VM_PAGE_SIZE - 1);
                DmaBufferBase = (UINT32)NKCreateStaticMapping(pa>>8, sizeof(g_oalKitlBuffer) + offset);
                DmaBufferBase += offset;
                KITL_RETAILMSG(ZONE_INIT, (
                    "NKCreateStaticMapping PA[%x] UA[%x, %x] offset[%x]\r\n",
                    pa, DmaBufferBase, DmaBufferSize, offset
                ));
            }
        }
        KITL_RETAILMSG(ZONE_INIT, (
            "DMA Buffer [%x] pa[%x] UA=>CA[%x] CA=>UA[%x]\r\n", 
            DmaBufferBase, 
            OALVAtoPA((PVOID)DmaBufferBase), 
            OALUAtoCA(DmaBufferBase), 
            OALCAtoUA(OALUAtoCA(DmaBufferBase))));
    ...
    g_kitlEthState.dhcpState = DHCP_BOUND;
    g_kitlEthState.FailedDHCPIPAddress = 0;  
    ...
    if ((pArgs->flags & OAL_KITL_FLAGS_VMINI) != 0) {
        VBridgeInit();
        VBridgeKSetLocalMacAddress((PUCHAR)pArgs->mac);
    }
#endif
