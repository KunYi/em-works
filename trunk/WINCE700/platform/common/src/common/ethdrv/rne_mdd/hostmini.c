...
//          if (!PDD_Get(
                    ...
                    &ulRequiredLength))
            {
                ...
                ulTotalBytes = strlen((const char *)DefaultVendorID);
...
//          if (!PDD_Get(
                    ...
                    &ulRequiredLength))
            {
                ...
                ulTotalBytes = strlen((const char *)DefaultVendorDescription);
...
//
BOOL
HostMiniInit( UINT8 *pbBaseAddress, UINT32 dwLogicalLocation, UINT16 MacAddr[3])
{
    //
...
*/
    ...
    //
    if (!RndisInit( pbBaseAddress,dwLogicalLocation,MacAddr))
    {
//      CloseHandle(RndisMdd.hRndisMddEvent);
        return FALSE;
    }
