...
BOOL CS8900AInit(UINT8 *pAddress, UINT32 logicalLocation, UINT16 mac[3])
{
    ...
    KITL_RETAILMSG(ZONE_KITL_ETHER, (
        "+CS8900AInit(0x%08x, 0x%08x, %02x:%02x:%02x:%02x:%02x:%02x)\r\n",
        pAddress, logicalLocation, mac[0]&0xFF, mac[0]>>8, mac[1]&0xFF, mac[1]>>8,
        mac[2]&0xFF, mac[2]>>8
        ));
    ...
    KITL_RETAILMSG(ZONE_KITL_ETHER, ("INFO: CS8900AInit chip detected\r\n"));
    ...
    if (count == 0) {
        KITL_RETAILMSG(ZONE_ERROR, (
            "ERROR: CS8900AInit: Failed to reset card\r\n"
            ));
        goto cleanUp;
    }
    count = RETRY_COUNT;
...
cleanUp:
    KITL_RETAILMSG(ZONE_KITL_ETHER, ("-CS8900AInit(rc = %d)\r\n", rc));
    return rc;
}
...
cleanUp:
    KITL_RETAILMSG(ZONE_KITL_ETHER, ("-CS8900ASendFrame(rc = %d)\r\n", !rc));
    return !rc;
}
...
VOID CS8900AEnableInts()
{
    ...
    KITL_RETAILMSG(ZONE_KITL_ETHER, ("+CS8900AEnableInts\r\n"));
    
    // Clear the Interrupt Status Queue before enabling interrupts
    do
    {
        isq = INPORT16(&g_pCS8900->ISQ);
    } while(isq != 0);
...
VOID CS8900ADisableInts()
{
    ...
    KITL_RETAILMSG(ZONE_KITL_ETHER, ("+CS8900ADisableInts\r\n"));
    data = ReadPacketPage(BUS_CTL);
    WritePacketPage(BUS_CTL, data & ~BUS_CTL_ENABLE_IRQ);
    KITL_RETAILMSG(ZONE_KITL_ETHER, ("-CS8900ADisableInts\r\n"));
}
...
VOID CS8900ACurrentPacketFilter(UINT32 filter)
{
    ...
    KITL_RETAILMSG(ZONE_KITL_ETHER, (
        "+CS8900ACurrentPacketFilter(0x%08x)\r\n", filter
        ));
    ...
    KITL_RETAILMSG(ZONE_KITL_ETHER, ("-CS8900ACurrentPacketFilter\r\n"));
}
...
BOOL CS8900AMulticastList(UINT8 *pAddresses, UINT32 count)
{
    ...
    KITL_RETAILMSG(ZONE_KITL_ETHER, ("-CS8900AMulticastList(rc = 1)\r\n"));
    return TRUE;
}
