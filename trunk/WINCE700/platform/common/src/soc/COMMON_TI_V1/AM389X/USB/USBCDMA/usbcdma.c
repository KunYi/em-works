...
#include "am389x_usb.h"
#include "am389x_usbcdma.h"
#pragma warning(pop)
...
HANDLE USBCDMA_RegisterUsbModule(UINT16 nHdCount, UINT8 cbHdSize, PHYSICAL_ADDRESS *ppaHdPool,
    VOID **ppvHdPool, VOID (*callback)(VOID *), VOID *param )
{
    ...
    DEBUGMSG(ZONE_INIT || ZONE_VERBOSE,
             ...
             nHdCount, cbHdSize, ppaHdPool, ppvHdPool, callback, param));
...
{
    if(chanNum >= USB_CPPI_MAX_RX_CHANNELS) return;
...
{
    if(chanNum >= USB_CPPI_MAX_TX_CHANNELS) return;
...
DWORD UCD_Init(LPCTSTR pContext, DWORD dwBusContext)
{
    ...
    pa.LowPart = AM389X_CPPI_REGS_PA;
    ...
    pa.LowPart = AM389X_DEVICE_CONF_REGS_PA;
    g_Dc.pSys = (AM389X_DEVICE_CONF_REGS *)MmMapIoSpace(pa, sizeof(AM389X_DEVICE_CONF_REGS), FALSE);
    if (g_Dc.pSys == NULL) {
	...
	PrcmDomainResetRelease(PWR_DEFAULT, USB2_LRST | USB1_LRST );

	EnableDeviceClocks(AM_DEVICE_USB, TRUE);

	g_Dc.pSys->USB_CTRL |= 0x3;
	g_Dc.pSys->USB_CTRL &= ~0x100;
