    ...
    )
{
    ...
    */
	...
	if(pAdapter->m_HwStatus != NdisHardwareStatusReset)
	{
		...
		if(pAdapter->m_EmacBaseAddr)
		{
			NdisMUnmapIoSpace(pAdapter->m_AdapterHandle,
							  (PVOID)pAdapter->m_EmacBaseAddr,
							  g_EmacMemLayout.EMAC_TOTAL_MEMORY);

			pAdapter->m_EmacBaseAddr = 0;
		}
...
)
{
    ...
    if (!KernelIoControl(
			(pAdapter->m_device == AM_DEVICE_EMAC0)?IOCTL_HAL_GET_MAC_ADDRESS:
			                                        IOCTL_HAL_GET_MAC_ADDRESS1 ,
...

{
    DWORD                               phys = 0;
    DWORD                               dwEmacIndex = 0;
	DWORD								PhyAddr = 32;  // Unknown address
    ...
    NDIS_STRING                         PhyAddrRegName = NDIS_STRING_CONST("PhyAddress");

    DEBUGMSG(DBG_FUNC, (L"--> NICReadAdapterInfo\r\n"));
//RETAILMSG(1, (L"--> NICReadAdapterInfo\r\n"));
...
//RETAILMSG(1, (L"NdisOpenConfiguration failed\r\n"));
        return Status;
    }
...
//RETAILMSG(1, (L"NdisOpenConfiguration Status %d; index %d; 0x%08x\r\n",
//		  Status, dwEmacIndex, pReturnedValue));
    if (Status == NDIS_STATUS_SUCCESS)
    {        
    ...
    if (Status != NDIS_STATUS_SUCCESS)
    {
        ...
        if (pv)
        {                
            RETAILMSG(1/*FALSE*/, (L"NICReadAdapterInfo: Allocated Buffers 0x%X.\r\n", (DWORD) phys));
            Status = NDIS_STATUS_SUCCESS;
        }
        else
        {
            RETAILMSG(1/*FALSE*/, (L"NICReadAdapterInfo:Unable to get physicall buffers\r\n"));
        }
    }
    else
    {
        RETAILMSG(1/*FALSE*/, (L"NICReadAdapterInfo: Successfull read phys base as 0x%X.\r\n",
...
RETAILMSG(1, (L"pAdapter->m_TxBufBasePa=%p\r\n", pAdapter->m_TxBufBasePa));
RETAILMSG(1, (L"pAdapter->m_RxBufsBasePa=%p\r\n", pAdapter->m_RxBufsBasePa));

	/* Get the configuration value for a specific parameter for Phy Address */
    NdisReadConfiguration(&Status, &pReturnedValue, ConfigurationHandle,
						   &PhyAddrRegName, NdisParameterInteger);
    if (Status != NDIS_STATUS_SUCCESS){
        RETAILMSG(1, (L"NICReadAdapterInfo:Unable to get Phy Address\r\n"));
		PhyAddr = 1; // TODO:  need to decide what to do in that case
    } else {
        RETAILMSG(1/*FALSE*/, (L"NICReadAdapterInfo: Successfully read phy address as %d\r\n",
                         (DWORD) pReturnedValue->ParameterData.IntegerData));
        PhyAddr = (DWORD) pReturnedValue->ParameterData.IntegerData;
    }

	pAdapter->m_PhyAddress = PhyAddr;
    ...
    )
{
    ...
    RETAILMSG (1,(L"+Emac_MiniportInitialize() %d Status %d\r\n", __LINE__, Status));
