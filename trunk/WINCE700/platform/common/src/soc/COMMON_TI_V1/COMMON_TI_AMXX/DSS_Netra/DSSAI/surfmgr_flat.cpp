...
#if PROFILE_ENABLE
    LARGE_INTEGER   startTimeHighRes, stopTimeHighRes;
    DWORD           deltaHighRes;
...
#if PROFILE_ENABLE
    QueryPerformanceCounter(&startTimeHighRes);
#endif /* DMA_PROFILE_ENABLE */
...
#if PROFILE_ENABLE
    QueryPerformanceCounter(&stopTimeHighRes);
    deltaHighRes = stopTimeHighRes.LowPart - startTimeHighRes.LowPart;

    RETAILMSG(1, (L"OMAPFlatSurface::Allocate. Memset avoided for Addr=0x%x, Size=0x%x, High Res Counter=%d (in ~1/10000ms)\r\n",
                    pSurfHeap->Address(), pSurfHeap->NodeSize(), deltaHighRes));
#endif
...
#ifdef DEBUG
    RETAILMSG(1, (L"OMAPFlatSurface::Allocate. Paddr=0x%x, vaddr=0x%x, pSurfHeap=0x%x, allocsize=0x%x\r\n", 
                (DWORD)m_dwPhysicalAddr, (DWORD)pSurfHeap->Address(), (DWORD)pSurfHeap, allocateSize));
#endif
    ...
    )
{
    ...
    //  Get frame buffer memory attributes
    bResult = Display_GetFrameBufMemory( &m_dwDisplayBufferSize, &m_dwPhysicalDisplayAddr );
...
//        goto cleanUp;
    ...
    // memset the allocated memory so that display while booting up is clean.
    // (Memset per surface allocation is disabled for efficiency)
    memset(m_pVirtualDisplayBuffer, 0, m_dwDisplayBufferSize-dwOffscreenMemory);
...
#ifdef DEBUG
    RETAILMSG(1, (L"OMAPFlatSurfaceManager::Initialize. PhysBufferAddr = 0x%x, PhysOffScreenAddr = 0x%x, TotalPhysMemSize=0x%x, OffscreenMemSize = 0x%x\r\n", 
                    m_dwPhysicalDisplayAddr, m_dwOffscreenPhysical, m_dwDisplayBufferSize, dwOffscreenMemory));
#endif
