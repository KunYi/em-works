...
/* Single global instance. ToDo WinCE */
...
static int vps_grpx_flip_buffer(struct vps_grpx_ctrl *gctrl,
             u32 buffer_addr)
{
    static int count = 1;
    int r = 0;

//    VPSSDBG("(%d)- queue buffer %#x\n", gctrl->grpx_num, buffer_addr);
...
//    r = vps_grpx_apply_changes(gctrl);
    ...
    /* Dequeue in the callback. Set a flag to indicate that dequeue is needed */
    if (count >= 3)
    {
        r = vps_fvid2_dequeue(gctrl->handle,
...
void vps_fvid2_grpx_ctrl_init(struct vps_grpx_ctrl *gctrl)
{
    gctrl->inputf->scanformat = FVID2_SF_PROGRESSIVE;

    gctrl->gcparam->memtype = VPS_VPDMA_MT_NONTILEDMEM;
#if NETRA_GRPX_FLIPPING_SUPPORT
    gctrl->gcparam->drvmode = VPS_GRPX_STREAMING_MODE;
#else
    ...
    gctrl->gparams->regparams.regionposx = 0;
    gctrl->gparams->regparams.regionposy = 0;
    gctrl->gparams->regparams.firstregion = 1u;
    gctrl->gparams->regparams.lastregion = 1u;
    gctrl->gparams->regparams.regionwidth = REGION_WIDTH;
    gctrl->gparams->regparams.regionheight = REGION_HEIGHT;
    gctrl->gparams->regparams.disppriority = 1;
    gctrl->gparams->regparams.bbenable = 0;
    gctrl->gparams->regparams.bbalpha = 20;
    gctrl->gparams->regparams.blendalpha = 40;
    gctrl->gparams->regparams.blendtype = VPS_GRPX_BLEND_NO;

    gctrl->grtparam->regparams.firstregion = true;
    gctrl->grtparam->regparams.lastregion = true;
    gctrl->grtparam->regparams.disppriority = 1;

    VPSSDBG("(%d)- Grpx init: Width %d, Height %d, Scan fmt %d\n",
        gctrl->grpx_num, gctrl->gparams->regparams.regionwidth, 
        gctrl->gparams->regparams.regionheight, gctrl->inputf->scanformat);
    ...
    gctrl->flip_buffer = vps_grpx_flip_buffer;
...
void assign_payload_addr(struct vps_grpx_ctrl *gctrl,
...
int vps_grpx_init(struct platform_device *pdev)
{
    ...
    size = get_alloc_size();
    pinfo->vaddr = vps_sbuf_alloc(size, &pinfo->paddr, &pinfo->p_handle);
    ...
    for (i = 0; i < VPS_DISP_GRPX_MAX_INST; i++) {
        ...
        size = sizeof(struct vps_grpx_ctrl);
        gctrl = kzalloc(size, GFP_KERNEL);
...
// ToDo WinCE        mutex_init(&gctrl->gmutex);
        numends = 1;
...
void vps_grpx_deinit(struct platform_device *pdev)
{
    ...
    if (grpx_payload_info) {
        if (grpx_payload_info->vaddr) {
            vps_sbuf_free(grpx_payload_info);
        }
...
//    INIT_LIST_HEAD(&gctrl_list);
...

    {
        ...
        size = sizeof(struct vps_grpx_ctrl);
        VPSSDBG("sizeof(struct vps_grpx_ctrl)= 0x%x\n", size);
        gctrl = kzalloc(size, GFP_KERNEL);
...
#if NETRA_DESKTOP_COMPOSITE_OUTPUT
        gctrl->snode = VPS_DC_GRPX0_INPUT_PATH;
//      gctrl->snode = VPS_DC_GRPX2_INPUT_PATH;
        gctrl->enodes[0] = VPS_DC_SDVENC_BLEND;
#endif  // NETRA_DESKTOP_COMPOSITE_OUTPUT

#if NETRA_DESKTOP_SVIDEO_OUTPUT
        gctrl->snode = VPS_DC_GRPX0_INPUT_PATH;
//      gctrl->snode = VPS_DC_GRPX2_INPUT_PATH;
        gctrl->enodes[0] = VPS_DC_SDVENC_BLEND;
#endif  // NETRA_DESKTOP_SVIDEO_OUTPUT
