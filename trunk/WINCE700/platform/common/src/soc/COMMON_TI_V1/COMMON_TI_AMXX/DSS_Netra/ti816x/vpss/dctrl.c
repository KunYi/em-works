...
#if 0   // WinCE ToDo. Temporarily done by the M3. 
...
static int parse_def_clksrc(const char *clksrc)
{
    ...
    options = str;
    VPSSDBG("clksrc %s\n", clksrc);
    while (!r && (this_opt = strtok(options, ",")) != NULL) {
        char *p, *venc, *csrc;
        int vid, idx;
...
void vps_dc_ctrl_init(struct vps_dispctrl *dctrl)
{
              ...
              const char *clksrc)
{
    ...
    dc_payload_info->size = get_payload_size();
    /*always on the page size*/
    dc_payload_info->size = PAGE_ALIGN(dc_payload_info->size);

    vps_sbuf_alloc(dc_payload_info);
