...
#define REG_USBFN_DRV_PATH TEXT("Drivers\\BuiltIn\\USBOTG\\USBFN")
...
BYTE musb_test_packet[TEST_PKT_SIZE] = {
...
//-------------------------------------------------------------------------
BOOL OMAPMHSUSBOTG::WriteTestPkt(void *pData, DWORD size)
{
    DWORD total  = size/4;
    DWORD remain = size%4;
    DWORD i;
    ...
    DEBUGMSG(ZONE_OTG_INFO, (TEXT("WriteFIFO: total (0x%x), remain (0x%x), size(0x%x)\r\n"), total, remain, size));

    // this is 32-bit align
    for (i = 0; i < total; i++)
    {
        OUTREG32(&m_pOTG->pUsbGenRegs->fifo[0], *pDword++);
    }
    ...
    if (remain != 0)
    {
        ...
        if (remain & 0x2)  // either 2 or 3
        {
            ...
            OUTREG16(&m_pOTG->pUsbGenRegs->fifo[0], *pWORD++);
        }

        if (remain & 0x1)
        {
            // Write 1 byte to there
            OUTREG8(&m_pOTG->pUsbGenRegs->fifo[0], *((UCHAR*)pWORD));
        }
    }
    return TRUE;
}
                                 ...
                                 PBYTE pBufOut, DWORD dwLenOut,PDWORD pdwActualOut)
{
    ...
    switch (dwCode)
    {
        ...
        case IOCTL_USBOTG_REQUEST_TESTMODE:
             DEBUGMSG(ZONE_OTG_INFO, (TEXT("USB OTG in Test mode(+)\r\n")));
             if (pBufIn && (dwLenIn == sizeof(BYTE)))
             {  
             __try
                {
                DEBUGMSG(ZONE_OTG_INFO, (TEXT("In Buf vaild!!!!\r\n")));
                BYTE bReqTemp = *pBufIn;
                
                rc = StartUSBClock(TRUE);
                
                if((bReqTemp & 0x02) |(bReqTemp & 0x04))
                    {   
                    RETAILMSG(1, (TEXT("TEST_J or TEST_K!!!!\r\n")));
                    // For TEST_J or TEST_K, OPMODE need to set mode2
                    // Errata 1.179
                   m_pTransceiver->SetOPmode(2);
                    }
                    
                if(bReqTemp & 0x08)
                    {
                    RETAILMSG(1, (TEXT("TEST_PACKET!!!!\r\n")));
                    // For TEST_PACKET, OPMODE need to set mode0
                   
                    OUTREG8(&m_pOTG->pUsbGenRegs->Index, 0);
                    OUTREG16(&m_pOTG->pUsbCsrRegs->ep[0].CSR.CSR0, 0);
                    
                    if(WriteTestPkt(musb_test_packet, TEST_PKT_SIZE))
                        {
                        SessionRequest(TRUE,TRUE);
                        m_pTransceiver->SetOPmode(0);

                        OUTREG8(&m_pOTG->pUsbGenRegs->Testmode, bReqTemp);
                        OUTREG16(&m_pOTG->pUsbCsrRegs->ep[0].CSR.CSR0, CSR0_P_TXPKTRDY);
                        }
                    else
                        RETAILMSG(1, (TEXT("Write test packet to EP0 failed!!!\r\n")));    
                    }
                else
                    OUTREG8(&m_pOTG->pUsbGenRegs->Testmode, bReqTemp); 
                bReturn = TRUE;
             ...
             __except(EXCEPTION_EXECUTE_HANDLER)
                 {
                 DEBUGMSG(ZONE_OTG_ERROR, (L"Exception in IOCTL_USBOTG_REQUEST_TESTMODE ioctl\r\n"));
                 }
             }   
              break;
