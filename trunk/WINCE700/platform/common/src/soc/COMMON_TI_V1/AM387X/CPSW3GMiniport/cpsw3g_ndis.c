...
BOOL Cpsw3gReadMacAddrFromFuse(PCPSW3G_ADAPTER pAdapter, UINT32 port)
{
	...
	switch (port){
		case CPGMAC_PORT_START:
            NdisSetPhysicalAddressLow (PhysicalAddress, AM387X_DEVICE_CONF_REGS_PA + AM387X_MAC_ID0_OFFSET);
			break;
		case CPGMAC_PORT_END:
            NdisSetPhysicalAddressLow (PhysicalAddress, AM387X_DEVICE_CONF_REGS_PA + AM387X_MAC_ID1_OFFSET);
			break;
		default:
            rc = FALSE;
	}
