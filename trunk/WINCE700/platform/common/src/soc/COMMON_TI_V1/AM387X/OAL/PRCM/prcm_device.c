...
BOOL _PrcmHwEnableModuleClocks(AM387X_DEVICE_ID devId, BOOL bEnable)
{
	UINT32   timeout = 2000000;
	volatile unsigned int *pmodulemode;
	
	if (devId >= AM_DEVICE_COUNT) return FALSE;
	if (s_rgDeviceLookupTable[devId].CLKCTRL_REG == 0) 
		return TRUE; // there is no clkctrl reg for this device
...
// There are several groups of devices that share the one CLKCTRL_REG register per group.
// to control clock we use the fist device in the group

		switch (devId) {
			case AM_DEVICE_I2C2: devId = AM_DEVICE_I2C0; break;
			case AM_DEVICE_I2C3: devId = AM_DEVICE_I2C1; break;
			case AM_DEVICE_MCSPI1:
			case AM_DEVICE_MCSPI2:
			case AM_DEVICE_MCSPI3: devId = AM_DEVICE_MCSPI0; break; 
			case AM_DEVICE_GPIO2:
			case AM_DEVICE_GPIO3:  devId = AM_DEVICE_GPIO1; break;
		}
...
BOOL EnableDeviceClocks(AM387X_DEVICE_ID devID, BOOL bEnable)
{
...
BOOL DeviceInitialize(void)
{
	...
	for (j=0; j<AM_DEVICE_COUNT; j++){
		...
		if (s_rgDeviceLookupTable[j].fclk_domain != CLKDMN_UNKNOWN && 
			...
			ClockEnableClkDomain(s_rgDeviceLookupTable[j].fclk_domain, TRUE);
			OALMSG(OAL_FUNC, (L"\tDeviceInitialize: updating clk domain %d for dev %d\r\n",
		...
		if (s_rgDeviceLookupTable[j].iclk_domain != CLKDMN_UNKNOWN && 
			...
			ClockEnableClkDomain(s_rgDeviceLookupTable[j].iclk_domain, TRUE);
			OALMSG(OAL_FUNC, (L"\tDeviceInitialize: updating clk domain %d for dev %d\r\n",
