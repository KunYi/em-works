...
BOOL SdhcCardDetect()
{
    ...
    /* if BSPGetSDHCCardDetect() returns -1, then card detection is not GPIO based */
    if (BSPGetSDHCCardDetect(m_dwSlot) == (DWORD)(-1)) 
	return TRUE;
...
static BOOL InitializeHardware(DWORD Slot)
{
    ...
    if(!m_pbRegisters)
    {
        OALMSG(1, (TEXT("Init HW: No SDHC device found\r\n")));
        return FALSE;
    }
...
static void SdhcControllerInit()
{
    ...
    m_dwSlot = BSPGetBootMMCSlotConfig();
    m_dwSDIOCard = 0;
