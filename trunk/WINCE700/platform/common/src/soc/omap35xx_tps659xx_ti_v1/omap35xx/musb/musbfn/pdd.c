    ...
    if (bHalt)
    {
        ...
        {        
            if (SwitchReadDma2FIFO(pPdd))
            {
                ...
                if (INREG32(&pOTG->pUsbDmaRegs->dma[MUSB_RX_DMA_CHN].Addr) > pPdd->paDmaRx0Buffer)
                {
...
)
{
    ...
    //Clear the device attached state as device is de-attached from USB bus
    pPdd->devState &= ~MUSB_DEVSTAT_ATT;
...
)
{
    ...
    wcscpy_s(szChargeEventName,_countof(szChargeEventName),TEXT(""));
    dwType = REG_SZ;
