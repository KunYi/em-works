...
DeviceInitialize()
{
    ...
    for (DevIndex = 0; DevIndex < OMAP_DEVICE_COUNT - 1; ++DevIndex)
        {
        pPrcmCm = GetCmRegisterSet(s_rgDeviceLookupTable[DevIndex].powerDomain);

        // update autoidle information
        if (s_rgDeviceLookupTable[DevIndex].pautoidle != NULL)
            {
            preg = (volatile unsigned int*)((UCHAR*)pPrcmCm + s_rgDeviceLookupTable[DevIndex].pautoidle->offset);

            if (INREG32(preg) & s_rgDeviceLookupTable[DevIndex].pautoidle->mask)
                {
                PrcmDeviceEnableAutoIdle(DevIndex, TRUE);
                }            
            }
        
        // update functional clock information
        if (s_rgDeviceLookupTable[DevIndex].pfclk != NULL)
            {
            preg = (volatile unsigned int*)((UCHAR*)pPrcmCm + s_rgDeviceLookupTable[DevIndex].pfclk->offset);

            if (INREG32(preg) & s_rgDeviceLookupTable[DevIndex].pfclk->mask)
                {
                PrcmDeviceEnableFClock(DevIndex, TRUE);               
                }            
            }

        // update inteface clock information
        if (s_rgDeviceLookupTable[DevIndex].piclk != NULL)
            {
            preg = (volatile unsigned int*)((UCHAR*)pPrcmCm + s_rgDeviceLookupTable[DevIndex].piclk->offset);

            if (INREG32(preg) & s_rgDeviceLookupTable[DevIndex].piclk->mask)
                {
                PrcmDeviceEnableIClock(DevIndex, TRUE);
                }            
            }
        }
