    ...
    )
{
    ...
    if (!TWLSetIntrEvent(pDevice->hTWL, TWL_INTR_RTC_IT, pDevice->hIntrEvent))
        {
        DEBUGMSG(ZONE_ERROR, (L"ERROR: RTC_Init: "
    ...
    // Enable RTC event
    if (!TWLIntrEnable(pDevice->hTWL, TWL_INTR_RTC_IT))
        {
        DEBUGMSG(ZONE_ERROR, (L"ERROR: RTC_Init: "
    ...
    )
{
    ...
    if (pDevice->hTWL != NULL)
        {
        TWLIntrDisable(pDevice->hTWL, TWL_INTR_RTC_IT);
        if (pDevice->hIntrEvent != NULL)
            {
            TWLSetIntrEvent(pDevice->hTWL, TWL_INTR_RTC_IT, NULL);
            }
