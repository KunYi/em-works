...
;
        ...
        ; |   |   |   |       |   |                  |               |
        ; ------------------------------------------------------------
        ;                  ARM1136  Cache Type Register Format
        ;
        ;
        ; ISize    11  10  9   8    6 5     3  2  1     0
        ;         ----------------------------------------
        ;         |   |   |   |      |       |   |       |
        ;         | P | 0 | 0 | Size | Assoc | M |  Len  |               
        ;         |   |   |   |      |       |   |       |
        ;         |   |   |   |      |       |   |       |
        ;         ----------------------------------------
        ; DSize    23   22  21 20  18 17   15 14  13   12
        ;
        ;                  DSize and ISize field Format
        ;
        ;--------------------------------------------------------------
        LEAF_ENTRY OALGetL1CacheInfo
...
20      str     r2, [r1, #L1INumWays]
        ...
        ; Get I sets as (1 << (b[8..6] + 6 - b[5..3] - b[1..0]))
        mov     r3, r0, lsr #6
        and     r3, r3, #7
        add     r2, r3, #6
        mov     r3, r0, lsr #3
...
40      str     r2, [r1, #L1DNumWays]
        ...
        ; Get D size as ((2 + b[14]) << (b[20..18] + 8))
        mov     r3, r0, lsr #14
        and     r3, r3, #1
        add     r2, r3, #2
        mov     r3, r0, lsr #18
        ...
        ; Get # sets as (1 << (b[20..18] + 6 - b[17..15] - b[13..12]))
        mov     r3, r0, lsr #18
        and     r3, r3, #7
        add     r2, r3, #6
        mov     r3, r0, lsr #15
