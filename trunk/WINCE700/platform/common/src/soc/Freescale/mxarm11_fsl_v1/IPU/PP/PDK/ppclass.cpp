...
void PpClass::PpTaskParamConfig(pPpCSCCoeffs pCSCCoeffs)
{
    ...
    writeICTaskParam(0x87c, 0, pCSCCoeffs->C22 | (pCSCCoeffs->C11 << 9) |
        (pCSCCoeffs->C00 <<18) | (pCSCCoeffs->A0 <<27));
    writeICTaskParam(0x87c, 1, (pCSCCoeffs->A0 >> 5) | (pCSCCoeffs->Scale<<8) | (0 << 10));    //Scale=2, sat=0

    writeICTaskParam(0x87d, 0, pCSCCoeffs->C20 | (pCSCCoeffs->C10 << 9) |
        (pCSCCoeffs->C01 <<18) | (pCSCCoeffs->A1 <<27));
    writeICTaskParam(0x87d, 1, (pCSCCoeffs->A1 >> 5));

    writeICTaskParam(0x87e, 0, pCSCCoeffs->C21 | (pCSCCoeffs->C12 << 9) |
        (pCSCCoeffs->C02 <<18) | (pCSCCoeffs->A2 <<27));
    writeICTaskParam(0x87e, 1, (pCSCCoeffs->A2 >> 5));
...
BOOL PpClass::PpGetResizeCoeffs(UINT16 inSize, UINT16 outSize, pPpResizeCoeffs resizeCoeffs)
{
    ...
    //tempOutSize must be smaller than outSize x 2, the rest part can be handled by resize module.
