...
BOOL PrpClass::PrpInit(void)
{
    ...
    m_hEncBufWaitList[prpBufTerminateEvent] = CreateEvent(NULL, FALSE, FALSE, NULL);
    if(!m_hEncBufWaitList[prpBufTerminateEvent])
    {
        DEBUGMSG(ZONE_ERROR,
            (TEXT("%s: CreateEvent failed for Terminate Encoding Buffer Thread\r\n"), __WFUNCTION__));
        return FALSE;
    }
    ...
    m_hEncRotBufWaitList[prpBufTerminateEvent] = CreateEvent(NULL, FALSE, FALSE, NULL);
    if(!m_hEncRotBufWaitList[prpBufTerminateEvent])
    {
        DEBUGMSG(ZONE_ERROR,
            (TEXT("%s: CreateEvent failed for Terminate Encoding Rotation Buffer Thread\r\n"), __WFUNCTION__));
        return FALSE;
    }
    ...
    m_hVfBufWaitList[prpBufTerminateEvent] = CreateEvent(NULL, FALSE, FALSE, NULL);
    if(!m_hVfBufWaitList[prpBufTerminateEvent])
    {
        DEBUGMSG(ZONE_ERROR,
            (TEXT("%s: CreateEvent failed for Terminating Viewfinding Buffer Thread\r\n"), __WFUNCTION__));
        return FALSE;
    }
    ...
    m_hVfRotBufWaitList[prpBufTerminateEvent] = CreateEvent(NULL, FALSE, FALSE, NULL);
    if(!m_hVfRotBufWaitList[prpBufTerminateEvent])
    {
        DEBUGMSG(ZONE_ERROR,
            (TEXT("%s: CreateEvent failed for Terminating Viewfinding Rotation Buffer Thread\r\n"), __WFUNCTION__));
        return FALSE;
    }
    ...
    //Thread Stop events
    HANDLE m_hStopPrpISRThread = NULL;
    HANDLE m_hStopPrpEncBufThread = NULL;
    HANDLE m_hStopPrpVfBufThread = NULL;
    HANDLE m_hStopPrpEncRotBufThread = NULL;
    HANDLE m_hStopPrpVfRotBufThread = NULL;

    //Thread stop status 
    BOOL m_bPrpStopISRThread = FALSE;
    BOOL m_bPrpStopEncBufThread = FALSE;
    BOOL m_bPrpStopVfBufThread = FALSE;
    BOOL m_bPrpStopEncRotBufThread = FALSE;
    BOOL m_bPrpStopVfRotBufThread = FALSE;
...
void PrpClass::PrpDeinit(void)
{
    ...
    if (m_hPrpISRThread)
    {
        m_bPrpStopISRThread = TRUE;
        SetEvent(m_hPrpIntrEvent);
        if((WaitForSingleObject(m_hPrpISRThread, 5000)) == WAIT_TIMEOUT)
        {
          DEBUGMSG(ZONE_ERROR,  (TEXT("%s: PRP ISR thread Terminate Failed! \r\n"), __WFUNCTION__));
          ASSERT(FALSE);
        }
    ...
    {
        m_bPrpStopEncBufThread = TRUE;
        SetEvent(m_hEncBufWaitList[prpBufTerminateEvent]);
        if((WaitForSingleObject(m_hPrpEncBufThread, 5000)) == WAIT_TIMEOUT)
        {
          DEBUGMSG(ZONE_ERROR,  (TEXT("%s: ENC Buffer thread Terminate Failed! \r\n"), __WFUNCTION__));
          ASSERT(FALSE);
        }
    ...
    {
        m_bPrpStopEncRotBufThread = TRUE;
        SetEvent(m_hEncRotBufWaitList[prpBufTerminateEvent]);
        if((WaitForSingleObject(m_hPrpEncRotBufThread, 5000)) == WAIT_TIMEOUT)
        {
          DEBUGMSG(ZONE_ERROR,  (TEXT("%s: ENC Rot Buffer thread Terminate Failed! \r\n"), __WFUNCTION__));
          ASSERT(FALSE);
        }
    ...
    {
        m_bPrpStopVfBufThread = TRUE;
        SetEvent(m_hVfBufWaitList[prpBufTerminateEvent]);
        if((WaitForSingleObject(m_hPrpVfBufThread, 5000)) == WAIT_TIMEOUT)
        {
          DEBUGMSG(ZONE_ERROR,  (TEXT("%s: VF Buffer thread Terminate Failed! \r\n"), __WFUNCTION__));
          ASSERT(FALSE);
        }
    ...
    {
        m_bPrpStopVfRotBufThread = TRUE;
        SetEvent(m_hVfRotBufWaitList[prpBufTerminateEvent]);
        if((WaitForSingleObject(m_hPrpVfRotBufThread, 5000)) == WAIT_TIMEOUT)
        {
          DEBUGMSG(ZONE_ERROR,  (TEXT("%s: VF Rot Buffer thread Terminate Failed! \r\n"), __WFUNCTION__));
          ASSERT(FALSE);
        }
...
void PrpClass::PrpTaskParamConfig(prpCSCTaskParam taskParamMemory, pPrpCSCCoeffs pCSCCoeffs)
{
    ...
    switch (taskParamMemory)
    {
        case prpCSCEncMatrix1:
            writeICTaskParam(0x2D1, 0, pCSCCoeffs->C22 | (pCSCCoeffs->C11 << 9) |
                (pCSCCoeffs->C00 <<18) | (pCSCCoeffs->A0 <<27));
            writeICTaskParam(0x2D1, 1, (pCSCCoeffs->A0 >> 5) | (pCSCCoeffs->Scale<<8) | (0 << 10));    //Scale=2, sat=0

            writeICTaskParam(0x2D2, 0, pCSCCoeffs->C20 | (pCSCCoeffs->C10 << 9) |
                (pCSCCoeffs->C01 <<18) | (pCSCCoeffs->A1 <<27));
            writeICTaskParam(0x2D2, 1, (pCSCCoeffs->A1 >> 5));

            writeICTaskParam(0x2D3, 0, pCSCCoeffs->C21 | (pCSCCoeffs->C12 << 9) |
                (pCSCCoeffs->C02 <<18) | (pCSCCoeffs->A2 <<27));
            writeICTaskParam(0x2D3, 1, (pCSCCoeffs->A2 >> 5));
            break;

        case prpCSCVfMatrix1:
            writeICTaskParam(0x5a5, 0, pCSCCoeffs->C22 | (pCSCCoeffs->C11 << 9) |
                (pCSCCoeffs->C00 <<18) | (pCSCCoeffs->A0 <<27));
            writeICTaskParam(0x5a5, 1, (pCSCCoeffs->A0 >> 5) | (pCSCCoeffs->Scale<<8) | (0 << 10));    //Scale=2, sat=0

            writeICTaskParam(0x5a6, 0, pCSCCoeffs->C20 | (pCSCCoeffs->C10 << 9) |
                (pCSCCoeffs->C01 <<18) | (pCSCCoeffs->A1 <<27));
            writeICTaskParam(0x5a6, 1, (pCSCCoeffs->A1 >> 5));

            writeICTaskParam(0x5a7, 0, pCSCCoeffs->C21 | (pCSCCoeffs->C12 << 9) |
                (pCSCCoeffs->C02 <<18) | (pCSCCoeffs->A2 <<27));
            writeICTaskParam(0x5a7, 1, (pCSCCoeffs->A2 >> 5));
            break;
...
//
//---------------------------------------------------------------------
DWORD PrpClass::PrpIntrThread(LPVOID lpParameter)
{
...
//
//---------------------------------------------------------------------
DWORD PrpClass::PrpEncBufferWorkerThread(LPVOID lpParameter)
{
    PrpClass *pPrp = (PrpClass *)lpParameter;
...
void PrpClass::PrpEncBufferWorkerRoutine(UINT32 timeout)
{
    ...
    while (1)
    {
        ...
        result = WaitForMultipleObjects(_countof(m_hEncBufWaitList), m_hEncBufWaitList, FALSE, timeout);

        //check for terminate event
        if(((WAIT_OBJECT_0 + prpBufTerminateEvent) == result) && (m_bPrpStopEncBufThread))
        {
            return;
        }
...
//
//---------------------------------------------------------------------
DWORD PrpClass::PrpEncRotBufferWorkerThread(LPVOID lpParameter)
{
    PrpClass *pPrp = (PrpClass *)lpParameter;
...
void PrpClass::PrpEncRotBufferWorkerRoutine(UINT32 timeout)
{
    ...
    while (1)
    {
        // Wait until the ISR loop receives a filled buffer, at which
        // time it will signal that it needs a new buffer.
        result = WaitForMultipleObjects(_countof(m_hEncRotBufWaitList), m_hEncRotBufWaitList, FALSE, timeout);

        if(((WAIT_OBJECT_0 + prpBufTerminateEvent) == result) && (m_bPrpStopEncRotBufThread))
        {
            return;
        }
        ...
        DEBUGMSG(ZONE_ERROR, (_T("%s: Enc rotation buffer ready\r\n"), __WFUNCTION__));
...
//
//---------------------------------------------------------------------
DWORD PrpClass::PrpVfBufferWorkerThread(LPVOID lpParameter)
{
    PrpClass *pPrp = (PrpClass *)lpParameter;
...
void PrpClass::PrpVfBufferWorkerRoutine(UINT32 timeout)
{
    ...
    while (1)
    {
        // Wait until the ISR loop receives a filled buffer, at which
        // time it will signal that it needs a new buffer.
        result = WaitForMultipleObjects(_countof(m_hVfBufWaitList), m_hVfBufWaitList, FALSE, timeout);

        if(((WAIT_OBJECT_0 + prpBufTerminateEvent) == result) && (m_bPrpStopVfBufThread))
        {
            return;
        }
...
//
//---------------------------------------------------------------------
DWORD PrpClass::PrpVfRotBufferWorkerThread(LPVOID lpParameter)
{
    PrpClass *pPrp = (PrpClass *)lpParameter;
...
void PrpClass::PrpVfRotBufferWorkerRoutine(UINT32 timeout)
{
    ...
    while (1)
    {
        // Wait until the ISR loop receives a filled buffer, at which
        // time it will signal that it needs a new buffer.
        result = WaitForMultipleObjects(_countof(m_hVfRotBufWaitList), m_hVfRotBufWaitList, FALSE, timeout);

        if(((WAIT_OBJECT_0 + prpBufTerminateEvent) == result) && (m_bPrpStopVfRotBufThread))
        {
            return;
        }
