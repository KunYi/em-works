...
//        "Ata!CDisk::SendIOCommand+> sector(%d), sectors left(%x), command(%x), Status: %x\r\n"
//        ), dwStartSector,dwNumberOfSectors,bCmd, GetAltStatus()));

    if (ZONE_CELOG) CeLogData(TRUE, CELID_ATA_IOCOMMAND, &bCmd, sizeof(bCmd), 0, CELZONE_ALWAYSON, 0, FALSE);

    SelectDevice();
      while(GetAltStatus() & ATA_STATUS_BUSY);

    if (WaitOnBusy(FALSE)) {
        DEBUGMSG(ZONE_IO, (TEXT(
            "Ata!CDisk::SendIOCommand> Failed to send command; status(%x), error(%x)\r\n"
...
//    DEBUGMSG(ZONE_IO, (TEXT(
//        "Ata!CDisk::SendIOCommand-> Status: %x\r\n"), GetAltStatus()));
    return (TRUE);
}
    ...
    for (;;)  {
        ...
        switch (bStatusType) {
            case WAIT_TYPE_BUSY:
                if (bStatusRead & ATA_STATUS_BUSY) {
                    //DEBUGMSG(ZONE_IO, (TEXT("Ata!CDisk::WaitForDisc> WAIT_TYPE_BUSY\r\n")));
                    goto ExitDone;
                }
                break;
            case WAIT_TYPE_NOT_BUSY:
                if (!(bStatusRead & ATA_STATUS_BUSY)) {
                    //DEBUGMSG(ZONE_IO, (TEXT("Ata!CDisk::WaitForDisc> WAIT_TYPE_NOT_BUSY\r\n")));
                    goto ExitDone;
                }
                break;
            case WAIT_TYPE_READY:
                if (bStatusRead & ATA_STATUS_READY) {
                    //DEBUGMSG(ZONE_IO, (TEXT("Ata!CDisk::WaitForDisc> WAIT_TYPE_READY\r\n")));
                    StallExecution(100);
                    goto ExitDone;
                }
                break;
            case WAIT_TYPE_DRQ:
                if (bStatusRead & ATA_STATUS_DATA_REQ) {
                    //DEBUGMSG(ZONE_IO, (TEXT("Ata!CDisk::WaitForDisc> WAIT_TYPE_DRQ\r\n")));
                    goto ExitDone;
                }
                break;
            case WAIT_TYPE_NOT_DRQ:
                if (!(bStatusRead & ATA_STATUS_DATA_REQ)) {
                    //DEBUGMSG(ZONE_IO, (TEXT("Ata!CDisk::WaitForDisc> WAIT_TYPE_NOT_DRQ\r\n")));
                    goto ExitDone;
                }
                break;
            case WAIT_TYPE_ERROR:
                if (bStatusRead & ATA_STATUS_ERROR) {
                    //DEBUGMSG(ZONE_IO, (TEXT("Ata!CDisk::WaitForDisc> WAIT_TYPE_ERROR\r\n")));
                    goto ExitDone;
                }
                break;
        }
        if ((int)dwTimeOut > 0) {
            ...
            DEBUGMSG(ZONE_ERROR, (TEXT(
                "Ata!CDisk::WaitForDisc> timeout; bStatusType(%d); status(%02X)\r\n"
    ...
    )
{
    ...
    if (ZONE_CELOG) CeLogData(TRUE, CELID_ATA_WAITDRQ, NULL, 0, 0, CELZONE_ALWAYSON, 0, FALSE);
    ...
    for (i = 0; i < m_dwWaitCheckIter; i++) {
        for (j = 0; j < m_dwWaitSampleTimes; j++) {
            ...
            if ((bStatus & ATA_STATUS_BUSY) == 0) {
                if (bStatus & ATA_STATUS_DATA_REQ){
                    if (ZONE_CELOG) CeLogData(TRUE, CELID_ATA_STATUSWAITDRQ, &bStatus, sizeof(bStatus), 0, CELZONE_ALWAYSON, 0, FALSE);
                    return TRUE;
                }
            }
            StallExecution(m_dwWaitStallTime);
        ...
        DEBUGMSG(ZONE_WARNING, (TEXT(
            "Ata!CDisk::WaitForDRQ> status(%02X), error(%02X), reason(%02X)\r\n"
    ...
    )
{
    ...
    if (ZONE_CELOG) CeLogData(TRUE, CELID_ATA_STARTREADBUFFER, &dwCount, sizeof(dwCount), 0, CELZONE_ALWAYSON, 0, FALSE);
    ...
    if (m_wNextByte != 0xFFFF) {
        DEBUGMSG(ZONE_WARNING, (TEXT("Ata!CDisk::ReadBuffer> Unaligned buffer on prevous read\r\n")));
        // update first byte
    ...
    if ((DWORD)pBuffer & 1) {
        DEBUGMSG(ZONE_WARNING, (TEXT("Ata!CDisk::ReadBuffer> Unaligned buffer\r\n")));
        while (dwCount> 1) {
            unisc.us = ReadWord();
            *pBuffer++= unisc.uc[0];
            *pBuffer++= unisc.uc[1];
    ...
    if (dwCount == 1) {
        DEBUGMSG(ZONE_WARNING, (TEXT("Ata!CDisk::ReadBuffer> Reading word for one byte\r\n")));
        unisc.us = ReadWord();
        *pBuffer=   unisc.uc[0];
    ...
    if (ZONE_CELOG) CeLogData(TRUE, CELID_ATA_COMPLETEREADBUFFER, &bStatus, sizeof(bStatus), 0, CELZONE_ALWAYSON, 0, FALSE);
}
    ...
    )
{
    ...
    if (ZONE_CELOG) CeLogData(TRUE, CELID_ATA_STARTWRITEBUFFER, &dwCount, sizeof(dwCount), 0, CELZONE_ALWAYSON, 0, FALSE);
    ...
    if (m_wNextByte != 0xFFFF) {
        // update first byte
        DEBUGMSG(ZONE_WARNING, (TEXT("Ata!CDisk::WriteBuffer> Unaligned buffer on previous write\r\n")));
    ...
    if ((DWORD) pBuffer & 1) {
        DEBUGMSG(ZONE_WARNING, (TEXT("Ata!CDisk::WriteBuffer> Unaligned buffer\r\n")));
        while (dwCount> 1) {
            unisc.uc[0] = *pBuffer++;
            unisc.uc[1] = *pBuffer++;
            WriteWord(unisc.us);
    ...
    if (dwCount == 1) {
        DEBUGMSG( ZONE_WARNING, (TEXT("Ata!CDisk::WriteBuffer> Writing one word for one byte\r\n")));
        m_wNextByte = (WORD) *pBuffer; // save the second byte
    }

    BYTE bStatus = GetAltStatus();

    if (ZONE_CELOG) CeLogData(TRUE, CELID_ATA_COMPLETEWRITEBUFFER, &bStatus, sizeof(bStatus), 0, CELZONE_ALWAYSON, 0, FALSE);
}
    ...
    for (;;) { // BSY := Bit 7
        ...
        if (bError & 0x04) { // ABRT := Bit 2
            ...
            DEBUGMSG(ZONE_ERROR, (_T(
                "Ata!CDisk::SetTransferMode> Failed to send SET FEATURES command, parameter 0x%x\r\n"
                ), bMode));
            fOk = FALSE;
            break;
        }
