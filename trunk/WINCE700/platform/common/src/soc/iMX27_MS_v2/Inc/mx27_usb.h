...
typedef struct _USB_USBSTS{
    ...
    unsigned int UEI:1;     //USB Error Interrupt (USBERRINT) 〞 R/WC.
                            ...
                            // See Section (Reference Host Operation Model: Transfer/Transaction Based Interrupt 每 i.e. 4.15.1 in EHCI Enhanced Host Controller Interface Specification
    ...
    unsigned int AAI:1;     //Interrupt on Async Advance 〞 R/WC. 0=Default.
                            ...
                            //Only used by the host controller.
    unsigned int URI:1;     // USB Reset Received 每 R/WC. 0=Default.
    ...
    unsigned int SRI:1;     //SOF Received 每 R/WC. 0=Default.
    ...
    unsigned int SLI:1;         //DCSuspend 每 R/WC. 0=Default.
                            ...
                            // Only used by the device controller.
    unsigned int R1:1;          //Reserved. These bits are reserved and should be set to zero.
    unsigned int ULPII:1;       //ULPI Interrupt 每 R/WC. 0=Default.
...
typedef struct _USB_ULPI_VIEWPORT { //The register provides indirect access to the ULPI PHY register set. Although the core performs access to
    ...
    unsigned int ULPIPORT:3;    // ULPI Port Number 每 Read/Write. For the wakeup or read/write operation to be executed, this value selects the port number the ULPI PHY is
    ...
    unsigned int ULPISS:1;      //ULPI Sync State 每 Read Only.
                            // (1) 每 Normal Sync. State. (0) In another state (ie. carkit, serial, low power) This bit represents the state of the ULPI
                            ...
                            // always remain 0.
    unsigned int R:1;           //Reserved. These bits are reserved and should be set to zero.
    unsigned int ULPIRW:1;  // ULPI Read/Write Control 每 Read/Write.
                            // (0) 每 Read; (1) 每 Write. This bit selects between running a read or write operation.
    unsigned int ULPIRUN:1; // ULPI Read/Write Run 每 Read/Write.
    ...
    unsigned int ULPIWU:1;  //ULPI Wakeup 每 Read/Write.
...
typedef struct _USB_PORTSC {
    ...
    unsigned int PHCD:1;    // PHY Low Power Suspend - Clock Disable (PLPSCD) 每 Read/Write. Default= 0b.
                        ...
                        // cannot be disabled if it is being used as the system clock. In device mode, The PHY can be put into Low Power Suspend 每 Clock Disable
    ...
    unsigned int PFSC:1;    //Port Force Full Speed Connect 每 Read/Write. Default = 0b.
    ...
    unsigned int PSPD:2;    //Port Speed 每 Read Only.
                        ...
                        // 10 每 High Speed
                        //This bit is not defined in the EHCI specification.
    unsigned int PTW:1; //Parallel Transceiver Width 每 Read/Write.
    ...
    unsigned int PTS:2; //Parallel Transceiver Select 每 Read/Write.
                        ...
                        // 00 if VUSB_HS_PHY_TYPE = 0,4 每 UTMI/UTMI+
                        // 01 if VUSB_HS_PHY_TYPE = 1,5 每 Philips Classic
                        // 10 if VUSB_HS_PHY_TYPE = 2,6 每 ULPI
                        // 11 if VUSB_HS_PHY_TYPE = 3,7 每 Serial/1.1 PHY (FS Only)
}USB_PORTSC_T;


typedef struct _USB_OTGSC {
    unsigned int VD:1;      //VBUS_Discharge 每 Read/Write. Setting this bit causes VBus to discharge through a resistor.
    unsigned int VC:1;      //VBUS Charge 每 Read/Write. Setting this bit causes the VBus line to be charged. This is used for VBus pulsing during SRP.
    unsigned int R1:1;      //Reserved. These bits are reserved and should be set to zero.
    unsigned int OT:1;      //OTG Termination 每 Read/Write.This bit must be set when the OTG device is in device mode, this controls the pulldown on DM.
    unsigned int DP:1;      //Data Pulsing 每 Read/Write.Setting this bit causes the pullup on DP to be asserted for data pulsing during SRP.
    unsigned int IDPU:1;    //ID Pullup 每 Read/Write This bit provide control over the ID pull-up resister ; 0 = off, 1 = on [default]. When this bit is 0, the ID input will not be sampled.
    unsigned int R2:2;      //Reserved. These bits are reserved and should be set to zero.

    unsigned int ID:1;      //USB ID 每 Read Only. 0 = A device, 1 = B device
    unsigned int AVV:1; //A VBus Valid 每 Read Only. Indicates VBus is above the A VBus valid threshold.
    unsigned int ASV:1; //A Session Valid 每 Read Only. Indicates VBus is above the A session valid threshold.
    unsigned int BSV:1; //B Session Valid 每 Read Only. Indicates VBus is above the B session valid threshold.
    unsigned int BSE:1; //B Session End 每 Read Only. Indicates VBus is below the B session end threshold.
    unsigned int _1MST:1;   // 1 milisecond timer toggle - Read Only. This bit toggles once per millisecond.
    unsigned int DPS:1; //Data Bus Pulsing Status 每 Read Only. A '1' indicates data bus pulsing is being detected on the port.
    unsigned int R3:1;      //Reserved. These bits are reserved and should be set to zero.

    unsigned int IDIS:1;    // USB ID Interrupt Status 每 Read/Write. This bit is set when a change on the ID input has been detected. Software must write a one to clear this bit.
    unsigned int AVVIS:1;   // A VBus Valid Interrupt Status 每 Read/Write to Clear. This bit is set when VBus has either risen above or fallen below the VBus valid threshold (4.4 VDC) on an A device. Software must write a one to clear this bit.
    unsigned int ASVIS:1;   // A Session Valid Interrupt Status 每 Read/Write to Clear. This bit is set when VBus has either risen above or fallen below the A session valid threshold (0.8 VDC). Software must write a one to clear this bit.
    unsigned int BSVIS:1;   // B Session Valid Interrupt Status 每 Read/Write to Clear. This bit is set when VBus has either risen above or fallen below the B session valid threshold (0.8 VDC). Software must write a one to clear this bit.
    unsigned int BSEIS:1;   // B Session End Interrupt Status 每 Read/Write to Clear.This bit is set when VBus has fallen below the B session end threshold.Software must write a one to clear this bit
    unsigned int _1MSS:1;   // 1 milisecond timer Interrupt Status 每 Read/Write to Clear.This bit is set once every millisecond. Software must write a one to clear this bit.
    unsigned int DPIS:1;    //Data Pulse Interrupt Status 每 Read/Write to Clear.
    ...
    unsigned int IDIE:1;    //USB ID Interrupt Enable 每 Read/Write.Setting this bit enables the USB ID interrupt.
    unsigned int AVVIE:1;   // A VBus Valid Interrupt Enable 每 Read/Write.Setting this bit enables the A VBus valid interrupt.
    unsigned int ASVIE:1;   //A Session Valid Interrupt Enable 每 Read/Write. Setting this bit enables the A session valid interrupt.
    unsigned int BSVIE:1;   // B Session Valid Interrupt Enable 每 Read/Write. Setting this bit enables the B session valid interrupt.
    unsigned int BSEIE:1;   //B Session End Interrupt Enable 每 Read/Write. Setting this bit enables the B session end interrupt.
    unsigned int _1MSE:1;   // 1 milisecond timer Interrupt Enable 每 Read/Write
...
typedef struct _USB_USBMODE {
    unsigned int CM:2;      //Controller Mode 每 R/WO. Controller mode is defaulted to the proper mode for
...
typedef struct _USB_CTRL {
    unsigned int BPE:1;     // Bypass Enable
                            ...
                            // 0 = Bypass inactive -- Normal mode operation.
    unsigned int R1:3;          //Reserved. These bits are reserved and should be set to zero.
    unsigned int H1DT:1;        //Host 1 TLL Disable
                            // This bit controls whether or not the Transceiverless Link Logic is enabled for the serial interface of Host Port 1.
                            // 1 = TLL is disabled
                            // 0 = TLL is enabled
    unsigned int H2DT:1;        //Host 2 TLL Disable
                            // This bit controls whether or not the Transceiverless Link Logic is enabled for the serial interface of Host Port 2.
                            // 1 = TLL is disabled
                            // 0 = TLL is enabled
    unsigned int R2:2;          //Reserved. These bits are reserved and should be set to zero.
    ...
    unsigned int H2PM:1;        //Host 2 Power Mask
                            ...
                            // 0 = The USBPWR pin will assert with the Host 2 core.s Vbus power Enable and the assertion of the OC input will be reported to the Host 2 core.
    unsigned int R4:2;          //Reserved. These bits are reserved and should be set to zero.
    unsigned int H2WIE:1;       //Host 2 Wake-up Interrupt Enable
                            // This bit enables or disables the Host 2 wake-up interrupt. Disabling the interrupt also clears the Interrupt request bit. Wake-up interrupt enable should be turned off after receiving a
                            // wake-up interrupt and turned on again prior to going in suspend mode
                            // 1 = Interrupt Enabled
                            // 0 = Interrupt Disabled
    ...
    unsigned int H2SIC:2;       // Host 2 Serial Interface Configuration
                            // Controls the interface type of the Host 2 port when used with a serial transceiver. This bit field allows for configuring the serial interface for Single Ended or Differential operation combined
                            // with Bidirectional or Unidirectional operation.
                            // 0 0 Differential / Unidirectional (6-wire)
                            // 0 1 Differential / Bidirectional (4-wire)
                            // 1 0 Single Ended / Unidirectional (6-wire)
    ...
    unsigned int H2WIR:1;       //Host 2 Wake-up Interrupt Request(RO)
                            // Indicates a pending Wake-up request on Host port 2. This bit is cleared by disabling the interrupt. The interrupt must be disabled for at least 2 clock cycles of the standby clock.
                            // 1 = Wake-up interrupt received
                            // 0 = No Wake-up interrupt received
