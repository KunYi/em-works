...
PMIC_STATUS PmicADCSetComparatorThresholds(UINT16 whigh, UINT16 wlow)
{
    ...
    if (whigh <= wlow)
    {
        ERRORMSG(1, (_T("PmicIoctlADCSetComparatorThresholds Invalid Parameter!\r\n")));
...
PMIC_STATUS PmicADCGetSingleChannelOneSample(UINT16 channels, UINT16* pResult)
{
    ...
    if (!DeviceIoControl(hPMI, PMIC_IOCTL_ADC_GET_SGL_CH_1SPL, &channels,
                sizeof(channels), pResult, sizeof(pResult), NULL, NULL))
    {
                                     ...
                                     UINT16* pResult)
{
    ...
    if (!DeviceIoControl(hPMI, PMIC_IOCTL_ADC_GET_HS_CURRENT, &mode,
                sizeof(mode), pResult, sizeof(pResult), NULL, NULL))
    {
...
//------------------------------------------------------------------------------
//
//  Function: PmicADCTouchPowerMode
//
//  This function implements Touch Power Mode 
//
//  Parameters:
...
//
//------------------------------------------------------------------------------
void PmicADCTouchPowerMode(bool bOff)
{
    PMIC_TOUCH_POWER_MODE mode;
    ...
    if (!DeviceIoControl(hPMI, PMIC_IOCTL_ADC_POWER_MODE, &mode, sizeof(mode),
          NULL, 0, NULL, NULL))
    {
        ERRORMSG(TRUE, (_T("PMIC_IOCTL_ADC_POWER_MODE failed!\r\n")));
    }
