...
static HANDLE hCSPI2;
CSPI_BUSCONFIG_T PMICCspiBusConfig =
{
    CSPI_CONTROLREG_CS_SS0,
    8000000, //BSP_PMIC_CSPI_FREQ, query BSP function later
    CSPI_CONTROLREG_BITCOUNT_32BIT,
    CSPI_CONTROLREG_SSPOL_ACTIVE_HIGH,
    CSPI_CONTROLREG_SSCTL_PULSE,
    CSPI_CONTROLREG_POL_ACTIVE_HIGH,
    CSPI_CONTROLREG_PHA0,
    CSPI_CONTROLREG_DRCTL_DONTCARE
};
...
static VOID PackTxData(UINT32& content ,UINT32 addr, UINT32 data, UINT32 mask);
static VOID PackRxData(UINT32& content ,UINT32 addr);
static VOID ExchangeData(UINT32* TxContents,UINT32* RxContents, UINT32 numPackets);
//Touch Panel power mode implementation.
static VOID PMICAdcSetPowerMode(PMIC_TOUCH_POWER_MODE mode);
static BOOL PMICIoctlAdcPowerSetMode(PBYTE pBufIn, DWORD dwLenIn, PBYTE pBufOut,
                         DWORD dwLenOut);
...
InitializePMIC()
{
    ...
    // Initialize CSPI
    // Get handle to the CSPI2 bus for writing to the display panel control
    hCSPI2 = CreateFile (L"SPI2:",                    // Pointer to the name of the port
    ...
    if(hCSPI2 == NULL || hCSPI2 == INVALID_HANDLE_VALUE){
        DEBUGMSG(ZONE_ERROR, (_T("CreateFile:  CreateFile failed for CSPI2! \r\n")));
        return FALSE;
    }

    // Get the real PMIC CSPI frequency
    PMICCspiBusConfig.freq = BSPPmicGetSpiFreqOut();
    ...
    *result = PMIC_ERROR;
    ...
    if (inParam->op == OP_RS232_SET_CFG)
    {
        ...
        data1 = CSP_BITFVAL(MC13783_CHG_USB1_RSTRI,
                        (inParam->PARAMS.RS232_CFG.txTristated == TRUE) ? 1: 0);
                                                          ...
                                                          PMIC_STATUS *result)
{
    ...
    *result = PMIC_ERROR;
    ...
    if (inParam->PARAMS.USB_SPEED.usbMode == USB_HOST)
    {
        ...
        data1 |= CSP_BITFVAL(MC13783_CHG_USB1_VBUSEN, MC13783_CHG_USB1_OP_ENABLE);
    }
    else
    {
...
// Function: PackTxData
//
// This function packs the tx contents in the CSPI Exchange format
//
// Parameters:
//      content
//          [in/out] Data after packing.
//
//      addr
//          [in] Address portion of the data.
//
//      data
//          [in] Data to be transimitted.
//
//      mask
//          [in] Mask for the data.
//
// Returns:
//      NONE
//
...
static VOID PackTxData(UINT32& content ,UINT32 addr, UINT32 data, UINT32 mask)
{
    //bit31=1 wirte flag, bit30-25 address, bit24 dead bit, bit23-0 register content.
    content &= ~mask;
    content |= data & mask;
    content = (1<<31) | ((addr & 0x3F)<<25) | (content & 0xFFFFFF);
}
...
// Function: PackRxData
//
// This function packs the rx contents in the CSPI Exchange format
//
// Parameters:
//      content
//          [out] Data after packing.
//
//      addr
//          [in] Address portion of the data.
//
...
static VOID PackRxData(UINT32& content ,UINT32 addr)
{
    //b31=0 read flag, b30-25 address, b24 dead bit, bit23-0 ignored for read
    content = (addr & 0x3F)<<25;
}
...
//
// This function Transmits the data using the CSPI driver
//
// Parameters:
//      TxContents
//          [in] TX content
...
static VOID ExchangeData(UINT32* TxContents,UINT32* RxContents, UINT32 numPackets)
{
    CSPI_XCH_PKT_T CSPIPacket = {0};
    CSPIPacket.pBusCnfg = (PCSPI_BUSCONFIG_T)(&PMICCspiBusConfig);
    CSPIPacket.pTxBuf = TxContents;
    CSPIPacket.pRxBuf = RxContents;
    CSPIPacket.xchCnt = numPackets;
    CSPIPacket.xchEvent = NULL;  //send package immediately
    DeviceIoControl(hCSPI2,                    // File handle to the driver
                   CSPI_IOCTL_EXCHANGE,        // I/O control code
                   &CSPIPacket,         // In buffer
                      ...
                      DWORD dwLenOut)
{
    ...
    UINT32 ContentMSK0=0;
    UINT32 ContentSTAT0=0;
    UINT32 ContentADC0=0;
    UINT32 ContentADC1=0;
    UINT32 ADCMode=TM_INACTIVE;

    UINT32 temp;
    UINT32 contents[4]={0};
    UINT32 Rxcontents[4]={0};
    int ix=0;
    ...
    GetRegister(MC13783_ADC0_ADDR, &ContentADC0);
    GetRegister(MC13783_ADC0_ADDR, &ContentADC1);
    GetRegister(MC13783_INT_STAT0_ADDR, &ContentSTAT0);
    GetRegister(MC13783_INT_MSK0_ADDR, &ContentMSK0);

    //Keep the current ADC Mode for restoring
    ADCMode = CSP_BITFEXT(ContentADC0,MC13783_ADC0_TSMOD);

    // Place the ADC in touch position mode and Enable ADC0
    addr = MC13783_ADC0_ADDR;

    data = CSP_BITFVAL(MC13783_ADC0_TSMOD, TM_POSITION) |
           CSP_BITFVAL(MC13783_ADC0_ADINC1, MC13783_ADC0_ADINC_NO_INCR) |
           CSP_BITFVAL(MC13783_ADC0_ADINC2, MC13783_ADC0_ADINC_NO_INCR);

    mask = CSP_BITFMASK(MC13783_ADC0_TSMOD) |
           CSP_BITFMASK(MC13783_ADC0_ADINC1) |
           CSP_BITFMASK(MC13783_ADC0_ADINC2);
    //SetRegister(addr, data, mask);
    PackTxData(ContentADC0,addr, data, mask);
    contents[ix++] = ContentADC0;

    // Make sure ADC done interrupt is unmasked and cleared
    PackTxData(ContentSTAT0,MC13783_INT_STAT0_ADDR, (1 << PMIC_MC13783_INT_ADCDONEI), (1 << PMIC_MC13783_INT_ADCDONEI));
    contents[ix++] = ContentSTAT0;

    // Unmask the interrupt
    PackTxData(ContentMSK0,MC13783_INT_MSK0_ADDR, 0, (1 << PMIC_MC13783_INT_ADCDONEI));
    contents[ix++] = ContentMSK0;
    ...
    data = CSP_BITFVAL(MC13783_ADC1_AD_SEL, MC13783_ADC1_AD_SEL_GROUP1) |
           ...
           CSP_BITFVAL(MC13783_ADC1_ASC, MC13783_ADC1_ASC_START_ADC);
    mask = MC13783_ADC_REGISTER_MASK_ALL;

    // Start the ADC conversion
    PackTxData(ContentADC1,addr, data, mask);
    contents[ix++] = ContentADC1;
    ExchangeData(contents,&temp,ix);
    ...
    // configure for auto increment
    addr = MC13783_ADC0_ADDR;
    data = CSP_BITFVAL(MC13783_ADC0_ADINC1, MC13783_ADC0_ADINC_AUTO_INCR) |
             CSP_BITFVAL(MC13783_ADC0_ADINC2, MC13783_ADC0_ADINC_AUTO_INCR);
    ...
    PackRxData(data,addr);
    contents[0]=contents[1]=contents[2]=contents[3]=data;
    ExchangeData(contents,Rxcontents,4);

    // Read conversion values
    for ( int i = 0; i < 4; i++ )
    {
        // Read channel data
        data = Rxcontents[i] &=0xFFFFFF;    //Only low 24 bits are real content
        pData[i] = CSP_BITFEXT(data, MC13783_ADC2_ADD1);
        DEBUGMSG(ZONE_FUNC, (_T("t_data[%x] = %x\r\n"), i, pData[i]));

        pData[i+4] = CSP_BITFEXT(data, MC13783_ADC2_ADD2);
        DEBUGMSG(ZONE_FUNC, (_T("t_data[%x] = %x\r\n"), (i+4), pData[i+4]));
    }
...
cleanUp:
    ...
    // Place the ADC in back in the original mode
    addr = MC13783_ADC0_ADDR;

    data = CSP_BITFVAL(MC13783_ADC0_TSMOD, ADCMode);
    mask = CSP_BITFMASK(MC13783_ADC0_TSMOD);
                                        ...
                                        DWORD dwLenOut, BOOL bMultipleSampleFlag)
{
    ...
    else
    {
        ...
        *pData = CSP_BITFEXT(temp, MC13783_ADC2_ADD1);
        DEBUGMSG(ZONE_FUNC, (_T("data = %d\r\n"), *pData));
    }
    ...
    PBYTE pBufOut, DWORD dwLenOut, PDWORD pdwActualOut)
{
    ...
    case PMIC_IOCTL_LLA_INT_REGISTER:
        ...
        if(CheckIntIdValid(pir->int_id))
        {
            ...
            {
                if( (NULL == pir->event_name) || FAILED(CeOpenCallerBuffer(
                                    (PVOID *)&event,
                                    pir->event_name,
                                    0,
                                    ARG_I_WSTR,
                ...
                {
                    DEBUGMSG(ZONE_ERROR,
                            (_T("event_name couldnt be extracted.\r\n")));
                    result = FALSE;
                    break;
                }

                phIntrHdlr[pir->int_id] = CreateEvent(NULL, FALSE, FALSE, event);
                ...
                if (phIntrHdlr[pir->int_id] == NULL)
                {
                    ...
                    CeCloseCallerBuffer((PVOID *)event,
                                        pir->event_name,
                                        _tcslen(pir->event_name),
                                        ARG_I_WSTR);
                    break;
                }
                ...
                if (pir->int_id < PMIC_MC13783_INT_ID_OFFSET)
                    SetRegister(MC13783_INT_MSK0_ADDR, 0, (1 << pir->int_id));
                else
                    SetRegister(MC13783_INT_MSK1_ADDR, 0,
                             (1 << (pir->int_id - PMIC_MC13783_INT_ID_OFFSET)));
                LeaveCriticalSection(&intRegCs);
    ...
    case PMIC_IOCTL_ADC_POWER_MODE:
        result = PMICIoctlAdcPowerSetMode(pBufIn,dwLenIn,pBufOut,dwLenOut);
        break;
...
SetRegister(UINT32 addr, UINT32 data, UINT32 mask)
{
    ...
    content = (1<<31) | ((addr & 0x3F)<<25) | (content & 0xFFFFFF);
    CSPIPacket.pBusCnfg = (PCSPI_BUSCONFIG_T)(&PMICCspiBusConfig);
    CSPIPacket.pTxBuf = &content;
    CSPIPacket.pRxBuf = &temp;
    CSPIPacket.xchCnt = 1; // num packets to send
    CSPIPacket.xchEvent = NULL;  //send package immediately
    DeviceIoControl(hCSPI2,                    // File handle to the driver
                   CSPI_IOCTL_EXCHANGE,        // I/O control code
                   &CSPIPacket,         // In buffer
...
GetRegister(UINT32 addr, UINT32* content)
{
    ...
    CSPIPacket.pBusCnfg = (PCSPI_BUSCONFIG_T)(&PMICCspiBusConfig);
    CSPIPacket.pTxBuf = &addr;
    CSPIPacket.pRxBuf = content;
    CSPIPacket.xchCnt = 1; // num packets to send
    CSPIPacket.xchEvent = NULL; //send package immediately
    DeviceIoControl(hCSPI2,                    // File handle to the driver
                   CSPI_IOCTL_EXCHANGE,        // I/O control code
    ...
    *content &=0xFFFFFF;    //Only low 24 bits are real content
}
        ...
        if (WaitForSingleObject(hIntrEventPmic, INFINITE) == WAIT_OBJECT_0)
        {
            ...
            do
            {
                ...
                SetRegister(MC13783_INT_MSK0_ADDR,
                            imr0 | active_int0, PMIC_ALL_BITS& (~(1 << PMIC_MC13783_INT_CHGDETI)));
                ...
                while (active_int0 || active_int1)
                {
                    ...
                    DEBUGMSG(ZONE_FUNC,
                    (_T("pending interrupts 0 =0x%x, interrupt 1 =0x%x %d\r\n"),
                    ...
                    if (int_src < PMIC_INT_MAX_ID)
                    {
                        ...
                        {
                            ERRORMSG(1,
                            (_T("No PMIC event registered for interrupt source = %d\r\n"),
                              int_src));
                        }
                    }
                    else
...
//-----------------------------------------------------------------------------
//
// Function: PMICAdcSetPowerMode
//
// This function sets the power mode for the PMIC ADC.
//
// Parameters:
//      mode
...
static VOID PMICAdcSetPowerMode(PMIC_TOUCH_POWER_MODE mode)
{
    ...
    if (mode == TM_POWERUP)
    {
        //Enable Touch Screen Reference and low power mode
        data = CSP_BITFVAL(MC13783_ADC0_ADREFMOD, MC13783_ADC0_ENABLE)|
               CSP_BITFVAL(MC13783_ADC0_ADREFEN, MC13783_ADC0_ENABLE);
    }
    else
    {
        //Disable Touch Screen Reference and low power mode
        data = CSP_BITFVAL(MC13783_ADC0_ADREFMOD, MC13783_ADC0_DISABLE)|
               CSP_BITFVAL(MC13783_ADC0_ADREFEN, MC13783_ADC0_DISABLE);
    }

    mask = CSP_BITFMASK(MC13783_ADC0_ADREFMOD)|
...
//-----------------------------------------------------------------------------
//
// Function: PMICIoctlAdcSetPowerMode
//
// This function sets the power mode for the PMIC ADC.
//
// Parameters:
//      pBufIn
//          [in] Points to requested PMIC_TOUCH_POWER_MODE.
//
//      dwLenIn
//          [in] Unused.
//
                         ...
                         DWORD dwLenOut)
{
    DEBUGMSG(ZONE_FUNC, (TEXT("+%s() %d\r\n"), __WFUNCTION__));
    EnterCriticalSection(&adcCs);
    //Set the power mode for PMIC ADC
    PMICAdcSetPowerMode(*((PMIC_TOUCH_POWER_MODE *) pBufIn));
