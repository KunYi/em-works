...
//------------------------------------------------------------------------------
// Global Variables
extern PVOID pv_HWregICOLL;		// PVOID pv_HWregICOLL   = NULL;
extern PVOID pv_HWregPINCTRL;	// PVOID pv_HWregPINCTRL = NULL;
...
BOOL OALIntrInit()
{
    ...
    HW_PINCTRL_IRQSTAT3_CLR(0xFFFFFFFF);

    HW_PINCTRL_IRQEN4_CLR(0xFFFFFFFF);
    HW_PINCTRL_IRQSTAT4_CLR(0xFFFFFFFF);
...
BOOL OALIntrRequestIrqs(DEVICE_LOCATION *pDevLoc, UINT32 *pCount, UINT32 *pIrqs)
{
    ...
    switch (pDevLoc->IfcType)
    {
        case Internal:
            ...
            {
                case CSP_BASE_REG_PA_USBCTRL0:
                    *pIrqs = IRQ_USB0;
                    *pCount = 1;
                    rc = TRUE;
                    break;
        
                case CSP_BASE_REG_PA_USBCTRL1:
                    *pIrqs = IRQ_USB1;
                    *pCount = 1;
                    rc = TRUE;
                    break;
    ...
    for (i = 0; i < count; i++)
    {
        ...
        switch (irq >> 5)
        {
            ...
            case 7:
                ...
                HW_PINCTRL_IRQEN3_SET(1U << (irq - IRQ_GPIO3_PIN0));
                HW_ICOLL_INTERRUPTn_SET(IRQ_GPIO3, BM_ICOLL_INTERRUPTn_ENABLE);
                break;
    
            case 8:
                // Secondary IRQ in GPIO4
                HW_PINCTRL_IRQEN4_SET(1U << (irq - IRQ_GPIO4_PIN0));
                HW_ICOLL_INTERRUPTn_SET(IRQ_GPIO4, BM_ICOLL_INTERRUPTn_ENABLE);
                break;            
...
VOID OALIntrDisableIrqs(UINT32 count, const UINT32 *pIrqs)
{
    ...
    for (i = 0; i < count; i++)
    {
        ...
        switch (irq >> 5)
        {
            ...
            case 7:
                // Secondary IRQ in GPIO3
                HW_PINCTRL_IRQEN3_CLR(1U << (irq - IRQ_GPIO3_PIN0));
                break;
    
            case 8:
                // Secondary IRQ in GPIO4
                HW_PINCTRL_IRQEN4_CLR(1U << (irq - IRQ_GPIO4_PIN0));
                break;
