...
//
//  This function clock gate status for respective channel
//
//  Parameters:
//
//      Channel
...
BOOL DDKApbhDmaChanCLKGATE(UINT8 Channel,BOOL bClockGate)
{
    ...
    if( bClockGate)
    {
        HW_APBH_CTRL0_SET(1 << Channel);         //Reset state
    }
    else
    {
        HW_APBH_CTRL0_CLR(1 << Channel);         //Normal operation
    }
    rc =TRUE;
cleanUp:
    return rc;
...
BOOL DDKApbhDmaResetChan(UINT8 Channel,BOOL bReset)
{
    ...
    if(bReset)
    {
        HW_APBH_CHANNEL_CTRL_SET( 1 << (16 + Channel));
    }
    else
    {
        HW_APBH_CHANNEL_CTRL_CLR( 1 << (16 + Channel));
    }
    rc = TRUE;
cleanUp:
    return rc;
...
BOOL DDKApbhDmaFreezeChan(UINT8 Channel,BOOL bFreeze)
{
    ...
    if( bFreeze )               
    {
        HW_APBH_CHANNEL_CTRL_SET(1 << Channel);  // Freeze the channel
    }
    else                 
    {
        HW_APBH_CHANNEL_CTRL_CLR(1 << Channel);  // unFreeze the channel
    }
    rc = TRUE;
cleanUp:
    return rc;
...
//-----------------------------------------------------------------------------
//
//  Function:  DDKApbhDmaSetBurstSize
//
//
//  Parameters:
//      Channel
...
VOID DDKApbhDmaSetBurstSize(UINT32 Channel, DDK_APBH_BURST_SIZE bstsize)
{
    ...
    if(Channel < DDK_APBH_CHANNEL_GPMI0)
    {
        dwReg = HW_APBH_DMA_BURST_SIZE_RD();
        dwReg &= ~(0x3 << (Channel * 2));
        dwReg |= bstsize << (Channel * 2);
        HW_APBH_DMA_BURST_SIZE_WR(dwReg);
    }
}
...
static BOOL ApbhDmaValidChan(UINT8 Channel)
{
    ...
    {
        ERRORMSG(1, (_T("ApbhDmaValidChan..%d is Invalid channel!\r\n"),Channel));
        goto cleanUp;
    }
    rc = TRUE;
