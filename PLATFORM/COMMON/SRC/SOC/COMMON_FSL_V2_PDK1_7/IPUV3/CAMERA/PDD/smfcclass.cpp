...
BOOL SMFCClass::SMFCInit(void)
{
    ...
    m_hSMFCIntrEvent = CreateEvent(NULL, FALSE, FALSE, IPU_SMFC_INTR_EVENT);
    ...
    m_hCameraSMFCEOFEvent = CreateEvent(NULL, FALSE, FALSE, IPU_SMFC_EOF_EVENT);
    if (m_hCameraSMFCEOFEvent == NULL)
    {
        ERRORMSG(TRUE,
            (TEXT("%s: CreateEvent for SMFC EOF Interrupt failed\r\n"), __WFUNCTION__));
        goto Error;
    }
...
//
//-----------------------------------------------------------------------------
BOOL SMFCClass::SMFCIsBusy(UINT8 ch)
{
    BOOL ret = FALSE;
...
//
//-----------------------------------------------------------------------------
void SMFCClass::SMFCSetChannel(UINT8 ch)
{
    m_iSMFCChannel = ch;
...
//
//-----------------------------------------------------------------------------
BOOL SMFCClass::SMFCConfigureViewfinding(UINT8 ch,CSI_SELECT csi_sel, CSI_SELECT_FRAME csi_FrameId,pSMFCConfigData configData)
{
...
//
//-----------------------------------------------------------------------------
BOOL SMFCClass::SMFCConfigureEncoding(UINT8 ch,CSI_SELECT csi_sel,CSI_SELECT_FRAME csi_FrameId,pSMFCConfigData configData)
{
...
//
//-----------------------------------------------------------------------------
BOOL SMFCClass::SMFCConfig(UINT8 ch,CSI_SELECT csi_sel,CSI_SELECT_FRAME csi_FrameId, pSMFCIDMACChannelParams pChannelParams)
{
    //-----------------------------------
...
BOOL SMFCClass::SMFCStartChannel(void)
{
    ...
    DEBUGMSG(ZONE_FUNCTION,
            (TEXT("%s(): Starting SMFC channel begin!\r\n"),__WFUNCTION__));
    ...
    DEBUGMSG(ZONE_FUNCTION,
            (TEXT("%s(): Starting SMFC channel End!\r\n"),__WFUNCTION__));
...
{    
    ...
    DEBUGMSG(ZONE_FUNCTION,
            (TEXT("%s(): Stopping SMFC channel begin!\r\n"),__WFUNCTION__));
...
//
//-----------------------------------------------------------------------------
void SMFCClass::SMFCIDMACChannelConfig(UINT8 ch,
...
void SMFCClass::SMFCISRLoop(UINT32 timeout)
{
    ...
    while(TRUE)
    {
        RETAILMSG (0, (TEXT("%s: In the loop\r\n"), __WFUNCTION__));

        if ( WaitForSingleObject ( m_hExitSMFCISRThread, 0 ) != WAIT_TIMEOUT )
        {
            ExitThread(1);
        }

        if (WaitForSingleObject(m_hSMFCIntrEvent, timeout) == WAIT_OBJECT_0)
        {
...
void SMFCClass::SMFCDumpIntrRegs(UINT8 ch)
{
