...
static UINT32    g_nOverlayDstBufferOffset[MAX_VISIBLE_OVERLAYS];
    ...
    if(pOverlaySurfaceInfo == NULL)
    {
        ...
        if((pCurMode->width != g_ppOverlays[0]->DstWidth)||
           (pCurMode->height!= g_ppOverlays[0]->DstHeight)||
            ...
            (g_ppOverlays[0]->globalAlpha != TRUE))
    ...
    else
    {
        ...
        if((pCurMode->width != pOverlaySurfaceInfo->DstWidth)||
           (pCurMode->height!= pOverlaySurfaceInfo->DstHeight)|| 
            ...
            (pOverlaySurfaceInfo->globalAlpha != TRUE))
...
void Display_SetupStep2_4Overlay(pOverlaySurf_t pOverlaySurfInfo, UINT32 order,BOOL b4SecDisp)
{
    ...
    switch (pOverlaySurfInfo->iRotate)
    {
        ...
        case DMDO_180:
        case DMDO_270:
            ERRORMSG(1,(TEXT("Rotation is not supported for second overlay\r\n")));
            break;
    }
    ...
    if(pOverlaySurfInfo->ColorKeyPlane == DisplayPlane_0)
    {
        ...
        switch(BSPGetPixelDepthFromRegistry())
        {
            ...
            case 16:
                // We add 3 to R, 2 to G, and 3 to B, because IPU internal format is 888.
                // Thus, we must expand from 565 color key values to 888.
                RGBColorKey =
                    (((tempColorKey & 0xF800) << (GRAPH_WIND_CTRL_SDC_KEY_COLOR_R_LSH-11+3)) |
                    ((tempColorKey & 0x07E0) << (GRAPH_WIND_CTRL_SDC_KEY_COLOR_G_LSH-5+2)) |
                    ((tempColorKey & 0x001F) << (GRAPH_WIND_CTRL_SDC_KEY_COLOR_B_LSH+3)));
                break;        
            case 24:
    ...
    if((g_ppData4ICCombine[order].inputcombAlpha != 0xff)||
       (g_ppData4ICCombine[order].inputcombColorkey!= 0xFFFFFFFF)||
       !pOverlaySurfInfo->globalAlpha)
    {
        g_ppData4ICCombine[order].bCombineEnable = TRUE;
        g_ppData4ICCombine[order].alphaType = pOverlaySurfInfo->globalAlpha ? icAlphaType_Global : icAlphaType_Local;
