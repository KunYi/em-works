...
//
//------------------------------------------------------------------------------
DWORD MXDDLcdc::UpdateOverlay(LPDDHAL_UPDATEOVERLAYDATA pd)
{
    /*
    ...
    */
    ...
    {
        DEBUGMSG(GPE_ZONE_HW, (TEXT("MXDDLcdc::OVERLAY HIDE REQUEST\r\n")));
        if (pSrcSurf == m_pVisibleOverlay)
        {
            ...
            DEBUGMSG(GPE_ZONE_HW, (TEXT("MXDDLcdc::OVERLAY HIDE!\r\n")));
        }
        else
        {
    ...
    if (pSrcSurf != m_pVisibleOverlay)
    {
        ...
        {
            DEBUGMSG(GPE_ZONE_HW, (TEXT("MXDDLcdc::OVERLAY SHOW REQUEST\r\n")));
            ...
            else
            {
                ...
                DEBUGMSG(GPE_ZONE_HW, (TEXT("MXDDLcdc::OVERLAY SHOW\r\n")));
            }
        }
    ...
    {
        RETAILMSG(1, (TEXT("MXDDLcdc UpdateOverlay: wrong parameters. Caution rect width must be 16 aligned!\r\n")));
        DUMP_OVERLAY_OP();
    ...
    overlaySurfInfo.LineStride = (UINT16)(pSrcSurf->AlignedWidth() * m_pMode->Bpp / 8);

    SETUP_OVERLAY_POSITION(pd->rDest.left, pd->rDest.top);

    m_pOverlaySrcOffset = (pd->rSrc.top * overlaySurfInfo.LineStride) + (pd->rSrc.left  * m_pMode->Bpp / 8);
    overlaySurfInfo.nBufPhysicalAddr = m_nLAWPhysical + pSrcSurf->OffsetInVideoMemory() + m_pOverlaySrcOffset;

    // Setup if overlay is flipped as upside-down
    if(IsOverlayMirrorUpDown(pd))
    {
        DEBUGMSG(GPE_ZONE_HW, (TEXT("MXDDLcdc UpdateOverlay: upside-down overlay is requested!\r\n")));
    ...
    {
        //DEBUGMSG(1, (TEXT("Error: can't handle dest color keying!\r\n")));
        //pd->ddRVal = DDERR_INVALIDPARAMS;
    ...
    {
        overlaySurfInfo.Transparency = (UINT16)(pd->overlayFX.dwAlphaConst);
    }
    ...
    if (!IsOverlaySurfEqual(m_pOverlaySurfaceOp, &overlaySurfInfo))
    {
        ...
        // Update the graphic window of LCDC of iMX processor
...
_done:
    DEBUGMSG(GPE_ZONE_HW, (TEXT("MXDDLcdc UpdateOverlay: %s!\r\n"), (pd->ddRVal == DD_OK) ? L"successful" : L"failed"));
    return DDHAL_DRIVER_HANDLED;
}
...
//
//------------------------------------------------------------------------------
DWORD MXDDLcdc::SetOverlayPosition(LPDDHAL_SETOVERLAYPOSITIONDATA pd)
{
    /*
    ...
    */
    ...
    {
        DEBUGMSG(GPE_ZONE_ERROR, (TEXT("MXDDLcdc SetOverlayPosition: error pd->lXPos [%d], pd->lYPos[%d]\r\n"), pd->lXPos, pd->lYPos));
        DUMP_OVERLAY_OP();
        pd->ddRVal = DDERR_INVALIDPARAMS;
        return DDHAL_DRIVER_HANDLED;
    }
...
//
//------------------------------------------------------------------------------
VOID MXDDLcdc::SetVisibleSurfaceOverlay(DDGPESurf * pSurf)
{
    if(!m_pOverlaySurfaceOp->isGraphicWindowRunning)
    {
        return;
    ...
    {
        m_pOverlaySurfaceOp->nBufPhysicalAddr = m_nLAWPhysical + pSurf->OffsetInVideoMemory() + m_pOverlaySrcOffset + m_pOverlaySurfaceOp->LineStride * (m_pOverlaySurfaceOp->HeightHw - 1);
    }
    else
    {
        m_pOverlaySurfaceOp->nBufPhysicalAddr = m_nLAWPhysical + pSurf->OffsetInVideoMemory() + m_pOverlaySrcOffset;
    }
...
//
//------------------------------------------------------------------------------
HRESULT MXDDLcdc::EnableOverlay(VOID)
{
    ...
    else
    {
        ...
        // PGAL: Caution the orginal code just disable the DDK_CLOCK_GATE_INDEX_LCDC clock and not the two clock
        //        initState = DDKClockSetGatingMode(DDK_CLOCK_GATE_INDEX_LCDC, DDK_CLOCK_GATE_MODE_DISABLE);
        
        // Disable LCDC hw clocks
        initState = LCDCEnableClock(FALSE);
        if(!initState)
        {
            DEBUGMSG(GPE_ZONE_ERROR, (TEXT("MXDDLcdc SetModeHardware: can not shutdown LCDC clock, error[%d]!\r\n"), GetLastError()));        
            goto _done;
        }
        ...
        initState = LCDCEnableClock(TRUE);
        if(initState != TRUE)
        {
            DEBUGMSG(GPE_ZONE_ERROR, (TEXT("MXDDLcdc EnableOverlay: LCDC hw clock enable failed!\r\n")));
            goto _done;
        }
    }
...
_done:
    ...
    DEBUGMSG(HAL_ZONE_INIT, (TEXT("MXDDLcdc EnableOverlay: result 0x%08x\r\n"), result));
...
//
//------------------------------------------------------------------------------
HRESULT MXDDLcdc::DisableOverlay(VOID)
{
    if(m_pOverlaySurfaceOp->isGraphicWindowRunning)
    {
...
//
//------------------------------------------------------------------------------
HRESULT MXDDLcdc::GetOverlayAlphaValue(VOID)
{
    m_pOverlaySurfaceOp->Transparency = \
        (UINT16)LCDC_GW_TRANSPARENCY(EXTREG32BF(&m_pLcdcReg->GWCR, LCDC_GWCR_GWAV));
...
//
//------------------------------------------------------------------------------
HRESULT MXDDLcdc::SetOverlayAlphaValue(VOID)
{
...
//
//------------------------------------------------------------------------------
VOID MXDDLcdc::WaitForNotBusyOverlay(VOID)
{
