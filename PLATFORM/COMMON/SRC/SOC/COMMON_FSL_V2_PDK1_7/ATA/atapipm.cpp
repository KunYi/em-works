...
BOOL CDiskPower::RequestDevice(void)
{
    ...
    DEBUGMSG(ZONE_FUNC, (TEXT("CDiskPower: RequestDevice+\r\n")));
    TakeCS();
...
void CDiskPower::ReleaseDevice(void)
{
    ...
    DEBUGMSG(ZONE_FUNC, (TEXT("CDiskPower: ReleaseDevice+\r\n")));
    TakeCS();
...
BOOL CDiskPower::Init(CDisk *pDiskParent)
{
    ...
    if(fOk) {
        ...
        if(!fGotClass) {
            ...
            StringCchCopyN(szClass,dim(szClass),PMCLASS_BLOCK_DEVICE,dim(PMCLASS_BLOCK_DEVICE));
...
#else
            CeSetThreadPriority(m_htPower, nPriority);
#endif
...
DWORD CDiskPower::SetDiskPower(CEDEVICE_POWER_STATE newDx)
{
    ...
    DEBUGMSG(ZONE_FUNC, (TEXT("CDiskPower: SetDiskPower+\r\n")));
    TakeCS();
    ...
    if(newDx != m_curDx) {
        ...
        if(dwStatus == ERROR_SUCCESS) {
            ...
            if(!fGotQPC) {
                ...
                switch(m_curDx) {
                case D0:
                    m_dxInfo[D0].totalQPC.QuadPart += li.QuadPart - m_startQPC.QuadPart;
                    break;
                case D1:
                    m_dxInfo[D1].totalQPC.QuadPart += li.QuadPart - m_startQPC.QuadPart;
                    break;
                case D2:
                    m_dxInfo[D2].totalQPC.QuadPart += li.QuadPart - m_startQPC.QuadPart;
                    break;
                case D3:
                    m_dxInfo[D3].totalQPC.QuadPart += li.QuadPart - m_startQPC.QuadPart;
                    break;
                case D4:
                    m_dxInfo[D4].totalQPC.QuadPart += li.QuadPart - m_startQPC.QuadPart;
                    break;
                }
            }
            m_curDx = newDx;
...
DWORD CDiskPower::GetDiskCapabilities(PPOWER_CAPABILITIES pCap)
{
    ...
    if(!m_pDisk->IsPMEnabled()) {
        ...
        pCap->DeviceDx = 0x11;       // support D4, D0
    }
...
DWORD CDiskPower::DiskPowerIoctl(PIOREQ pIOReq)
{
    ...
    DEBUGMSG(ZONE_FUNC, (TEXT("CDisk: DiskPowerIoctl+\r\n")));
    ...
    case IOCTL_POWER_SET:
        DEBUGMSG(ZONE_FUNC, (TEXT("CDisk: DiskPowerIoctl:IOCTL_POWER_SET+\r\n")));
    ...
    case IOCTL_POWER_GET:
        DEBUGMSG(ZONE_FUNC, (TEXT("CDisk: DiskPowerIoctl:IOCTL_POWER_GET+\r\n")));
        if(pIOReq->pOutBuf != NULL && pIOReq->dwOutBufSize == sizeof(CEDEVICE_POWER_STATE) && pIOReq->pBytesReturned != NULL) {
    ...
    case IOCTL_DISK_GETPMTIMINGS:
        DEBUGMSG(ZONE_FUNC, (TEXT("CDisk: DiskPowerIoctl:IOCTL_DISK_GETPMTIMINGS+\r\n")));
