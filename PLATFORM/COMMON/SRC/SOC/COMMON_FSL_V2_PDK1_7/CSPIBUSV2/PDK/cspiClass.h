...
public:
    cspiClass();
    ~cspiClass(void);
    virtual BOOL CspiInitialize(DWORD Index);
	BOOL CspiIOMux( void );
    virtual void CspiRelease(void);
...
protected:
	...
	//lqk Sep 13,2011
	static UINT32 CspiBufRd8EX(LPVOID pBuf);
	static UINT32 CspiBufRd16EX(LPVOID pBuf);
	static void CspiBufWrt8EX(LPVOID pBuf, UINT32 data);
	static void CspiBufWrt16EX(LPVOID pBuf, UINT32 data);
	//end Sep 13,2011
};
