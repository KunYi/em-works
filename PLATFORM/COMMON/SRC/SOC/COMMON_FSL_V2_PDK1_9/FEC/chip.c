...
BOOL FECEnetInit(pFEC_t pEthernet)
{
    ...
    {
        // Disable MII gasket
        INSREG32BF(&gpFECReg->MIIGSK_ENR, FEC_MIIGSK_ENR_EN, FEC_MIIGSK_ENR_EN_DISABLED);

        // Configure MIIGSK for  RMII mode    
        INSREG32BF(&gpFECReg->MIIGSK_CFGR, FEC_MIIGSK_CFGR_IF_MODE, FEC_MIIGSK_CFGR_IF_MODE_RMII);

        // Enable MII gasket
        INSREG32BF(&gpFECReg->MIIGSK_ENR, FEC_MIIGSK_ENR_EN, FEC_MIIGSK_ENR_EN_ENABLED);
        
        // Wait for MIIGSK to become ready
        while(EXTREG32BF(&gpFECReg->MIIGSK_ENR, FEC_MIIGSK_ENR_READY) != FEC_MIIGSK_ENR_IS_READY);
    }
