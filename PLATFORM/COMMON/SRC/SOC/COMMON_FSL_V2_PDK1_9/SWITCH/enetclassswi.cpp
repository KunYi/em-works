...

{
    ...
    {
        pEthernet->ENETPrivateInfo.MIIPhyAddr = indextmp;
    ...
    IN NDIS_HANDLE WrapperConfigurationContext)
{
    ...
    do {
        ...
        *SelectedMediumIndex = i;
        ...
        pEthernet->EnetMacAddress[ETHER_ADDR_SIZE-1] =pEthernet->EnetMacAddress[ETHER_ADDR_SIZE-1] +(UCHAR)switchindex;
        for(indextmp=0;indextmp<4;indextmp++)
        {
            // Step 5: Map the hardware registers into virtual address
            MemPhysAddr = BSPENETBaseAddress(indextmp);
        
            Status = NdisMRegisterIoPortRange(
                (indextmp==0)? (PVOID *)&pv_HWregENET0
                :(indextmp==1)? (PVOID *)&pv_HWregENET1
                :(indextmp==2)? (PVOID *)&pv_HWregENETSWI
                :(indextmp==3)? (PVOID *)&pv_HWLMENETSWI:0x0,
                MiniportAdapterHandle,
                MemPhysAddr,
                0x4000 );
        ...
        if(result == TRUE)
        {
            ...
            for(indextmp=0;indextmp<3;indextmp++)
            {
                // Step 8: if the hardware initialization is successful,
                // register the interrupt
                pEthernet->intLine[indextmp] = BSPENETGetIrq(indextmp);
            
                Status = NdisMRegisterInterrupt(
                        (PNDIS_MINIPORT_INTERRUPT)&(pEthernet->interruptObj[indextmp]),
                        MiniportAdapterHandle,
                        pEthernet->intLine[indextmp],
                        pEthernet->intLine[indextmp],
                        TRUE,
                        FALSE,
        ...
        CreateThread(NULL, 0, (LPTHREAD_START_ROUTINE)SwitchLearningThread, 0, 0, NULL);
        CreateThread(NULL, 0, (LPTHREAD_START_ROUTINE)SwitchAgeingThread, 0, 0, NULL);
                ...
                if((pv_HWregENET0) != NULL)
                {
                    if(pEthernet!=NULL)
                    {
                        NdisMDeregisterIoPortRange( pEthernet->ndisAdapterHandle,
                                            (ULONG)BSPENETBaseAddress(indextmp),
                                             0x4000,
                                            (PVOID)pv_HWregENET0);
                    }
                    (pv_HWregENET0) = NULL;
                ...
                if( (pv_HWregENETSWI) != NULL)
                {
                    if(pEthernet!=NULL)
                    {
                        NdisMDeregisterIoPortRange( pEthernet->ndisAdapterHandle,
                                            (ULONG)BSPENETBaseAddress(indextmp),
                                            0x4000,
                                            (PVOID)pv_HWregENETSWI);
                    }
                    (pv_HWregENETSWI) = NULL;
                }
            }   
            else if(indextmp==3)
            {
                if( (pv_HWLMENETSWI) != NULL)
                {
                    if(pEthernet!=NULL)
                    {
                        NdisMDeregisterIoPortRange( pEthernet->ndisAdapterHandle,
...
void ENETClass::ENETHalt(IN NDIS_HANDLE MiniportAdapterContext)
{
    ...
    for(indextmp=0;indextmp<3;indextmp++)
    {
        NdisMDeregisterInterrupt(&(pEthernet->interruptObj[indextmp]));    
    }
    ...
    )
{
    ...
    PUCHAR pQuerySetData;
    
    pENET_t pEthernet = ((pENET_t)(MiniportAdapterContext));
    
    DEBUGMSG(ZONE_FUNCTION, (TEXT("ENET: +ENETQueryInformation\r\n")));

    result = NdisAllocateMemory((PVOID *)&pQuerySetData, 1024, 0, HighestAcceptableMax);
    
    if (Status != NDIS_STATUS_SUCCESS)
    {
        DEBUGMSG(ZONE_ERROR, (TEXT("ENET: NdisAllocateMemory failed\n")));
    }
    ...
    switch(Oid)
    {
        ...
        case OID_GET_XMIT_TIMER:
            RETAILMSG(1, (TEXT("@@@  OID_GET_XMIT_TIMER \n") ));
            MoveSource = &TxTimer;
            MoveBytes = sizeof(TxTimer);
            break;
            
        case OID_GET_RCV_TIMER:
            RETAILMSG(1, (TEXT("OID_GET_RCV_TIMER %x %x  \n"), RxTimer.sec,RxTimer.nsecs));  
            MoveSource = &RxTimer;
            MoveBytes = sizeof(RxTimer);            
            break;

        case OID_TCP_TASK_OFFLOAD:
             RETAILMSG(1, (TEXT("@@@  OID_TCP_TASK_OFFLOAD \n") ));
             break;
        ...
        case OID_QUERY_SWITCH:
            // RETAILMSG(1, (TEXT("OID_QUERY_SWITCH 0x%x\r\n"), (UINT32) (*InfoBuffer)));    
            MoveSource=pQuerySetData;
            if((UINT32) (*InfoBuffer)==PORTENA)
            {           
                QueryPORTENA(InfoBuffer, MoveSource, &MoveBytes);
            }
            if((UINT32) (*InfoBuffer)==INPUTLEARNBLOCK)
            {               
                QueryINPUTLEARNBLOCK(InfoBuffer, MoveSource, &MoveBytes);
            }
            if((UINT32) (*InfoBuffer)==VLANVERIFY)
            {               
                QueryVLANVERIFY(InfoBuffer, MoveSource, &MoveBytes);
            }
            if((UINT32) (*InfoBuffer)==VLANRESTABLE)
            {               
                QueryVLANRESTABLE(InfoBuffer, MoveSource, &MoveBytes);
            }
            if((UINT32) (*InfoBuffer)==IPSNOOP)
            {               
                QueryIPSNOOP(InfoBuffer, MoveSource, &MoveBytes);
            }
            if((UINT32) (*InfoBuffer)==PORTSNOOP)
            {               
                QueryPORTSNOOP(InfoBuffer, MoveSource, &MoveBytes);
            }
            if((UINT32) (*InfoBuffer)==MCASTDEFAULTMASK)
            {               
                QueryMCASTDEFAULTMASK(InfoBuffer, MoveSource, &MoveBytes);
            }
            if((UINT32) (*InfoBuffer)==BCASTDEFAULTMASK)
            {               
                QueryBCASTDEFAULTMASK(InfoBuffer, MoveSource, &MoveBytes);
            }
            if((UINT32) (*InfoBuffer)==MIRRORCONTROL)
            {               
                QueryMIRRORCONTROL(InfoBuffer, MoveSource, &MoveBytes);
            }
            if((UINT32) (*InfoBuffer)==IPPRIORITY)
            {               
                QueryIPPRIORITY(InfoBuffer, MoveSource, &MoveBytes);
            }
            if((UINT32) (*InfoBuffer)==PRIORITYCFG)
            {               
                QueryPRIORITYCFG(InfoBuffer, MoveSource, &MoveBytes);
            }
            if((UINT32) (*InfoBuffer)==VLANPRIORITY)
            {               
                QueryVLANPRIORITY(InfoBuffer, MoveSource, &MoveBytes);
            }
            if((UINT32) (*InfoBuffer)==MIRRORCONFIG)
            {               
                QueryMIRRORCONFIG(InfoBuffer, MoveSource, &MoveBytes);
            }
            if((UINT32) (*InfoBuffer)==MGMTCONFIG)
            {               
                QueryMGMTCONFIG(InfoBuffer, MoveSource, &MoveBytes);
            }
            // RETAILMSG(1, (TEXT("after OID_QUERY_SWI 0x%x\r\n"), MoveBytes));  
            break;
    ...
    )
{
    ...
    switch (Oid)
    {
        case OID_802_3_MULTICAST_LIST:
            for(indextmp=0;indextmp<2;indextmp++)
            {
              // BW_ENET_MAC_RCR_PROM(indextmp,0);
            }
            // Verify length
        ...
        case OID_GEN_CURRENT_PACKET_FILTER:
            ...
            else
            {
                ...
                {
                    RETAILMSG(1, (TEXT("NDIS_PACKET_TYPE_PROMISCUOUS BW_ENET_MAC_RCR_PROM 1\r\n")));
                    for(indextmp=0;indextmp<2;indextmp++)
                    {
                        BW_ENET_MAC_RCR_PROM(indextmp,1);
                    }
                }
      
                
                // TODO: how to set the ENET hardware filter? NO IDEA 
        ...
        case OID_GEN_PROTOCOL_OPTIONS:
            break;
        case OID_SET_SWITCH:
            // RETAILMSG(1, (TEXT("OID_SET_SWITCH 0x%x\r\n"), (UINT32) (*InfoBuffer)));     
            if((UINT32) (*InfoBuffer)==PORTENA)
                SetPORTENA(InfoBuffer, &BytesLeft, BytesNeeded, BytesRead, &StatusToReturn);
            if((UINT32) (*InfoBuffer)==INPUTLEARNBLOCK)
                SetINPUTLEARNBLOCK(InfoBuffer, &BytesLeft, BytesNeeded, BytesRead, &StatusToReturn);
            if((UINT32) (*InfoBuffer)==VLANVERIFY)
                SetVLANVERIFY(InfoBuffer, &BytesLeft, BytesNeeded, BytesRead, &StatusToReturn);
            if((UINT32) (*InfoBuffer)==VLANRESTABLE)
                SetVLANRESTABLE(InfoBuffer, &BytesLeft, BytesNeeded, BytesRead, &StatusToReturn);
            if((UINT32) (*InfoBuffer)==IPSNOOP)
                SetIPSNOOP(InfoBuffer, &BytesLeft, BytesNeeded, BytesRead, &StatusToReturn);
            if((UINT32) (*InfoBuffer)==PORTSNOOP)
                SetPORTSNOOP(InfoBuffer, &BytesLeft, BytesNeeded, BytesRead, &StatusToReturn);
            if((UINT32) (*InfoBuffer)==MCASTDEFAULTMASK)
                SetMCASTDEFAULTMASK(InfoBuffer, &BytesLeft, BytesNeeded, BytesRead, &StatusToReturn);
            if((UINT32) (*InfoBuffer)==BCASTDEFAULTMASK)
                SetBCASTDEFAULTMASK(InfoBuffer, &BytesLeft, BytesNeeded, BytesRead, &StatusToReturn);
            if((UINT32) (*InfoBuffer)==MIRRORCONTROL)
                SetMIRRORCONTROL(InfoBuffer, &BytesLeft, BytesNeeded, BytesRead, &StatusToReturn);
            if((UINT32) (*InfoBuffer)==IPPRIORITY)
                SetIPPRIORITY(InfoBuffer, &BytesLeft, BytesNeeded, BytesRead, &StatusToReturn);
            if((UINT32) (*InfoBuffer)==PRIORITYCFG)
                SetPRIORITYCFG(InfoBuffer, &BytesLeft, BytesNeeded, BytesRead, &StatusToReturn);
            if((UINT32) (*InfoBuffer)==VLANPRIORITY)
                SetVLANPRIORITY(InfoBuffer, &BytesLeft, BytesNeeded, BytesRead, &StatusToReturn);
            if((UINT32) (*InfoBuffer)==MIRRORCONFIG)
                SetMIRRORCONFIG(InfoBuffer, &BytesLeft, BytesNeeded, BytesRead, &StatusToReturn);
            if((UINT32) (*InfoBuffer)==MGMTCONFIG)
                SetMGMTCONFIG(InfoBuffer, &BytesLeft, BytesNeeded, BytesRead, &StatusToReturn);
            // RETAILMSG(1, (TEXT("after OID_SET_SWITCH 0x%x\r\n"), BytesRead));    
            break;
