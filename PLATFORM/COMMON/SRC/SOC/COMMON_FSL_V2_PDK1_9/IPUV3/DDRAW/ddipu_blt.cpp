...
SCODE DDIPU::VidmemToVidmemBlt(GPEBltParms* bltParms)
{
    ...
    if((bltParms->pDst == (m_pPrimarySurface2 ? m_pPrimarySurface2 : m_pActiveSurface)) ||    // only care if dest is main display surface
         (bltParms->pSrc == (m_pPrimarySurface2 ? m_pPrimarySurface2 : m_pActiveSurface)))    // only care if src is main display surface
    {
...
SCODE DDIPU::CopyToVideoMemory(GPESurf* src, DDIPUSurf** dst)
{
    ...
    if(formatPtr)
    {
        if (formatPtr->m_pPalette)
        {
            ...
            {
                case    0x00000F00:
                    format = ddgpePixelFormat_4444;
                    break;
                case    0x00007C00:
                    format = ddgpePixelFormat_5551; 
                    break;
                case    0x0000F800:
                    format = ddgpePixelFormat_565;
                    break;
                case    0x00FF0000:
                    format = ddgpePixelFormat_8888;
                    break;
                default:
                    format = EGPEFormatToEDDGPEPixelFormat[src->Format()];
                    break;
            }
            result = AllocSurface((DDGPESurf**)dst, src->Width(), src->Height(), src->Format(), format, GPE_REQUIRE_VIDEO_MEMORY);
        }
        else
        {
            result = AllocSurface((GPESurf**)dst, src->Width(), src->Height(), src->Format(), GPE_REQUIRE_VIDEO_MEMORY);
        }
    }
    else
    {
    ...
    VMSetAttributeExData vmSetting;        
    vmSetting.lpvAddress = (*dst)->Buffer();
    vmSetting.cbSize = (*dst)->SurfaceSize();
    vmSetting.vmAttr = VMATTR_CACHED_WRITETHROUGH;
    VMSetAttributeEx(&vmSetting, &(*dst)->m_vmAttr);
   
    if (!(*dst)->AssignSurfData(src))
    {
        vmSetting.vmAttr = (*dst)->m_vmAttr;
        VMSetAttributeEx(&vmSetting, &(*dst)->m_vmAttr);
        delete dst;
    ...
    vmSetting.vmAttr = (*dst)->m_vmAttr;
    VMSetAttributeEx(&vmSetting, &vmSetting.vmAttr);
...
C2D_STATUS DDIPU::SetBltParms(GPEBltParms* bltParms)
{
    ...
    if (bltParms->bltFlags & BLT_ALPHABLEND)
    {
        ...
        if (bltParms->blendFunction.AlphaFormat == AC_SRC_ALPHA )
            //&& bltParms->blendFunction.BlendOp == AC_SRC_OVER)  //Only one BlendOP is supported in CE6 and this value is not initialized in MSFT's code, so we can't check it.
        {
    ...
    if (bltParms->bltFlags & BLT_TRANSPARENT)
    {
        ...
        // transparent color.
        C2D_CALL(c2dSetSrcColorkey(m_c2dCtx, static_cast<unsigned int>(bltParms->solidColor), 1))
    }
    else
    {
...
{    
    ...
    //For alphablt we should change the imode to accelerate it.
    if(((bltParms->bltFlags & (BLT_STRETCH | BLT_ALPHABLEND)) == (BLT_STRETCH | BLT_ALPHABLEND)) 
        && (bltParms->iMode == BLACKONWHITE))
    {
        bltParms->iMode = COLORONCOLOR;
    }
    ...
    if (bltParms->pSrc)
    {
        ...
        if(((bltParms->pSrc->Format() == gpe24Bpp) && (bltParms->pDst->Format() != gpe24Bpp))||
            ((bltParms->pDst->Format() == gpe24Bpp) && (bltParms->pSrc->Format() != gpe24Bpp)))
            return FALSE;
        ...
        if(bltParms->pSrc->InVideoMemory())
        {
            ...
            if((((DDIPUSurf *)bltParms->pSrc)->PixelFormat() <= ddgpePixelFormat_YUY2)
                && (((DDIPUSurf *)bltParms->pSrc)->PixelFormat() >= ddgpePixelFormat_YUYV)
                && ((m_c2dFlag & 0x200)== 0))
                return FALSE;
        }
    }
    ...
    else if(bltParms->pLookup != 0 || ((bltParms->pConvert != 0)&&(!bltParms->pSrc->InVideoMemory())))
    {

        GPEFormat       *formatPtr;
        formatPtr = bltParms->pSrc->FormatPtr();
        if(formatPtr)
        {
            if (formatPtr->m_pPalette)
            {
                switch(formatPtr->m_pPalette[0])
                {
                    ...
                    case    0x00FF0000:
                        return true;
                    case    0x0000F800:
                        if (formatPtr->m_pPalette[1] == 0x000007e0)
                            return true;
                }
            }
        }
...
bool DDIPU::IsSimpleCopyBlt(GPEBltParms* bltParms)
{
    ...
    if (bltParms->rop4 != 0xCCCC || (bltParms->bltFlags & 0x3ff) != 0 || 
        !standard || rotate || bltParms->pConvert)
    {
        return false;
    }
    
    //For large resolution, we still use hardware acceleration.
    if(bltParms->pSrc->Width()*bltParms->pSrc->Height() >= 10000)
        return false;
...
SCODE DDIPU::WrapEmulatedBlt(GPEBltParms *blitParameters)
{
    ...
    // This will be used if we have an ASync driver (to trigger a partial refresh)
    // or if we are using TV out.
    if((blitParameters->pDst == (m_pPrimarySurface2 ? m_pPrimarySurface2 : m_pActiveSurface)) ||    // only care if dest is main display surface
         (blitParameters->pSrc == (m_pPrimarySurface2 ? m_pPrimarySurface2 : m_pActiveSurface)))    // only care if src is main display surface
    {
        // Now, calculate the dirty-rect to refresh to the actual hardware
        RECT bounds;
