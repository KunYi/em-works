...
extern BOOL BSPGetYUV420OutPutSupport(); //IPUv3EX in Elvis TO2 can't output YV12 data correctly.
...
BOOL PpClass::PpInit(void)
{
    ...
    // Events to exit PPISRThread
    m_hExitPpISRThread = CreateEvent ( NULL,TRUE,FALSE,NULL ) ;
    if(NULL == m_hExitPpISRThread)
    {
        ERRORMSG(TRUE,
            (TEXT("%s: CreateEvent failed for Exit PPISRThread\r\n"), __WFUNCTION__));
        goto Error;
    }
...
void PpClass::PpDeinit(void)
{
    ...
    if (m_hPpISRThread)
    {
        // Set PpIntrThread end
        SetEvent ( m_hExitPpISRThread ); 
        // Wait for PpIntrThread end
        if (WAIT_OBJECT_0 == WaitForSingleObject (m_hPpISRThread,INFINITE))
            RETAILMSG(0,(TEXT("Wait for PPIntrThread end")));

        CloseHandle(m_hPpISRThread);
        m_hPpISRThread = NULL;
    }

    if (m_hExitPpISRThread != NULL)
    {
        CloseHandle(m_hExitPpISRThread);
        m_hExitPpISRThread = NULL;
    }
...
BOOL PpClass::PpConfigure(pPpConfigData pConfigData)
{
    ...
    // If current IPU not support YUV420 output
    if(!BSPGetYUV420OutPutSupport())
    {
    ...
    if(pConfigData->bCombineEnable == TRUE)
    {
        ...
        iFunctionFlag = 0;
        if((pConfigData->inputcombIDMAChannel.FrameSize.width != pConfigData->outputIDMAChannel.FrameSize.width)
          ||(pConfigData->inputcombIDMAChannel.FrameSize.height != pConfigData->outputIDMAChannel.FrameSize.height))
        {
             ERRORMSG(1,
    ...
    else
    {
        ...
        //Setup the final stride
        m_channelParams.iLineStride = pConfigData->outputIDMAChannel.LineStride;
        //If only rotation is enabled, we need reconfigure the output pxiel format for processing 
        // RGB -> RGB correctly.(like RGB565->RGB888)
        if(iFunctionFlag == PP_FUNC_ROTATION)
        {
            PpPixelFormatSetting(pConfigData->outputIDMAChannel.FrameFormat, 
                                 pConfigData->outputIDMAChannel.PixelFormat, 
                                 pConfigData->outputIDMAChannel.DataWidth);
...
BOOL PpClass::PpParamsCheck(pPpConfigData pConfigData)
{
    ...
    else if((pConfigData->inputIDMAChannel.FrameFormat== icFormat_YUV444)
        || (pConfigData->inputIDMAChannel.FrameFormat== icFormat_YUV422))
    {
        if( pConfigData->inputIDMAChannel.FrameSize.width & 0x07 )
        {
            DEBUGMSG(ZONE_ERROR,
    ...
    else if((pConfigData->outputIDMAChannel.FrameFormat== icFormat_YUV444)
        || (pConfigData->outputIDMAChannel.FrameFormat== icFormat_YUV422))
    {
        if( m_PpOutputWidth & 0x07 )
        {
            DEBUGMSG(ZONE_ERROR,
    ...
    if(pConfigData->alphaType == icAlphaType_SeparateChannel)
    {
        if((pConfigData->inputIDMAChannel.FrameSize.width & 0x07) ||
           (pConfigData->outputIDMAChannel.FrameSize.width & 0x07))
        {
            DEBUGMSG(ZONE_ERROR,
                    (TEXT("%s: width must aligned with 8 using icAlphaType_SeparateChannel\r\n"), __WFUNCTION__));
...
BOOL PpClass::PpChangeMaskPos(RECT *pRect)
{
    ...
    if(pRect->left%8 != 0)
    {
        pRect->left = pRect->left &0xfffffff0;
...
void PpClass::PpConfigureMask()
{
    ...
    IPUBufInfo.dwBufferSize = (IPUBufInfo.dwBufferSize+7)&0xfffffff0; //align with 8
...
BOOL PpClass::IRTConfigureInput(pIcFlipRot pFlipRot)
{
    ...
    else if(m_PpLastModule == ppSourceType_PP)
    {
        ...
        if (m_bCurrentPlanar)
        {
            ...
            if (IDMAC_NON_INTERLEAVED_FORMAT_CODE_YUV444 == m_channelParams.iFormatCode)
            {
                OffsetData.uOffset = OffsetData.vOffset;  // YUV444 is Y.U.V
                OffsetData.vOffset = OffsetData.uOffset + OffsetData.uOffset;
            }
            else if (IDMAC_NON_INTERLEAVED_FORMAT_CODE_YUV422 == m_channelParams.iFormatCode)
            {
                OffsetData.uOffset = OffsetData.vOffset;  // YUV422 is Y.U.V
                OffsetData.vOffset = OffsetData.uOffset + OffsetData.uOffset/2;
            }
            else if (IDMAC_NON_INTERLEAVED_FORMAT_CODE_YUV420 == m_channelParams.iFormatCode)
...
BOOL PpClass::PpConfigureOutput(pIdmaChannel pIDMAChannel)
{
    ...
    if (m_bCurrentPlanar)
    {
        ...
        OffsetData.uOffset = pIDMAChannel->UBufOffset;
        OffsetData.vOffset = pIDMAChannel->VBufOffset;
...
void PpClass::PpISRLoop(UINT32 timeout)
{
    ...
    hLoopEvent[0] = m_hExitPpISRThread;
    hLoopEvent[1] = m_hPpIntrEvent;
