...
#pragma warning(disable: 4100 4115 4189 4201 4204 4214 6244 )
    ...
    )
{
    ...
    if( !hKey )
    {
        DEBUGMSG( ZONE_INIT|ZONE_ERROR, (_T("CAM_Init : OpenDeviceKey failed!\r\n")));
        return FALSE;
    }
    ...
    if( ERROR_SUCCESS == RegQueryValueEx( hKey, L"Index", 0, &dwType, (BYTE *)&dwValue, &dwSize ) )
    {
        if((REG_DWORD == dwType) && ( sizeof( DWORD ) == dwSize ))
        {
            ...
            {
                m_uCameraID = 1;
                DEBUGMSG( ZONE_INIT, (_T("CAM_Init : Current cameraID is N0.1 \r\n")));
            }

            if( dwValue == 2)
            {
                m_uCameraID = 2;
                DEBUGMSG( ZONE_INIT, (_T("CAM_Init : Current cameraID is N0.2 \r\n")));
            }
        }
    }
    else
    {
        RegCloseKey( hKey );
        DEBUGMSG( ZONE_INIT|ZONE_ERROR, (_T("CAM_Init : Get camera driver key failed! \r\n")));
        return FALSE;
    }
    ...
    if( m_uCameraID==1 )
        m_hStream = ActivateDeviceEx( PIN1_REG_PATH, NULL, 0, reinterpret_cast<LPVOID>( this ) );
    else if( m_uCameraID==2 )
        m_hStream = ActivateDeviceEx( PIN2_REG_PATH, NULL, 0, reinterpret_cast<LPVOID>( this ) );
    else
        DEBUGMSG( ZONE_INIT|ZONE_ERROR, (_T("CAM_Init: Can not support CameraID NO.%d\r\n"),m_uCameraID));
    ...
    if( m_PDDFuncTbl.dwSize < sizeof( PDDFUNCTBL ) ||
        ...
        NULL == m_PDDFuncTbl.PDD_PreStillPicture ||
        NULL == m_PDDFuncTbl.PDD_PostStillPicture )
    {
        return false;
    }
    ...
    *pdwBytesTransferred = 0;
    ...
    switch( csProp.Id )
    {
        ...
        case CSPROPERTY_PIN_DEVICENAME:
        {
            ...
            if( m_uCameraID == 1)
            {
                *pdwBytesTransferred = ( wcslen( g_wszPin1DeviceNames[lPinId] ) + 1 ) * sizeof( g_wszPin1DeviceNames[0][0] ) ;
                ...
                if( CeSafeCopyMemory( pOutBuf, g_wszPin1DeviceNames[lPinId], *pdwBytesTransferred ))
                {
                    dwError = ERROR_SUCCESS;
                }
            }
            ...
            {
                *pdwBytesTransferred = ( wcslen( g_wszPin2DeviceNames[lPinId] ) + 1 ) * sizeof( g_wszPin2DeviceNames[0][0] ) ;
                ...
                if( CeSafeCopyMemory( pOutBuf, g_wszPin2DeviceNames[lPinId], *pdwBytesTransferred ))
                {
                    dwError = ERROR_SUCCESS;
                }
            }
    ...
    *pdwBytesTransferred = 0;
    ...
    case CSPROPERTY_VIDEOCONTROL_CAPS:
        ...
        case CSPROPERTY_TYPE_SET:
            ...
            if (ulCaps & (CS_VideoControlFlag_FlipHorizontal |CS_VideoControlFlag_FlipVertical))
             {
                    // Make sure pin is off when changing rotation
                    if(m_pStrmInstances[lIndex].pPinDev->GetState() != CSSTATE_STOP)
                    {
                        // Pin must be stopped before changing flip state
                        DEBUGMSG(ZONE_ERROR, (_T("CAM_IOControl(%08x): Must stop pin before changing camera flip/rotation settings.\r\n"), this));
                        break;
                    }
              }
          ...
          case CSPROPERTY_TYPE_GET:
            *pdwBytesTransferred = sizeof(CSPROPERTY_DROPPEDFRAMES_CURRENT_S);
            ...
            if ( m_pStrmInstances[CAPTURE].pPinDev )
            {
                ...
                DEBUGMSG(ZONE_FUNCTION, (TEXT("%s: DroppedFrame: PictureNumber: %x, DropCount: %x\r\n"), 
                                        __WFUNCTION__, csPropDroppedFramesOutput.PictureNumber,csPropDroppedFramesOutput.DropCount));
    ...
    )
{
    ...
    case IOCTL_POWER_CAPABILITIES:
        ...
        if ( pOutBuf && OutBufLen >= sizeof(POWER_CAPABILITIES) )
        {
             ...
             RETAILMSG (0, (_T("%s: IOCTL_POWER_CAPABILITIES\r\n"), __WFUNCTION__));
    ...
    case IOCTL_POWER_SET:
        ...
        if ( pOutBuf && OutBufLen >= sizeof(CEDEVICE_POWER_STATE) )
        {
            ...
            default:
                DEBUGMSG(ZONE_IOCTL, (TEXT("CAM: Unrecognized Valid Power state. Powering Up\r\n")));
...
CCameraDevice::PowerUp()
{
    if ( m_hCallerProcess )
    {
        // First enable sensor clk
        m_PDDFuncTbl.PDD_SetPowerState( m_PDDContext, m_PowerState );
        
        // Set CSSTATE_RUN state
        for( UINT i = 0; i < m_AdapterInfo.ulCTypes; i++ )
        {
            if ( NULL != m_pStrmInstances[i].pPinDev ) 
            {
...
DWORD CCameraDevice::PDDFillPinBufferEx( ULONG ulPinId, PUCHAR* ppImage )
{  
    return m_PDDFuncTbl.PDD_FillBufferEx( m_PDDContext, ulPinId, ppImage );
}
...
DWORD CCameraDevice::PDDEnqueueBuffer( ULONG ulPinId, PVOID pBuffer )
{   
    return m_PDDFuncTbl.PDD_EnqueueBuffer( m_PDDContext, ulPinId, pBuffer );
}
...
DWORD CCameraDevice::PDDPreStillPicture( ULONG ulPinId )
{   
    return m_PDDFuncTbl.PDD_PreStillPicture( m_PDDContext, ulPinId );
}

DWORD CCameraDevice::PDDPostStillPicture( ULONG ulPinId )
{   
    return m_PDDFuncTbl.PDD_PostStillPicture( m_PDDContext, ulPinId );
}
