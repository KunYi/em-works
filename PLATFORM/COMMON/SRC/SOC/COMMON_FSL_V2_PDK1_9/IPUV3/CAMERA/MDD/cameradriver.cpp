...
#pragma warning(push)
#pragma warning(disable: 4115 4201 4204 4214 6244)
#include <windows.h>
#include <ceddk.h>
#include <Devload.h>
#include <ceddk.h>
...
//Define a IOCTL only used by Freescale Codec
#define IOCTL_CS_FSLFILTER       CTL_CODE(FILE_DEVICE_KS, 0x00F, METHOD_NEITHER, FILE_ANY_ACCESS)
#define FSLFILTER_SUPPORT_ON     0x00
#define FSLFILTER_SUPPORT_OFF    0xFF
   ...
   )
{
    ...
    switch ( Ioctl )
    {
        ...
        case IOCTL_CS_FSLFILTER:
            //Support Freescale Codec Filter
            if(InBufLen == 1)
            {
                HANDLE FslFilter = CreateEvent(NULL, TRUE, FALSE, TEXT("Freescale codec filter"));

                if(*pInBuf == FSLFILTER_SUPPORT_ON)
                {
                    //if freescale filter require all format support, we will support all format
                    SetEvent(FslFilter);
                }
                else if(*pInBuf == FSLFILTER_SUPPORT_OFF)
                {
                    //Cancel all format support, We will only support few format which CETK supports
                    ResetEvent(FslFilter);
                }
                else
                {
