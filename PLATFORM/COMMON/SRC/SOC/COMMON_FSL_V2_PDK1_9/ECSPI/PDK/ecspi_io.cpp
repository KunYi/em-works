...
//  File:  ecspi_io.cpp
//
//   This module provides a stream interface for the eCSPI bus
        ...
        switch (dwCode) 
        {
            ...
            case IOCTL_POWER_SET: 
                    ...
                    && pdwActualOut != NULL) 
                {
                    ...
                    __try 
                    {
                        ...
                        if (VALID_DX(dx)) 
                        {
                            ...
                            *pdwActualOut = sizeof(CEDEVICE_POWER_STATE);
                            ...
                            if (dx == D4) 
                            {
                                if(pCspi->m_bExchanging)
                                {
                                    // Wait for the current burst exchanging finished
                    ...
                    __except(EXCEPTION_EXECUTE_HANDLER) 
                    {
                        ERRORMSG(TRUE, (_T("Exception in eCSPI IOCTL_POWER_CAPABILITIES\r\n")));
                    }
                }
                break;
