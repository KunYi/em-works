...
//  File:  ecspisdk.cpp
//
//  The implementation of eCSPI driver SDK.
//
//-----------------------------------------------------------------------------
...
HANDLE CSPIOpenHandle(LPCWSTR lpDevName)
{
    ...
    {
        DEBUGMSG(ZONE_ERROR,
            (TEXT("%s:  CreateFile eCSPI failed!\r\n"), __WFUNCTION__));
        return hCSPI;
    }
...
BOOL CSPIExchange(HANDLE hCSPI, PCSPI_XCH_PKT_T pCspiXchPkt)
{
    ...
    // Check if BurstLength matches xchCnt
    if (pCspiXchPkt->xchCnt>128||pCspiXchPkt->pBusCnfg->BurstLength<1)
    {
        DEBUGMSG(ZONE_ERROR,(TEXT("CSPI_IOCTL_EXCHANGE failed! please check ExchangeCount and BurstLength!\r\n")));
        RETAILMSG(1,(TEXT("CSPI_IOCTL_EXCHANGE failed! please check ExchangeCount and BurstLength!\r\n")));
        return FALSE;
    }

    if ( pCspiXchPkt->pBusCnfg->BurstLength>pCspiXchPkt->xchCnt*32 ||
        pCspiXchPkt->pBusCnfg->BurstLength<(pCspiXchPkt->xchCnt-1)*32 )
    {
        DEBUGMSG(ZONE_ERROR,(TEXT("CSPI_IOCTL_EXCHANGE failed! ExchangeCount does not match BurstLength\r\n")));
        RETAILMSG(1,(TEXT("CSPI_IOCTL_EXCHANGE failed! ExchangeCount does not match BurstLength!\r\n")));
        return FALSE;
    }
...
DllEntry(HANDLE hInstDll, DWORD dwReason, LPVOID lpvReserved)
{
    ...
    switch (dwReason) {
        case DLL_PROCESS_ATTACH:
            ...
            DEBUGMSG(ZONE_INIT,
                     (_T("***** DLL PROCESS ATTACH TO eCSPI SDK *****\r\n")));
            DisableThreadLibraryCalls((HMODULE) hInstDll);
            break;

        case DLL_PROCESS_DETACH:
            DEBUGMSG(ZONE_INIT,
                     (_T("***** DLL PROCESS DETACH FROM eCSPI SDK *****\r\n")));
            break;
    }
