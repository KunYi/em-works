...
//  Parameters:
//     tve_out_mode
//           [in] tv output mode for identifying which cable needs to be detected.
//
//  Returns:
//      Returns TRUE if successful, otherwise returns FALSE.
//
//-----------------------------------------------------------------------------   
BOOL TveClass::TveIsCableDetected(TVE_TV_OUT_MODE tv_out_mode)
{
    UINT32 statReg = 0;
    BOOL bRet = FALSE;
...
//     
//     tv_out_mode
//           [in] tv output mode (composite, svideo, componment, etc.) to set
//
//  Returns:
//      Returns TRUE if successful, otherwise returns FALSE.
//
...
BOOL TveClass::TveSetCableDetection(TVE_CABLE_DETECTION_MODE cd_mode, TVE_TV_STAND tv_stand, TVE_TV_OUT_MODE tv_out_mode)
{
    int data=0;
    UNREFERENCED_PARAMETER(tv_stand);
    switch (cd_mode) {
        case TVE_CD_TRIGGER_IN_MANUAL:
               
            // configure the CD module to trigger in manual mode (used in test mode also)
            OUTREG32(&m_pTVE->CD_CONT_REG, 
                CSP_BITFVAL( TVE_CD_EN, TVE_CD_EN_ENABLE)                     |  // enable cable detection
                CSP_BITFVAL( TVE_CD_TRIG_MODE, TVE_CD_TRIG_MODE_AUTO)         |  // TVE_CD_TRIG_MODE_MANUAL, Manual (single
                                                                                 // shot) trigger,set cd_trig_mode = 1    
                CSP_BITFVAL( TVE_CD_CH_0_LM_EN, TVE_CD_CH_0_LM_EN_ENABLE)     |
                CSP_BITFVAL( TVE_CD_CH_1_LM_EN, TVE_CD_CH_1_LM_EN_ENABLE)     |
                CSP_BITFVAL( TVE_CD_CH_2_LM_EN, TVE_CD_CH_2_LM_EN_ENABLE)     |
                CSP_BITFVAL( TVE_CD_CH_0_SM_EN, TVE_CD_CH_0_SM_EN_ENABLE)     |
                CSP_BITFVAL( TVE_CD_CH_1_SM_EN, TVE_CD_CH_1_SM_EN_ENABLE)     |
                CSP_BITFVAL( TVE_CD_CH_2_SM_EN, TVE_CD_CH_2_SM_EN_ENABLE)     |
                CSP_BITFVAL( TVE_CD_CH_0_REF_LVL, TVE_CD_CH_0_REF_LVL_LUMA)   |  // the ref level could be set to LUMA
                CSP_BITFVAL( TVE_CD_CH_1_REF_LVL, TVE_CD_CH_1_REF_LVL_LUMA)   |  // the ref level could be set to LUMA
                CSP_BITFVAL( TVE_CD_CH_2_REF_LVL, TVE_CD_CH_2_REF_LVL_LUMA)   |  // the ref level could be set to LUMA
                CSP_BITFVAL( TVE_CD_REF_MODE, TVE_CD_REF_MODE_MANUAL)         |  // Manual mode when the reference voltage is 
                                                                                 // set according to the CD_CH_#_REF_LVL. 
                CSP_BITFVAL( TVE_BG_RDY_TIME, TVE_BG_RDY_TIME_1MS));             // TVDAC bandgap reference ready time
        ...
        case TVE_CD_TRIGGER_IN_STANDBY:
            ...
            data = CSP_BITFVAL( TVE_CD_EN, TVE_CD_EN_ENABLE)                     |  // enable cable detection
                   CSP_BITFVAL( TVE_CD_TRIG_MODE, TVE_CD_TRIG_MODE_MANUAL)       |  // Manual mode, set cd_trig_mode = 1
                   CSP_BITFVAL( TVE_CD_MON_PER, TVE_CD_PER_TIME_200MS)           |  // 200ms for cable detection monitoring period 
                                                                                    // in standby mode
                   CSP_BITFVAL( TVE_CD_CH_0_LM_EN, TVE_CD_CH_0_LM_EN_ENABLE)     |
                   CSP_BITFVAL( TVE_CD_CH_1_LM_EN, TVE_CD_CH_1_LM_EN_ENABLE)     |
                   CSP_BITFVAL( TVE_CD_CH_2_LM_EN, TVE_CD_CH_2_LM_EN_ENABLE)     |
                   ...
                   CSP_BITFVAL( TVE_CD_CH_1_SM_EN, TVE_CD_CH_1_SM_EN_ENABLE)     |
                   CSP_BITFVAL( TVE_CD_CH_2_SM_EN, TVE_CD_CH_2_SM_EN_ENABLE)     |
                   CSP_BITFVAL( TVE_CD_CH_0_REF_LVL, TVE_CD_CH_0_REF_LVL_LUMA)   |  // the ref level could be set to CHROMA or LUMA
                   CSP_BITFVAL( TVE_CD_CH_1_REF_LVL, TVE_CD_CH_1_REF_LVL_LUMA)   |  // the ref level could be set to CHROMA or LUMA
                   CSP_BITFVAL( TVE_CD_CH_2_REF_LVL, TVE_CD_CH_2_REF_LVL_LUMA)   |  // the ref level could be set to CHROMA or LUMA
                   CSP_BITFVAL( TVE_CD_REF_MODE, TVE_CD_REF_MODE_AUTO)           |  // set TVE_CD_REF_MODE = 0
                   CSP_BITFVAL( TVE_BG_RDY_TIME, TVE_BG_RDY_TIME_1MS);              // TVDAC bandgap reference ready time
        ...
        case TVE_CD_TRIGGER_IN_FUNC:
            ...
            // configure the CD module to trigger in manual mode (used in test mode also)
            data = CSP_BITFVAL( TVE_CD_EN, TVE_CD_EN_ENABLE)                      |  // enable cable detection
                   CSP_BITFVAL( TVE_CD_TRIG_MODE, TVE_CD_TRIG_MODE_MANUAL)        |  // set cd_trig_mode = 1 for manual
                   CSP_BITFVAL( TVE_CD_CH_0_LM_EN, TVE_CD_CH_0_LM_EN_ENABLE)      |
                   CSP_BITFVAL( TVE_CD_CH_1_LM_EN, TVE_CD_CH_1_LM_EN_ENABLE)      |
                   CSP_BITFVAL( TVE_CD_CH_2_LM_EN, TVE_CD_CH_2_LM_EN_ENABLE)      |
                   CSP_BITFVAL( TVE_CD_CH_0_SM_EN, TVE_CD_CH_0_SM_EN_ENABLE)      |
                   CSP_BITFVAL( TVE_CD_CH_1_SM_EN, TVE_CD_CH_1_SM_EN_ENABLE)      |
                   CSP_BITFVAL( TVE_CD_CH_2_SM_EN, TVE_CD_CH_2_SM_EN_ENABLE)      |
                   CSP_BITFVAL( TVE_CD_CH_0_REF_LVL, TVE_CD_CH_0_REF_LVL_LUMA)    |
                   CSP_BITFVAL( TVE_CD_CH_1_REF_LVL, TVE_CD_CH_1_REF_LVL_LUMA)    |
                   CSP_BITFVAL( TVE_CD_CH_2_REF_LVL, TVE_CD_CH_2_REF_LVL_LUMA)    |
                   CSP_BITFVAL( TVE_CD_REF_MODE, TVE_CD_REF_MODE_AUTO)            |  // Auto mode, set TVE_CD_REF_MODE = 0
                   CSP_BITFVAL( TVE_BG_RDY_TIME, TVE_BG_RDY_TIME_1MS);               // TVDAC bandgap reference ready time
            
            
            if (tv_out_mode == TV_OUT_DISABLE)
                data |=  CSP_BITFVAL( TVE_CD_MON_PER, TVE_CD_PER_TIME_200MS);
            else
                data |=  CSP_BITFVAL( TVE_CD_MON_PER, 0);
