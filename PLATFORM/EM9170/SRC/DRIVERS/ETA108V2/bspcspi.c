...
void BSPETA108Reset( UINT32 dwTimes )
{
	...
	// define GPIO10 as the reset-pin of the ETA108
	DDKIomuxSetPinMux(DDK_IOMUX_PIN_GPIO_F, DDK_IOMUX_PIN_MUXMODE_ALT0, DDK_IOMUX_PIN_SION_REGULAR);
	DDKIomuxSetPadConfig(DDK_IOMUX_PIN_GPIO_F, DDK_IOMUX_PAD_SLEW_SLOW, DDK_IOMUX_PAD_DRIVE_NORMAL, DDK_IOMUX_PAD_OPENDRAIN_DISABLE, DDK_IOMUX_PAD_PULL_UP_100K, DDK_IOMUX_PAD_HYSTERESIS_ENABLE, DDK_IOMUX_PAD_VOLTAGE_3V3);
	DDKGpioSetConfig( DDK_GPIO_PORT1, 5, DDK_GPIO_DIR_OUT, DDK_GPIO_INTR_NONE );
	//low level
	DDKGpioWriteDataPin(DDK_GPIO_PORT1, 5, 0);
	Sleep( dwTimes ) ;
	//high level
	DDKGpioWriteDataPin(DDK_GPIO_PORT1, 5, 1);
