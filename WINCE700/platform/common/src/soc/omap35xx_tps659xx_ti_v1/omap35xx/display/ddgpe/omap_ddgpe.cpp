...
static const DEVICE_REGISTRY_PARAM g_deviceRegParams[] = {
    {
        ...
        fieldsize(OMAPDDGPEGlobals, m_dwTvOut_FilterLevel), (VOID*)3
    }, {
        ...
        fieldsize(OMAPDDGPEGlobals, m_lTvOut_Offset_W), (VOID*)0
    }, {
        L"TVOut_Offset_H", PARAM_DWORD, FALSE, offset(OMAPDDGPEGlobals, m_lTvOut_Offset_H),
        fieldsize(OMAPDDGPEGlobals, m_lTvOut_Offset_H), (VOID*)-15
    }, {
...
OMAPDDGPE::OMAPDDGPE()
{
    ...
    {
        DEBUGMSG(GPE_ZONE_ERROR, (L"ERROR: Failed advertise display power class\r\n"));
        return;
    }
    ...
    )
{
    ...
    switch (iEsc)
    {
        ...
        case DRVESC_GETSCREENROTATION:
            ...
            *(int *)pvOut = ((DMDO_0 | DMDO_90 | DMDO_180 | DMDO_270) << 8) | ((BYTE)m_iGraphicsRotate);
...
AdvertisePowerInterface(HMODULE hInst)
{
    ...
    if(!fOk)
    {
        DEBUGMSG(GPE_ZONE_ERROR, (L"ERROR: Failed convert display power class to GUID\r\n"));
        return fOk;
    }

    // check for an override for default power manageable display class in the registry
    ...
    if(dwStatus == ERROR_SUCCESS)
    {
        ...
        if(dwStatus == ERROR_SUCCESS && dwType == REG_SZ)
        {
            ...
            if(ConvertStringToGuid(szTemp, &gTemp))
            {
                gClass = gTemp;
            }
            else
            {
                DEBUGMSG(GPE_ZONE_WARNING, (L"WARNING: Failed convert display power class to GUID\r\n"));
                DEBUGMSG(GPE_ZONE_WARNING, (L"WARNING: Default power class will be advertised.\r\n"));
            }
        }
    ...
    if(fOk)
    {
        fOk = GetModuleFileName(hInst, szTemp, _countof(szTemp));
        if(!fOk)
        {
            DEBUGMSG(GPE_ZONE_ERROR, (L"ERROR: Failed to get the display module name.\r\n"));
        }
    }
