...
#define IDGND_IST_PRIORITY          107
#define OTG_IST_PRIORITY            105
#define DMA_IST_PRIORITY            106
                ...
                else
                {
                    ...
                    */
                    ...
                    /*
                        ...
                        to clear outstanding interrupts
                    */
                    INREG8(&pGen->IntrUSB);
                    INREG16(&pGen->IntrTx);
                    INREG16(&pGen->IntrRx);
...
BOOL OMAPMHSUSBOTG::SoftResetMUSBController(BOOL bCalledFromPowerThread)
{
    ...
    SETREG32(&pOtg->OTG_INTERFSEL, OTG_INTERFSEL_12_PIN_ULPI);
...
DWORD OMAPMHSUSBOTG::ThreadRun()
{
    ...
    /* This thread waits here infinitely untill both musbhost and musbfn drivers are loaded.
       SYSGEN_USBFN is required for building musbfn.dll. Selecting the USB OTG driver from
       catalog sets SYSGEN_USBFN by default. USB Host still works fine even without this
       check and musbfn not included in the image. */
    ...
    {                        
        ...
        else
        {
            ...
            if (!(INREG8(&m_pOTG->pUsbGenRegs->DevCtl) & DEVCTL_SESSION))
            {
                SETREG8(&m_pOTG->pUsbGenRegs->DevCtl, DEVCTL_SESSION);          
                m_timeout = 100;
                if (rc == WAIT_TIMEOUT)
