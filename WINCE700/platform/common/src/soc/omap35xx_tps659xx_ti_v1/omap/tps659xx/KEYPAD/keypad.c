    ...
    )
{
    ...
    if (!TWLSetIntrEvent(pDevice->hTWL, TWL_INTR_ITKPI, pDevice->hIntrEventKeypad))
        {
        DEBUGMSG (ZONE_ERROR, (L"ERROR: KPD_Init: "
    ...
    if (!pDevice->bDisablePwronKey)
        {
        if (!TWLSetIntrEvent(pDevice->hTWL, TWL_INTR_PWRON, pDevice->hIntrEventPower))
            {
            DEBUGMSG (ZONE_ERROR, (L"ERROR: KPD_Init: "
                L"Failed associate event with TWL PWRON interrupt\r\n"
                ));
            goto cleanUp;
            }
        }
    ...
    // Enable KP event
    if (!TWLIntrEnable(pDevice->hTWL, TWL_INTR_ITKPI))
        {
        DEBUGMSG (ZONE_ERROR, (L"ERROR: KPD_Init: "
            L"Failed associate event with TWL KBD interrupt\r\n"
            ));
    ...
    if (!pDevice->bDisablePwronKey)
        ...
        pDevice->hIntrThreadPower = CreateThread(
            NULL, 0, PWRON_IntrThread, pDevice, 0,NULL
            );
    ...
    )
{
    ...
    if (pDevice->hTWL != NULL)
        {
        TWLIntrDisable(pDevice->hTWL, TWL_INTR_ITKPI);
        if (!pDevice->bDisablePwronKey)
            {
            TWLIntrDisable(pDevice->hTWL, TWL_INTR_PWRON);
            }
        
        if (pDevice->hIntrEventKeypad != NULL)
            {
            TWLSetIntrEvent(pDevice->hTWL, TWL_INTR_ITKPI, NULL);
            }        
        ...
        if (!pDevice->bDisablePwronKey)
            ...
            if (pDevice->hIntrEventPower!= NULL)
                {
                TWLSetIntrEvent(pDevice->hTWL, TWL_INTR_PWRON, NULL);
                }
            }
...
DWORD KPD_IntrThread(VOID *pContext)
{
    ...
    while (!pDevice->intrThreadExit)
        ...
        TWLReadRegs(pDevice->hTWL, TWL_FULL_CODE_7_0, matrix,
            sizeof(matrix));
