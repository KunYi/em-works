...
DWORD UCD_Init(LPCTSTR pContext, DWORD dwBusContext)
{
    ...
    pa.LowPart = AM387X_USBSS_REGS_PA;
    ...
    pa.LowPart = AM387X_USB0_REGS_PA;
    ...
    pa.LowPart = AM387X_USB1_REGS_PA;
    ...
    pa.LowPart = AM387X_DEVICE_CONF_REGS_PA;
    g_Dc.pSys = (AM387X_DEVICE_CONF_REGS *)MmMapIoSpace(pa, sizeof(AM387X_DEVICE_CONF_REGS), FALSE);
    if (g_Dc.pSys == NULL) {
	...
	RETAILMSG(1,(L"UCD_Init: <><><><><><> COMPARE WITH SA\r\n"));

	PrcmDomainResetRelease(PWR_ALWAYSON2, USB2_LRST | USB1_LRST );

	EnableDeviceClocks(AM_DEVICE_USB0, TRUE); // TODO: fix PRCM
	EnableDeviceClocks(AM_DEVICE_USB1, TRUE); // TODO: fix PRCM

	g_Dc.pSys->USB_CTRL0 &= ~0x00818303;
	g_Dc.pSys->USB_CTRL0 |=  0x001E64C0;
	g_Dc.pSys->USB_CTRL1 &= ~0x00818303;
	g_Dc.pSys->USB_CTRL1 |=  0x001E64C0;
...
{
// SA has only one clock, check it for Centaurus
