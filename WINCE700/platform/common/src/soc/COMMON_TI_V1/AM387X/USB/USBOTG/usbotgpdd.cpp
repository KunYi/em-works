...
BOOL CAM3xxOTG::Init()
{
	...
	PA.QuadPart = (m_Index == 1) ?  AM387X_USB0_REGS_PA : AM387X_USB1_REGS_PA; 
    m_pUsbRegs  = (CSL_UsbRegs *)MmMapIoSpace (PA, sizeof(CSL_UsbRegs), FALSE);

    PA.QuadPart = AM387X_DEVICE_CONF_REGS_PA;
    m_pSys      = (AM387X_DEVICE_CONF_REGS *)MmMapIoSpace (PA, sizeof(AM387X_DEVICE_CONF_REGS), FALSE);
    ...
    if (USBOTG::Init()){
		...
		m_bUSB11Enabled = FALSE; // ??????? Not available on AM387
...
#pragma warning(push)
    ...
    if (m_pSys)
        MmUnmapIoSpace((PVOID)m_pSys, sizeof(AM387X_DEVICE_CONF_REGS));
