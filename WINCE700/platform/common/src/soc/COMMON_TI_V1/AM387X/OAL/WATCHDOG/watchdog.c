...
//
static AM387X_WDOG_REGS*    g_pWatchogTimerRegs;
static OMAP_DEVICE          g_WatchdogDevice = OMAP_DEVICE_NONE;
static DWORD                g_dwWatchdogPeriod;
static volatile UINT32 *    control_sec_ctrl_reg;
...
static void WatchdogRefresh(void)
{
	...
	if (fWatchdogInit == FALSE)
    {
		...
		control_sec_ctrl_reg = OALPAtoUA(AM387X_SECnFUSE_KEY_REGS_PA);
		*control_sec_ctrl_reg = 0x13; 

        // Make sure interface/functional clocks are running
		// Centaurus WDT clocks are always on
