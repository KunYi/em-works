...
static  bool isvalidpllclk(struct vps_systemvpllclk *pllclk)
{
...
static  int get_payload_size(void)
{
    int size = 0;
...
static  void assign_payload_addr(struct vps_system_ctrl *sctrl,
...
int vps_system_init(struct platform_device *pdev)
{
    ...
    r = vps_system_getplatformid((u32 *)&pid);
    if (r)
        goto exit;
...
int vps_system_deinit(struct platform_device *pdev)
{
    ...
    if (system_payload_info) {
        if (system_payload_info->vaddr) {
            vps_sbuf_free(system_payload_info);
        }
