...

{
    ...
    */
    Status = NdisMMapIoSpace((PVOID*)&EmacBaseAddr,
    ...
    */
    pAdapter->m_EmacBaseAddr    = EmacBaseAddr;
    pAdapter->m_EmacIRamBase    = EmacBaseAddr + g_EmacMemLayout.EMAC_RAM_OFFSET;
    pAdapter->m_EmacIRamBasePA  = temp + g_EmacMemLayout.EMAC_RAM_OFFSET;
    pAdapter->m_pEmacRegsBase   = (PEMACREGS)(EmacBaseAddr + g_EmacMemLayout.EMAC_REG_OFFSET);
    pAdapter->m_pEmacCtlRegs    = (PEMACCTRLREGS)(EmacBaseAddr + g_EmacMemLayout.EMAC_CTRL_OFFSET);
    pAdapter->m_pMdioRegsBase   = (PEMACMDIOREGS)(EmacBaseAddr + g_EmacMemLayout.EMAC_MDIO_OFFSET);
...

{
    ...
    EmacTxBufDesBasePa = pAdapter->m_EmacIRamBasePA + EMAC_TX_DESC_BASE;
...

{
    ...
    DEBUGMSG(DBG_FUNC, (L"---> NICInitRecv: HwStatus=%d \r\n", pAdapter->m_HwStatus));
    ...
    EmacRcvBufDesBasePa = (pAdapter->m_EmacIRamBasePA + EMAC_RX_DESC_BASE);
...
NDIS_STATUS NICInitializeAdapter(PEMAC_ADAPTER  pAdapter)
{
...
RETAILMSG(1,(L"NICInitializeAdapter Base %08X; Mdio %08X; Ctrl %08X\r\n", 
	pEmacRegs, pMdioRegs, pEmacCtlRegs));
...
RETAILMSG(TRUE, (TEXT("NICInitializeAdapter: mdio->REVID 0x%x\r\n"), pMdioRegs->REVID));
    pMdioRegs->CONTROL = ((clkdiv & 0xFF) | (MDIO_CONTROL_ENABLE) |
                           (MDIO_CONTROL_FAULTEN) | (MDIO_CONTROL_FAULT));
    ...
    while(!(pMdioRegs->ALIVE) && Count < 10000)
    {
        ++Count;
    }
    ...
    if(pMdioRegs->ALIVE)
        ...
        RETAILMSG(1, (L"NICInitializeAdapter: NO PHY is ACTIVE yet!!!!\r\n")); 

RETAILMSG(1, (L"NICInitializeAdapter: pMdioRegs->ALIVE = 0x%x; LINK=0x%08X; wait Count=%d\r\n", 
    pMdioRegs->ALIVE, pMdioRegs->LINK, Count));
	
	pAdapter->m_ActivePhy = pAdapter->m_PhyAddress;
	if (pAdapter->m_device = AM_DEVICE_EMAC0){
		pMdioRegs->USERPHYSEL0 = pAdapter->m_ActivePhy;
		pAdapter->m_LinkStatus = (pMdioRegs->LINK & (1 << pAdapter->m_ActivePhy))? UP : DOWN;
		pMdioRegs->USERPHYSEL0 |= (BIT(6));
	} else {
		pMdioRegs->USERPHYSEL1 = pAdapter->m_ActivePhy;
		pAdapter->m_LinkStatus = (pMdioRegs->LINK & (1 << pAdapter->m_ActivePhy))? UP : DOWN;
		pMdioRegs->USERPHYSEL1 |= (BIT(6));
	}
	
    RETAILMSG(1,(L"LINK STATUS %s; dev %s; LINK %04X; PHY %d\r\n", 
        (pAdapter->m_LinkStatus == UP)? L"UP" : L"DOWN",   
        (pAdapter->m_device == AM_DEVICE_EMAC0)? L"EMAC0" : L"EMAC1",
        pMdioRegs->LINK, pAdapter->m_ActivePhy));
