...
extern BOOL BSPIrdaEnable();
extern BOOL BSPIrdaSetMode(irMode_c mode);
    ...
    try {
        ...
        if (UseIR) {
            ...
            INSREG32BF(&pHWHead->pUartReg->UCR1, 
                       UART_UCR1_IREN, 
                       UART_UCR1_IREN_ENABLE);

            //Set IrDA reception to active low;
            OUTREG32(&(pHWHead->pUartReg->UCR4), 
                     INREG32(&(pHWHead->pUartReg->UCR4))&~CSP_BITFMASK(UART_UCR4_INVR));

            CspSerialSetTxIvt(pHWHead->pUartReg);
...
VOID SL_Open(PVOID pContext)
{
    ...
    try {
        ...
        CspSerialSetMux(pHWHead->pUartReg);
        ...
        if (pHWHead->UseIrDA)
        {
            bIrdaret = BSPIrdaEnable(TRUE);
            if(!bIrdaret)
            {
                RETAILMSG(TRUE,(TEXT("BSPIrdaEnable returned FALSE. IRDA not supported")));
                ASSERT(!bIrdaret);
            }
            bIrdaret = BSPIrdaSetMode(SIR_MODE);
            if(!bIrdaret)
            {
                RETAILMSG(TRUE,(TEXT("BSPIrdaSetMode returned FALSE. IRDA not supported")));
                ASSERT(!bIrdaret);
            }
        }
...
INTERRUPT_TYPE SL_GetIntrType(PVOID pContext)
{
    ...
    try {
        ...
        else if ((INREG32(&pHWHead->pUartReg->USR2) & CSP_BITFVAL(UART_USR2_TXDC,UART_USR2_TXDC_SET)) 
        ...
        else if (((INREG32(&pHWHead->pUartReg->USR1) & CSP_BITFVAL(UART_USR1_RRDY, UART_USR1_RRDY_SET))
        ...
        else if (((INREG32(&pHWHead->pUartReg->USR1) & CSP_BITFVAL(UART_USR1_DTRD, UART_USR1_DTRD_SET)) 
        ...
        else if (INREG32(&pHWHead->pUartReg->USR2) & CSP_BITFVAL(UART_USR2_IRINT, UART_USR2_IRINT_SET))    //Infrared
        {
        ...
        else    // Rx using SDMA
        {
            *pByteNumber = 0;
            ...
            while (TargetRoom > 0)
            {
                ...
                {
                    // no left-over bytes available. Wait/check for next
                    // SDMA buffer descriptor to complete.
                ...
                if (copyCount)
                {
                   /* LPBYTE pBuffer = (LPBYTE)MapCallerPtr(pTargetBuffer + *pByteNumber, 
                                              copyCount);
                    */
...
VOID SL_TxIntrHandler(PVOID pContext, PUCHAR pSourceBuffer, ULONG *pByteNumber)
{
    ...
    *pByteNumber = 0;  // In case we don't send anything below.
    ...
    try
    {
        ...
        {
            // issue SDMA requests for the requested bytes on available Tx 
            // buffer descriptors. 
            bufId = 0;
            while ((NumberOfBytes > 0) && (bufId < SERIAL_MAX_DESC_COUNT_TX))
            {
                    ...
                    (pSerHead->dmaTxBufFirstUseBmp & (1 << bufId)))
                {
                    ...
                    /*pBuffer = (LPBYTE)MapCallerPtr(pSourceBuffer + *pByteNumber, 
                                       curByteCount);
                    */
