...
void *PfClass::PfAllocPhysMem(UINT32 cbSize, PPhysicalAddress pPhysicalAddress, PUINT pDriverVirtualAddress)
{
    ...
    *pDriverVirtualAddress = (ULONG)AllocPhysMem(cbSize, PAGE_READWRITE, 0, 0, pPhysicalAddress);
    ...
    lpUserAddr = VirtualAllocEx(pvProcess, 0, SourceSize, MEM_RESERVE, PAGE_NOACCESS, NULL); 
    if (lpUserAddr == NULL)
    {
...
#pragma warning(pop)                                  
    {
        FreePhysMem((LPVOID )*pDriverVirtualAddress);
        VirtualFreeEx(pvProcess, lpUserAddr, 0, MEM_RELEASE, 0);
        PF_FUNCTION_EXIT();
        return NULL;
    }
...
BOOL PfClass::pfFreePhysMem(pPfAllocMemoryParams pBitsStreamBufMemParams) 
{
    ...
    bResult = VirtualFreeEx((void*)GetCallerVMProcessId(), (PVOID)pBitsStreamBufMemParams->userVirtAddr, MEM_DECOMMIT, pBitsStreamBufMemParams->size, 0); 
