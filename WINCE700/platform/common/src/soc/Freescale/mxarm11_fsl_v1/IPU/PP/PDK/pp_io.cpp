...
DWORD POP_Init(LPCTSTR pContext)
{
    ...
    //Create a named event to indicate that the PP driver has been loaded.
    m_hPPLoadEvent = CreateEvent(NULL, FALSE, FALSE, PP_LOAD_EVENT_NAME);
    if (m_hPPLoadEvent == NULL)
    {
        DEBUGMSG(ZONE_INIT|ZONE_ERROR, (TEXT("POP_Init:CreateEvent failed during PP load\r\n")));
        return NULL;
    }
    ...
    //Set the named event to indicate the display driver that the PP is loaded.
    SetEvent(m_hPPLoadEvent);
