    ...
    )
{
    ...
    DEBUGMSG(ZONE_FUNC, (_T("ATA_MX31: DSK_Init+")));
    ...
    if (pDisk) {
        ...
        if (NULL == pDisk->m_pSterileIoRequest) {
            ...
            // Clear undo flag for hDevKey since delete pDisk would close the key already
            dwUndo &= ~DSKINIT_UNDO_CLS_KEY_DEVICE;
            goto exit;
        }
        ...
        if(!BSPATAEnableClock(TRUE)) // Enable the clock before initialization
        {
        ...
        if (!pDisk->Init(hActiveKey)) {
            delete pDisk;
            pDisk = NULL;
            if(!BSPATAEnableClock(FALSE)) // Disable clock if init fails
            {
        ...
        if(!BSPATAEnableClock(FALSE)) // Disable clock after initialization
        {
            DEBUGMSG(ZONE_WARNING, (TEXT(" ATA Clock cannot be disabled!\r\n")));
        }
    ...
    )
{
    ...
    DEBUGMSG(ZONE_FUNC, (_T("ATA_MX31: DSK_Deinit+")));
    ...
    )
{
    ...
    DEBUGMSG(ZONE_FUNC, (_T("ATA_MX31: DSK_Open+")));
    ...
    )
{
    ...
    DEBUGMSG(ZONE_FUNC, (_T("ATA_MX31: DSK_Close+")));
    ...
    PDWORD pOverlapped)
{
    ...
    DEBUGMSG(ZONE_FUNC, (_T("ATA_MX31: DSK_IOControl+")));
...
{
    DEBUGMSG(ZONE_FUNC, (_T("ATA_MX31: DSK_PowerUp+")));
    DEBUGMSG(ZONE_FUNC, (_T("ATA_MX31: DSK_PowerUp-")));
}
...
{
    DEBUGMSG(ZONE_FUNC, (_T("ATA_MX31: DSK_PowerDown+")));
    DEBUGMSG(ZONE_FUNC, (_T("ATA_MX31: DSK_PowerDown-")));
}
