...
BOOL CDisk::Identify()
{
    ...
    if (SendIdentifyDevice(FALSE)) { // fIsAtapi=FALSE
        m_fAtapiDevice = FALSE;
        // MX31 IDE/ATA controller tweak for supporting a DMA-enabled ATAPI device
    ...
    )
{
    ...
    if(!BSPATAEnableClock(TRUE)) // Enable the clock before IOCTL handle
    {
    ...
    if(!BSPATAEnableClock(FALSE)) // Disable the clock after IOCTL handle
    {
    ...
    )
{
    ...
    while (dwNumSectors) {
        ...
        dwSectorsToTransfer = (dwNumSectors > MAX_SECT_PER_COMMAND_DMA) ? MAX_SECT_PER_COMMAND_DMA : dwNumSectors;
