...
void OEMCacheRangeFlush(VOID *pAddress, DWORD length, DWORD flags)
{
    ...
    // ARM Errata 326296 - Disable interrupts for Data Cache clears
    // ARM Errata 371025 - Disable interrupts duringing way invalidataion 
    // ARM Errata 399301 - Disable interrupts duringing data cache clean
    // ARM Errata 405875 - Disable interrupts I Cache index
    ...
    if (flags & (CACHE_SYNC_FLUSH_I_TLB | CACHE_SYNC_INSTRUCTIONS)) {
         //Flush BTAC whenever on all context switches
         // ARM11 errata Item #353494
         OALClearBTAC();
    }
    
    if ((flags & CACHE_SYNC_INSTRUCTIONS) != 0) {
        // ARM errata #325157 - Always flush all ICACHE instead of single line    
        OALFlushICache();
    }
