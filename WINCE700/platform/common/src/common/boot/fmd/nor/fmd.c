    ...
    )
{
    ...
    if (s_fmd.reservedRegions > _countof(s_fmd.reservedRegion))
        {
    ...
    if (s_fmd.regions > _countof(s_fmd.region))
        {
        OALMSG(OAL_ERROR, (L"ERROR: FMD_Init!BuildLayoutInfo: "
            L"To many regions (%d)\r\n", s_fmd.regions
            ));
    ...
    if ((s_fmd.regions + 1) > _countof(s_fmd.firstSector))
        {
        OALMSG(OAL_ERROR, (L"ERROR: FMD_Init!BuildLayoutInfo: "
            L"To many regions (%d)\r\n", s_fmd.regions
            ));
    ...
    )
{
    ...
    UNREFERENCED_PARAMETER(activePath);
    ...
    while (sectors-- > 0)
        ...
        while (sectorSize <= s_fmd.blockSize)
            ...
            if (IS_VALID_FLS(pAddress + sectorSize))
                {
                pLayoutSector = pAddress + sectorSize;
                break;
                }
            // Multiply by two
            sectorSize <<= 1;
    ...
    )
{
    ...
    UNREFERENCED_PARAMETER(pSectorInfo);
    ...
    )
{
    ...
    switch(code)
        ...
        case IOCTL_FMD_GET_RESERVED_TABLE:
            ...
            ULONG size;
            if (regions > _countof(s_fmd.reservedRegion))
                {
                regions = _countof(s_fmd.reservedRegion);
                }
