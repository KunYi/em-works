...
VOID OALBLMenuSetMacAddress(OAL_BLMENU_ITEM *pMenu)
{
    ...
    // Read input line
    if (OALBLMenuReadLine(buffer, _countof(buffer)) == 0) goto cleanUp;
...
VOID OALBLMenuSetDeviceId(OAL_BLMENU_ITEM * pMenu)
{
    ...
    // Read input line
    if (OALBLMenuReadLine(deviceId, _countof(deviceId)) == 0) goto cleanUp;
...
WCHAR OALBLMenuReadKey(BOOL wait)
{
    ...
    while ((key = (CHAR)OEMReadDebugByte()) == OEM_DEBUG_READ_NODATA && wait);
