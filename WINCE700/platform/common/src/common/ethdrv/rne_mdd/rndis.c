...
BOOL RndisInit( BYTE *pbBaseAddress, DWORD dwLogicalLocation, USHORT MacAddr[3])
{
    DWORD dwWaitMSec;
#define INITIALTIME_SEC 100

    UNREFERENCED_PARAMETER(dwLogicalLocation);
...
// Initialize The Ethnet Frame Buffer
    ...
    if (PDDInit(&(RndisMdd.PddCharacteristics), pbBaseAddress)) {
        ...
        // flip the "individually administered" bit to generate the peer's MAC address
...
//          PDD_ISR();
    ...
    if (RndisMdd.dwDeviceState == RNDIS_INITIALIZED) {
        ...
        while (OEMKitlGetSecs()-dwStartSec<=25) {
            PVOID pDataBuffer=NULL;
            WORD  wLength;
            RndisMdd.PddCharacteristics.ISRHandler(&dwWaitMSec);
...
void MddInitializeCriticalSection(CRITICAL_SECTION *pCS) {UNREFERENCED_PARAMETER(pCS);};
void MddDeleteCriticalSection(CRITICAL_SECTION *pCS){UNREFERENCED_PARAMETER(pCS);};
void MddEnterCriticalSection(CRITICAL_SECTION *pCS){UNREFERENCED_PARAMETER(pCS);};
void MddLeaveCriticalSection(CRITICAL_SECTION *pCS){UNREFERENCED_PARAMETER(pCS);};
