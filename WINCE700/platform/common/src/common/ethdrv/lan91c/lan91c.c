...
#define BUFFER_WAIT_TIMEOUT       100       // 100 ms
...
static UINT16 PhyRead(UINT8 address, UINT8 reg);
...
BOOL LAN91CInit(UINT8 *pAddress, UINT32 logicalLocation, UINT16 mac[3])
{
    ...
    KITL_RETAILMSG(ZONE_KITL_ETHER, (
        "+LAN91CInit(0x%08x, 0x%08x, 0x%08x)\r\n", pAddress, logicalLocation, mac
        ));
    ...
    if ((INPORT16(&g_pLAN91C->BANKSEL) & 0xFF00) != 0x3300) {
        KITL_RETAILMSG(ZONE_ERROR, (
            "ERROR: LAN91CInit: Network Chip not found at 0x%08x\r\n", pAddress
            ));
        goto cleanUp;
    }
    ...
    case CHIP_ID_LAN91C111:
        ...
        // autonegotiate - wait at least 1500ms as specified in spec Pg 40
        // using 3000ms to be safe
        KITL_RETAILMSG(TRUE, (
            "LAN91CInit: Waiting for Auto-Negotiation to complete..."
            ));
        {
            UINT32 startTime = OALGetTickCount();
            while ((OALGetTickCount() - startTime) < 3000);
        }

        // query ANEG_ACK bit
        if ((PhyRead(0, 1) & 0x20) == 0)
        {
            KITL_RETAILMSG(TRUE, (
                "failed!\r\n"
                ));
            // fall through, allow initialization to continue anyways
        }
        else
        {
            KITL_RETAILMSG(TRUE, (
...
cleanUp:
    KITL_RETAILMSG(ZONE_KITL_ETHER, (
        "-LAN91CInit(mac = %02x:%02x:%02x:%02x:%02x:%02x, rc = %d)\r\n",
...
UINT16 LAN91CSendFrame(UINT8 *pBuffer, UINT32 length)
{
    ...
    while ((OALGetTickCount() - startTime) < BUFFER_WAIT_TIMEOUT)
    {
    ...
    if ((INPORT16(&g_pLAN91C->INTR) & INTR_ALLOC) == 0)
        {
        KITL_RETAILMSG(ZONE_ERROR, (
            "ERROR: LAN91CSendFrame: Timed out allocating frame.\r\n"
            ));
        return 1;
        }
    ...
    if (INPORT16(&g_pLAN91C->PNRARR) & 0x8000)
        {
        KITL_RETAILMSG(ZONE_ERROR, (
            "ERROR: LAN91CSendFrame: Failed to allocate frame.\r\n"
            ));
        return 1;
        }
    ...
    while ((OALGetTickCount() - startTime) < TIMEOUT_VALUE)
        {
        if (INPORT16(&g_pLAN91C->INTR) & INTR_TX) break;
        }

    if ((INPORT16(&g_pLAN91C->INTR) & INTR_TX) == 0)
        {
        KITL_RETAILMSG(ZONE_ERROR, (
            "ERROR: LAN91CSendFrame: Timed out waiting for the transfer "
            "to complete.\r\n"
            ));
        return 1;
    }
    ...
    if (rc & (EPH_STAT_TXUNRN | EPH_STAT_SQET | EPH_STAT_LOSTCARR | EPH_STAT_LATCOL | EPH_STAT_16COL))
        ...
        KITL_RETAILMSG(ZONE_ERROR, ("ERROR: LAN91CSendFrame: status = ( "));
        if (rc & EPH_STAT_TXUNRN)
            {
            KITL_RETAILMSG(ZONE_ERROR, ("TXUNRN "));
            }
        if (rc & EPH_STAT_SQET)
            {
            KITL_RETAILMSG(ZONE_ERROR, ("SQET "));
            }
        if (rc & EPH_STAT_LOSTCARR)
            {
            KITL_RETAILMSG(ZONE_ERROR, ("LOSTCARR "));
            }
        if (rc & EPH_STAT_LATCOL)
            {
            KITL_RETAILMSG(ZONE_ERROR, ("LATCOL "));
            }
        if (rc & EPH_STAT_16COL)
            {
            KITL_RETAILMSG(ZONE_ERROR, ("16COL "));
            }
        KITL_RETAILMSG(ZONE_ERROR, (")\r\n"));
    ...
    while ((INPORT16(&g_pLAN91C->INTR) & INTR_RX) != 0) {
        ...
        if ((code & (STAT_ALGNERR|STAT_BADCRC|STAT_LONG|STAT_SHORT)) == 0) {
            ...
            else
            {
                ...
                KITL_RETAILMSG(ZONE_ERROR, ("ERROR: LAN91CGetFrame: "
                    "Packet size (%d) > than buf len (%d)\r\n", length, *pLength
                    ));
                length = 0;
                bErr = TRUE;
            }
...
{
    KITL_RETAILMSG(ZONE_KITL_ETHER, ("+LAN91CEnableInts\r\n"));
...
{
    KITL_RETAILMSG(ZONE_KITL_ETHER, ("+LAN91CDisableInts\r\n"));
...
VOID LAN91CCurrentPacketFilter(UINT32 filter)
{
    ...
    KITL_RETAILMSG(ZONE_KITL_ETHER, (
        "+LAN91CCurrentPacketFilter(0x%08x)\r\n", filter
        ));
    ...
    KITL_RETAILMSG(ZONE_KITL_ETHER, ("-LAN91CCurrentPacketFilter\r\n"));
}
...
BOOL LAN91CMulticastList(UINT8 *pAddresses, UINT32 count)
{
    ...
    KITL_RETAILMSG(ZONE_KITL_ETHER, ("-LAN91CMulticastList(rc = 1)\r\n"));
    return TRUE;
}
...
static UINT16 PhyRead(UINT8 PHYaddr, UINT8 PHYreg)
{
    INT32 i;
    UINT16 mask;
    UINT16 mii_reg;
    ...
    for (i = 0; i < clk_idx; ++i)
    {
        ...
        // Clock Hi - output data clocked in.
    ...
    for (i = 0; i < 16; ++i)
    {
        ...
        // Clock Low - input data is available.
        {
        UINT8 curBit = ((INPORT16(&g_pLAN91C->MGMT) & MGMT_MDI) != 0);
        PHYdata |= curBit << (15-i);
        }

        // Clock Hi
        OUTPORT16(&g_pLAN91C->MGMT, (UINT16)(mii_reg | MGMT_MCLK));
        OALStall(50);
    }
